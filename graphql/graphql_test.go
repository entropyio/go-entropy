...
func createGQLService(t *testing.T, stack *node.Node) {
	...
	ethBackend, err := entropy.New(stack, ethConf)
	if err != nil {
		t.Fatalf("could not create eth backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := blockchain.GenerateChain(config.TestChainConfig, ethBackend.BlockChain().Genesis(),
		ethash.NewFaker(), ethBackend.ChainDb(), 10, func(i int, gen *blockchain.BlockGen) {})
...
func createGQLServiceWithTransactions(t *testing.T, stack *node.Node) {
	...
	signer := model.LatestSigner(ethConf.Genesis.Config)

	legacyTx, _ := model.SignNewTx(key, signer, &model.LegacyTx{
		Nonce:    uint64(0),
		To:       &dad,
	...
	envelopTx, _ := model.SignNewTx(key, signer, &model.AccessListTx{
	...
	// Create some blocks and import them
	chain, _ := blockchain.GenerateChain(config.TestChainConfig, ethBackend.BlockChain().Genesis(),
		ethash.NewFaker(), ethBackend.ChainDb(), 1, func(i int, b *blockchain.BlockGen) {
