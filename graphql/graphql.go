...
func (t *Transaction) resolve(ctx context.Context) (*model.Transaction, error) {
	if t.tx == nil {
	...
	case model.DynamicFeeTxType:
		if t.block != nil {
			if baseFee, _ := t.block.BaseFeePerGas(ctx); baseFee != nil {
				...
				return (hexutil.Big)(*mathutil.BigMin(new(big.Int).Add(tx.GasTipCap(), baseFee.ToInt()), tx.GasFeeCap())), nil
			}
		}
		return hexutil.Big(*tx.GasPrice()), nil
	default:
...
func (t *Transaction) EffectiveGasPrice(ctx context.Context) (*hexutil.Big, error) {
	...
	return (*hexutil.Big)(mathutil.BigMin(new(big.Int).Add(tx.GasTipCap(), header.BaseFee), tx.GasFeeCap())), nil
}
...
func (t *Transaction) From(ctx context.Context, args BlockNumberArgs) (*Account, error) {
	...
	signer := model.LatestSigner(t.backend.ChainConfig())
...
func (t *Transaction) getReceipt(ctx context.Context) (*model.Receipt, error) {
	if _, err := t.resolve(ctx); err != nil {
		return nil, err
	}
	if t.block == nil {
...
// it if necessary.
func (b *Block) resolve(ctx context.Context) (*model.Block, error) {
	if b.block != nil {
		return b.block, nil
	}
...
func (b *Block) resolveHeader(ctx context.Context) (*model.Header, error) {
...
// if necessary.
func (b *Block) resolveReceipts(ctx context.Context) ([]*model.Receipt, error) {
	if b.receipts == nil {
		hash := b.hash
		if hash == (common.Hash{}) {
...
// `Log` objects.
func runFilter(ctx context.Context, be entropyapi.Backend, filter *filters.Filter) ([]*Log, error) {
...
func (b *Block) Call(ctx context.Context, args struct {
	Data entropyapi.TransactionArgs
}) (*CallResult, error) {
	if b.numberOrHash == nil {
		_, err := b.resolve(ctx)
		if err != nil {
	...
	result, err := entropyapi.DoCall(ctx, b.backend, args.Data, *b.numberOrHash, nil, b.backend.RPCEVMTimeout(), b.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	gas, err := entropyapi.DoEstimateGas(ctx, b.backend, args.Data, *b.numberOrHash, b.backend.RPCGasCap())
	return Long(gas), err
}
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := entropyapi.DoCall(ctx, p.backend, args.Data, pendingBlockNr, nil, p.backend.RPCEVMTimeout(), p.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := Long(1)
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	gas, err := entropyapi.DoEstimateGas(ctx, p.backend, args.Data, pendingBlockNr, p.backend.RPCGasCap())
	return Long(gas), err
}
...
func (r *Resolver) SendRawTransaction(ctx context.Context, args struct{ Data hexutil.Bytes }) (common.Hash, error) {
	...
	hash, err := entropyapi.SubmitTransaction(ctx, r.backend, tx)
	return hash, err
}
...
type SyncState struct {
	progress entropyio.SyncProgress
}
