...
func (ec *Client) getBlock(ctx context.Context, method string, args ...interface{}) (*model.Block, error) {
	...
	if head.UncleHash == model.EmptyUncleHash && len(body.UncleHashes) > 0 {
		return nil, fmt.Errorf("server returned non-empty uncle list but block header indicates no uncles")
	}
	if head.UncleHash != model.EmptyUncleHash && len(body.UncleHashes) == 0 {
		return nil, fmt.Errorf("server returned empty uncle list but block header indicates uncles")
	}
	if head.TxHash == model.EmptyRootHash && len(body.Transactions) > 0 {
	...
	if len(body.UncleHashes) > 0 {
		uncles = make([]*model.Header, len(body.UncleHashes))
		reqs := make([]rpc.BatchElem, len(body.UncleHashes))
		for i := range reqs {
			reqs[i] = rpc.BatchElem{
				Method: "entropy_getUncleByBlockHashAndIndex",
	...
	txs := make([]*model.Transaction, len(body.Transactions))
	...
	return model.NewBlockWithHeader(head).WithBody(txs, uncles), nil
}
