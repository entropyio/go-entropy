...
func newTestBackend(t *testing.T) (*node.Node, []*model.Block) {
	...
	if err != nil {
		t.Fatalf("can't create new entropy service: %v", err)
	}
...
func generateTestChain() (*blockchain.Genesis, []*model.Block) {
	db := rawdb.NewMemoryDatabase()
	configObj := config.TestChainConfig
	genesis := &blockchain.Genesis{
		Config:    configObj,
		Alloc:     blockchain.GenesisAlloc{testAddr: {Balance: testBalance, Storage: map[common.Hash]common.Hash{testSlot: testValue}}},
	...
	blocks, _ := blockchain.GenerateChain(configObj, gblock, engine, db, 1, generate)
	blocks = append([]*model.Block{gblock}, blocks...)
	return genesis, blocks
}

func TestGEntropyClient(t *testing.T) {
...
func testGetProof(t *testing.T, clientObj *rpc.Client) {
	ec := New(clientObj)
	ethcl := client.NewClient(clientObj)
...
func testSubscribePendingTransactions(t *testing.T, clientObj *rpc.Client) {
	ec := New(clientObj)
	ethcl := client.NewClient(clientObj)
	...
	// Create transaction
	tx := model.NewTransaction(0, common.Address{1}, big.NewInt(1), 22000, big.NewInt(1), nil)
