...
var (
	...
	eip1559Config *config.ChainConfig
)
...
func transaction(nonce uint64, gaslimit uint64, key *ecdsa.PrivateKey) *model.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *model.Transaction {
	tx, _ := model.SignTx(model.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), model.HomesteadSigner{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey, bytes uint64) *model.Transaction {
	data := make([]byte, bytes)
	rand.Read(data)

	tx, _ := model.SignTx(model.NewTransaction(nonce, common.Address{}, big.NewInt(0), gaslimit, gasprice, data), model.HomesteadSigner{}, key)
	return tx
}

func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key *ecdsa.PrivateKey) *model.Transaction {
	tx, _ := model.SignNewTx(key, model.LatestSignerForChainID(config.TestChainConfig.ChainID), &model.DynamicFeeTx{
		ChainID:    config.TestChainConfig.ChainID,
		Nonce:      nonce,
		GasTipCap:  tip,
		GasFeeCap:  gasFee,
		Gas:        gaslimit,
...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	return setupTxPoolWithConfig(config.TestChainConfig)
}

func setupTxPoolWithConfig(config *config.ChainConfig) (*TxPool, *ecdsa.PrivateKey) {
...
func validateEvents(events chan NewTxsEvent, count int) error {
	var received []*model.Transaction
...
func deriveSender(tx *model.Transaction) (common.Address, error) {
	return model.Sender(model.HomesteadSigner{}, tx)
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	pool.AddRemotesSync([]*model.Transaction{tx0, tx1})
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := model.SignTx(model.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), model.HomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := model.SignTx(model.NewTransaction(0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil), signer, key)
	tx2, _ := model.SignTx(model.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil), signer, key)
	tx3, _ := model.SignTx(model.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil), signer, key)
...
func TestTransactionGapFilling(t *testing.T) {
	...
	pool.AddRemotesSync([]*model.Transaction{
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	configObj := testTxPoolConfig
	configObj.NoLocals = nolocals
	configObj.GlobalQueue = configObj.AccountQueue*3 - 1 // reduce the queue limits to shorten test time (-1 to make it non divisible)

	pool := NewTxPool(configObj, config.TestChainConfig, blockchain)
	defer pool.Stop()
	...
	txs := make(model.Transactions, 0, 3*configObj.GlobalQueue)
	for len(txs) < cap(txs) {
	...
	for addr, list := range pool.queue {
		if list.Len() > int(configObj.AccountQueue) {
			t.Errorf("addr %x: queued accounts overflown allowance: %d > %d", addr, list.Len(), configObj.AccountQueue)
		}
		queued += list.Len()
	}
	if queued > int(configObj.GlobalQueue) {
		t.Fatalf("total transactions overflow allowance: %d > %d", queued, configObj.GlobalQueue)
	}
	// Generate a batch of transactions from the local account and import them
	txs = txs[:0]
	for i := uint64(0); i < 3*configObj.GlobalQueue; i++ {
	...
	if nolocals {
		...
		if queued := pool.queue[crypto.PubkeyToAddress(local.PublicKey)].Len(); uint64(queued) != 3*configObj.GlobalQueue {
			t.Fatalf("local account queued transaction count mismatch: have %v, want %v", queued, 3*configObj.GlobalQueue)
		}
	}
}
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	configObj := testTxPoolConfig
	configObj.Lifetime = time.Second
	configObj.NoLocals = nolocals
	...
	time.Sleep(2 * configObj.Lifetime)
...
func TestTransactionPendingGlobalLimiting(t *testing.T) {
	...
	configObj := testTxPoolConfig
	configObj.GlobalSlots = configObj.AccountSlots * 10
	...
	for _, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for j := 0; j < int(configObj.GlobalSlots)/len(keys)*2; j++ {
	...
	if pending > int(configObj.GlobalSlots) {
		t.Fatalf("total pending transactions overflow allowance: %d > %d", pending, configObj.GlobalSlots)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
...
func TestTransactionCapClearsFromAll(t *testing.T) {
	...
	configObj := testTxPoolConfig
	configObj.AccountSlots = 2
	configObj.AccountQueue = 2
	configObj.GlobalSlots = 8
	...
	txs := model.Transactions{}
	for j := 0; j < int(configObj.GlobalSlots)*2; j++ {
...
func TestTransactionPendingMinimumAllowance(t *testing.T) {
	...
	configObj := testTxPoolConfig
	configObj.GlobalSlots = 1

	pool := NewTxPool(configObj, config.TestChainConfig, blockchain)
	defer pool.Stop()

	// Create a number of test account and fund them
	keys := make([]*ecdsa.PrivateKey, 5)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		testAddBalance(pool, crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000))
	...
	for _, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for j := 0; j < int(configObj.AccountSlots)*2; j++ {
			txs = append(txs, transaction(nonces[addr], 100000, key))
			nonces[addr]++
		}
	}
	...
	for addr, list := range pool.pending {
		if list.Len() != int(configObj.AccountSlots) {
			t.Errorf("addr %x: total pending transactions mismatch: have %d, want %d", addr, list.Len(), configObj.AccountSlots)
		}
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
...
func TestTransactionPoolUnderpricing(t *testing.T) {
	...
	configObj := testTxPoolConfig
	configObj.GlobalSlots = 2
	configObj.GlobalQueue = 2
...
func TestTransactionPoolStableUnderpricing(t *testing.T) {
	...
	configObj := testTxPoolConfig
	configObj.GlobalSlots = 128
	configObj.GlobalQueue = 0
	...
	txs := model.Transactions{}
	for i := uint64(0); i < configObj.GlobalSlots; i++ {
	...
	pending, queued := pool.Stats()
	if pending != int(configObj.GlobalSlots) {
		t.Fatalf("pending transactions mismatched: have %d, want %d", pending, configObj.GlobalSlots)
	}
	if queued != 0 {
		t.Fatalf("queued transactions mismatched: have %d, want %d", queued, 0)
	}
	if err := validateEvents(events, int(configObj.GlobalSlots)); err != nil {
		t.Fatalf("original event firing failed: %v", err)
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
...
func TestDualHeapEviction(t *testing.T) {
	...
	var (
		highTip, highCap *model.Transaction
		baseFee          int
	)

	check := func(tx *model.Transaction, name string) {
	...
	add := func(urgent bool) {
		for i := 0; i < 20; i++ {
			...
			pool.AddRemotesSync([]*model.Transaction{tx})
		}
		pending, queued := pool.Stats()
...
func TestTransactionDeduplication(t *testing.T) {
	...
	txs := make([]*model.Transaction, 16)
	for i := 0; i < len(txs); i++ {
		txs[i] = pricedTransaction(uint64(i), 100000, big.NewInt(1), key)
	}
	var firsts []*model.Transaction
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	configObj := testTxPoolConfig
	configObj.NoLocals = nolocals
	configObj.Journal = journal
	configObj.Rejournal = time.Second
	...
	statedb.SetNonce(crypto.PubkeyToAddress(local.PublicKey), 2)
	<-pool.requestReset(nil, nil)
	time.Sleep(2 * configObj.Rejournal)
	pool.Stop()
...
func TestTransactionStatusCheck(t *testing.T) {
	...
	// Create the test account to check various transaction statuses with
	keys := make([]*ecdsa.PrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		testAddBalance(pool, crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000))
...
func benchmarkPoolBatchInsert(b *testing.B, size int, local bool) {
	...
	batches := make([]model.Transactions, b.N)
	for i := 0; i < b.N; i++ {
		batches[i] = make(model.Transactions, size)
		for j := 0; j < size; j++ {
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	locals := make([]*model.Transaction, 4096+1024) // Occupy all slots
	for i := 0; i < len(locals); i++ {
		locals[i] = transaction(uint64(i), 100000, key)
	}
	remotes := make([]*model.Transaction, 1000)
	...
	for i := 0; i < b.N; i++ {
		...
		for i := 0; i < len(remotes); i++ {
			pool.AddRemotes([]*model.Transaction{remotes[i]})
		}
		pool.Stop()
	}
}
