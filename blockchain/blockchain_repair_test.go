...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	...
	defer engine.Close()
	if snapshots {
		configObj.SnapshotLimit = 256
		configObj.SnapshotWait = true
	}
	chain, err := NewBlockChain(db, configObj, config.TestChainConfig, engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
	// If sidechain blocks are needed, make a light chain and import it
	var sideblocks model.Blocks
	if tt.sidechainBlocks > 0 {
		sideblocks, _ = GenerateChain(config.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), tt.sidechainBlocks, func(i int, b *BlockGen) {
	...
	canonblocks, _ := GenerateChain(config.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), tt.canonicalBlocks, func(i int, b *BlockGen) {
	...
	newChain, err := NewBlockChain(db, nil, config.TestChainConfig, engine, evm.Config{}, nil, nil)
	if err != nil {
...
// TestIssue23496 tests scenario described in https://github.com/entropy/go-entropy/pull/23496#issuecomment-926393893
...
func TestIssue23496(t *testing.T) {
	...
	blocks, _ := GenerateChain(config.TestChainConfig, genesis, engine, rawdb.NewMemoryDatabase(), 4, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{0x02})
		b.SetDifficulty(big.NewInt(1000000))
	})
	...
	chain, err = NewBlockChain(db, nil, config.TestChainConfig, engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
