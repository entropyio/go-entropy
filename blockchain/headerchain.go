...
func (hc *HeaderChain) Reorg(headers []*model.Header) error {
	...
	hc.currentHeader.Store(model.CopyHeader(last))
...
// procedure.
func (hc *HeaderChain) WriteHeaders(headers []*model.Header) (int, error) {
	if len(headers) == 0 {
		return 0, nil
	}
	...
	// Commit to disk!
	if err := batch.Write(); err != nil {
		log.Critical("Failed to write headers", "error", err)
	}
...
func (hc *HeaderChain) writeHeadersAndSetHead(headers []*model.Header, forker *ForkChoice) (*headerWriteResult, error) {
...
// or a side chain.
func (hc *HeaderChain) InsertHeaderChain(chain []*model.Header, start time.Time, forker *ForkChoice) (WriteStatus, error) {
	if hc.procInterrupt() {
		return 0, errors.New("aborted")
	}
...
type (
	...
	UpdateHeadBlocksCallback func(database.KeyValueWriter, *model.Header) (uint64, bool)
...
func (hc *HeaderChain) SetHead(head uint64, updateFn UpdateHeadBlocksCallback, delFn DeleteBlockContentCallback) {
	...
	for hdr := hc.CurrentHeader(); hdr != nil && hdr.Number.Uint64() > head; hdr = hc.CurrentHeader() {
		...
		if updateFn != nil {
			...
			if force && newHead < head {
				log.Warning("Force rewinding till ancient limit", "head", newHead)
				head = newHead
			}
		}
		...
		if err := markerBatch.Write(); err != nil {
			log.Critical("Failed to update chain markers", "error", err)
		}
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to rewind block", "error", err)
	}
