...
func IntrinsicGas(data []byte, contractCreation, homestead bool) (uint64, error) {
	...
	transactionLog.Debugf("IntrinsicGas: gas=%d, data=%X", gas, data)
	return gas, nil
}
...
func (st *StateTransition) useGas(amount uint64) error {
	...
	transactionLog.Debugf("useGas: remainGas=%d, useAmount=%d", st.gas, amount)
	return nil
}
...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	var (
		vm = st.vm
		// evm errors do not effect consensus and are therefor
	...
	if vmerr != nil {
		blockChainLog.Debug("VM returned with error", "err", vmerr)
...
func (st *StateTransition) refundGas() {
	...
	transactionLog.Debugf("refundGas: %d, from=%X", st.gas, st.msg.From())
}

// gasUsed returns the amount of gas used up by the state transition.
func (st *StateTransition) gasUsed() uint64 {
	transactionLog.Debugf("gasUsed: %d", st.initialGas-st.gas)
