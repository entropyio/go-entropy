...
type snapshotTestBasic struct {
	...
	gendb   database.Database
	engine  consensus.Engine
}

func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*model.Block) {
	...
	chain, err := NewBlockChain(db, cacheConfig, config.TestChainConfig, engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
	blocks, _ := GenerateChain(config.TestChainConfig, genesis, engine, gendb, basic.chainBlocks, func(i int, b *BlockGen) {})
...
func (basic *snapshotTestBasic) verify(t *testing.T, chain *BlockChain, blocks []*model.Block) {
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, nil, config.TestChainConfig, snaptest.engine, evm.Config{}, nil, nil)
	if err != nil {
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, nil, config.TestChainConfig, snaptest.engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	gappedBlocks, _ := GenerateChain(config.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.gapped, func(i int, b *BlockGen) {})
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, config.TestChainConfig, snaptest.engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	configObj := &CacheConfig{
		TrieCleanLimit: 256,
		TrieDirtyLimit: 256,
		TrieTimeLimit:  5 * time.Minute,
		SnapshotLimit:  0,
	}
	newchain, err := NewBlockChain(snaptest.db, configObj, config.TestChainConfig, snaptest.engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(config.TestChainConfig, blocks[len(blocks)-1], snaptest.engine, snaptest.gendb, snaptest.newBlocks, func(i int, b *BlockGen) {})
	...
	_, err = NewBlockChain(snaptest.db, configObj, config.TestChainConfig, snaptest.engine, evm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
