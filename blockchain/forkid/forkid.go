...
func NewID(chain *blockchain.BlockChain) ID {
	return newID(
		chain.Config(),
		chain.Genesis().Hash(),
...
func newID(config *config.ChainConfig, genesis common.Hash, head uint64) ID {
...
func NewFilter(chain *blockchain.BlockChain) Filter {
	return newFilter(
		chain.Config(),
		chain.Genesis().Hash(),
		func() uint64 {
