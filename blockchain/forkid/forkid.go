...
var log = logger.NewLogger("[forkid]")
...
// NewID calculates the Entropy fork ID from the chain config, genesis hash, and head.
func NewID(configObj *config.ChainConfig, genesis common.Hash, head uint64) ID {
	// Calculate the starting checksum from the genesis hash
	hash := crc32.ChecksumIEEE(genesis[:])

	// Calculate the current fork checksum and the next fork block
	var next uint64
	for _, fork := range gatherForks(configObj) {
		if fork <= head {
...
// NewIDWithChain calculates the Entropy fork ID from an existing chain instance.
...
func NewStaticFilter(config *config.ChainConfig, genesis common.Hash) Filter {
...
func newFilter(config *config.ChainConfig, genesis common.Hash, headfn func() uint64) Filter {
...
func gatherForks(configObj *config.ChainConfig) []uint64 {
	// Gather all the fork block numbers via reflection
	kind := reflect.TypeOf(config.ChainConfig{})
	conf := reflect.ValueOf(configObj).Elem()
