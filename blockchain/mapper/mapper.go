...
func InspectDatabase(db database.Database) error {
	...
	for it.Next() {
		...
		if count%1000 == 0 && time.Since(logged) > 8*time.Second {
			mapperLog.Info("Inspecting database", "count", count, "elapsed", common.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
	}
	...
	if unaccounted > 0 {
		mapperLog.Error("Database contains unaccounted data", "size", unaccounted)
	}
	return nil
}
