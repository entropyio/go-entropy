...
func TestLookupStorage(t *testing.T) {
	tests := []struct {
		name                 string
		writeTxLookupEntries func(database.Writer, *model.Block)
	}{
		{
			"DatabaseV6",
		...
		},
		{
			...
			func(db database.Writer, block *model.Block) {
				for index, tx := range block.Transactions() {
					...
					data, _ := rlputil.EncodeToBytes(entry)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			block := model.NewBlock(&model.Header{Number: big.NewInt(314)}, txs, nil, nil)
