...
func InitDatabaseFromFreezer(db database.Database) error {
	...
	var (
		number  = ^uint64(0) // -1
		results = make(chan *model.Block, 4*runtime.NumCPU())
	)
	...
	for i := uint64(0); i < frozen; i++ {
		...
		for !queue.Empty() {
			...
			block = queue.PopItem().(*model.Block)
			next++
			...
			if time.Since(logged) > 8*time.Second {
				mapperLog.Info("Initializing chain from ancient data", "number", block.Number(), "hash", block.Hash(), "total", frozen-1, "elapsed", common.PrettyDuration(time.Since(start)))
				logged = time.Now()
			}
		}
	}
	...
	mapperLog.Info("Initialized chain from ancient data", "number", frozen-1, "hash", hash, "elapsed", common.PrettyDuration(time.Since(start)))
	return nil
}
