...
var (
	...
	benchRootFunds  = mathutil.BigPow(2, 100)
)
...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		tx, _ := model.SignTx(model.NewTransaction(gen.TxNonce(benchRootAddr), toaddr, big.NewInt(1), gas, nil, data), model.HomesteadSigner{}, benchRootKey)
		gen.AddTx(tx)
	}
}
...
func genTxRing(naccounts int) func(int, *BlockGen) {
	...
	return func(i int, gen *BlockGen) {
		...
		for {
			...
			tx, _ = model.SignTx(tx, model.HomesteadSigner{}, ringKeys[from])
			gen.AddTx(tx)
			from = to
		}
	}
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		db, err = mapper.NewLevelDBDatabase(dir, 128, 128, "")
		if err != nil {
	...
	gspec := genesis.Genesis{
		...
		Alloc:  genesis.GenesisAlloc{benchRootAddr: {Balance: benchRootFunds}},
	}
	genesisObj := gspec.MustCommit(db)
	chain, _ := GenerateChain(gspec.Config, genesisObj, ethash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, ethash.NewFaker(), evm.Config{}, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func makeChainForBench(db database.Database, full bool, count uint64) {
	...
	for n := uint64(0); n < count; n++ {
		...
		mapper.WriteCanonicalHash(db, hash, n)
		mapper.WriteTd(db, hash, n, big.NewInt(int64(n+1)))

		if full || n == 0 {
			block := model.NewBlockWithHeader(header)
			mapper.WriteBody(db, hash, n, block.Body())
			mapper.WriteReceipts(db, hash, n, nil)
		}
	}
}
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		...
		db, err := mapper.NewLevelDBDatabase(dir, 128, 1024, "")
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, config.TestChainConfig, ethash.NewFaker(), evm.Config{}, nil)
		if err != nil {
		...
		for n := uint64(0); n < count; n++ {
			...
			if full {
				hash := header.Hash()
				mapper.ReadBody(db, hash, n)
				mapper.ReadReceipts(db, hash, n, chain.Config())
			}
		}
		chain.Stop()
		db.Close()
