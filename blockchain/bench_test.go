...
var (
	...
	benchRootFunds  = mathutil.BigPow(2, 200)
)
...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		signer := model.MakeSigner(gen.config, big.NewInt(int64(i)))
		gasPrice := big.NewInt(0)
		if gen.header.BaseFee != nil {
			gasPrice = gen.header.BaseFee
		}
		tx, _ := model.SignNewTx(benchRootKey, signer, &model.LegacyTx{
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, ethash.NewFaker(), evm.Config{}, nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, config.TestChainConfig, ethash.NewFaker(), evm.Config{}, nil, nil)
		if err != nil {
