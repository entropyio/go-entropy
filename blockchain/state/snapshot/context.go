...
type generatorContext struct {
	...
	db      database.KeyValueStore // Key-value store containing the snapshot data
	account *holdableIterator      // Iterator of account snapshot data
	storage *holdableIterator      // Iterator of storage snapshot data
	batch   database.Batch         // Database batch for writing batch data atomically
	logged  time.Time              // The timestamp when last generation progress was displayed
}

// newGeneratorContext initializes the context for generation.
func newGeneratorContext(stats *generatorStats, db database.KeyValueStore, accMarker []byte, storageMarker []byte) *generatorContext {
...
func (ctx *generatorContext) removeStorageBefore(account common.Hash) {
	...
	for iter.Next() {
		...
		count++
		_ = ctx.batch.Delete(key, "storageEntry")
		if ctx.batch.ValueSize() > database.IdealBatchSize {
			_ = ctx.batch.Write()
			ctx.batch.Reset()
		}
	}
...
func (ctx *generatorContext) removeStorageLeft() {
	...
	for iter.Next() {
		count++
		_ = ctx.batch.Delete(iter.Key(), "storageEntry")
		if ctx.batch.ValueSize() > database.IdealBatchSize {
			_ = ctx.batch.Write()
			ctx.batch.Reset()
		}
