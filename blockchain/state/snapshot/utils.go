...
func CheckDanglingStorage(chaindb database.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb database.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
	...
	for it.Next() {
		...
		if data := rawdb.ReadAccountSnapshot(chaindb, common.BytesToHash(accKey)); len(data) == 0 {
			log.Warning("Dangling storage - missing account", "account", fmt.Sprintf("%#x", accKey), "storagekey", fmt.Sprintf("%#x", k))
...
// snapshot difflayers.
func checkDanglingMemStorage(db database.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db database.KeyValueStore, hash common.Hash) error {
