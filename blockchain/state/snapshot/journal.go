...
func ParseGeneratorStatus(generatorBlob []byte) string {
	...
	if err := rlp.DecodeBytes(generatorBlob, &generator); err != nil {
		log.Warning("failed to decode snapshot generator", "err", err)
		return ""
	}
...
func loadAndParseJournal(db database.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadSnapshot(diskdb database.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	if err != nil {
		log.Warning("Failed to load new-format journal", "error", err)
	...
	if head := snapshot.Root(); head != root {
		...
		// disk layer.
		log.Warning("Snapshot is not continuous with chain", "snaproot", head, "chainroot", root)
	}
...
func iterateJournal(db database.KeyValueReader, callback journalCallback) error {
	journal := rawdb.ReadSnapshotJournal(db)
	if len(journal) == 0 {
		log.Warning("Loaded snapshot journal", "diffs", "missing")
		return nil
	}
	...
	if err != nil {
		log.Warning("Failed to resolve the journal version", "error", err)
		return errors.New("failed to resolve journal version")
	}
	if version != journalVersion {
		log.Warning("Discarded the snapshot journal with wrong version", "required", journalVersion, "got", version)
	...
	if baseRoot := rawdb.ReadSnapshotRoot(db); baseRoot != parent {
		log.Warning("Loaded snapshot journal", "diskroot", baseRoot, "diffs", "unmatched")
