...
func copyDestructs(destructs map[common.Hash]struct{}) map[common.Hash]struct{} {
	copyObj := make(map[common.Hash]struct{})
...
func copyAccounts(accounts map[common.Hash][]byte) map[common.Hash][]byte {
	copyObj := make(map[common.Hash][]byte)
...
func copyStorage(storage map[common.Hash]map[common.Hash][]byte) map[common.Hash]map[common.Hash][]byte {
	copyObj := make(map[common.Hash]map[common.Hash][]byte)
	for accHash, slots := range storage {
		copyObj[accHash] = make(map[common.Hash][]byte)
		for slotHash, blob := range slots {
			copyObj[accHash][slotHash] = blob
		}
	}
	return copyObj
}
...
func BenchmarkSearchSlot(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = layer.Storage(accountKey, storageKey)
	}
}
...
func BenchmarkJournal(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _ = layer.Journal(new(bytes.Buffer))
	}
}
