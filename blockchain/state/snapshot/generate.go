...
func generateSnapshot(diskdb database.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash) *diskLayer {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to write initialized state marker", "err", err)
	}
	base := &diskLayer{
		diskdb:     diskdb,
		triedb:     triedb,
	...
	log.Debugf("Start snapshot generation. root:%x", root)
	return base
}
...
func (dl *diskLayer) proveRange(ctx *generatorContext, owner common.Hash, root common.Hash, prefix []byte, kind string, origin []byte, max int, valueConvertFn func([]byte) ([]byte, error)) (*proofResult, error) {
	...
	if origin == nil && !diskMore {
		...
		for i, key := range keys {
			_ = stackTr.TryUpdate(key, vals[i])
		}
...
func (dl *diskLayer) generateRange(ctx *generatorContext, owner common.Hash, root common.Hash, prefix []byte, kind string, origin []byte, max int, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	if result.valid() {
		...
		log.Debug("Proved state range", "last", hexutil.Encode(last))
	...
	log.Debugf("Detected outdated state range. last:%s, err:%+v", hexutil.Encode(last), result.proofErr)
	...
	var snapNodeCache database.KeyValueStore
	if len(result.keys) > 0 {
	...
	log.Debugf("Regenerated state range. root:%x, last:%x, count:%d, created:%d, updated:%d, untouched:%d, deleted:%d",
		root, hexutil.Encode(last), count, created, updated, untouched, deleted)
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > database.IdealBatchSize || abort != nil {
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	log.Infof("Generated state snapshot. accounts:%d, slots:%d, storage:%.2f, dangling:%d, elapsed:%dµs",
		stats.accounts, stats.slots, stats.storage, stats.dangling, common.PrettyDuration(time.Since(stats.start))/1000)
