...
type (
	...
	trieGeneratorFn func(db database.KeyValueWriter, owner common.Hash, in chan trieKV, out chan common.Hash)
...
func stackTrieGenerate(db database.KeyValueWriter, owner common.Hash, in chan trieKV, out chan common.Hash) {
	t := trie.NewStackTrieWithOwner(db, owner)
	for leaf := range in {
		_ = t.TryUpdate(leaf.key[:], leaf.value)
	}
