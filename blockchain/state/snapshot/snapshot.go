...
// Tree is an Entropy state snapshot tree. It consists of one persistent base
...
//   a background thread.
func New(diskdb database.KeyValueStore, triedb *trie.TrieDatabase, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
	...
	if disabled {
		log.Warning("Snapshot maintenance disabled (syncing)")
		return snap, nil
	}
	if err != nil {
		if rebuild {
			log.Warning("Failed to load snapshot, regenerating", "err", err)
			snap.Rebuild(root)
			return snap, nil
		}
...
func (t *Tree) Disable() {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to disable snapshots", "err", err)
	}
}
...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	for hash := range bottom.destructSet {
		...
		for it.Next() {
			key := it.Key()
			_ = batch.Delete(key, "storageSnapshots")
			...
			if batch.ValueSize() > database.IdealBatchSize {
				if err := batch.Write(); err != nil {
					log.Critical("Failed to write storage deletions", "err", err)
				}
				batch.Reset()
			}
		}
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to write leftover snapshot", "err", err)
	}
