...
type Pruner struct {
	...
	headHeader    *model.Header
	snaptree      *snapshot.Tree
}

// NewPruner creates the pruner instance.
func NewPruner(db database.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
	...
	if bloomSize < 256 {
		log.Warning("Sanitizing bloomfilter size", "provided(MB)", bloomSize, "updated(MB)", 256)
		bloomSize = 256
	}
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb database.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db database.Database, trieCachePath string) error {
...
func extractGenesis(db database.Database, stateBloom *stateBloom) error {
	...
	for accIter.Next(true) {
		...
		if hash != (common.Hash{}) {
			_ = stateBloom.Put(hash.Bytes(), nil, "bloom")
		}
		// If it's a leaf node, yes we are touching an account,
		// dig into the storage trie further.
		if accIter.Leaf() {
			var acc model.StateAccount
			...
			if !bytes.Equal(acc.CodeHash, emptyCode) {
				_ = stateBloom.Put(acc.CodeHash, nil, "bloom")
			}
		}
	}
