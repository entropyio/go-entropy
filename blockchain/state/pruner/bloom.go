...
func (f stateBloomHasher) Write([]byte) (n int, err error) { panic("not implemented") }
func (f stateBloomHasher) Sum([]byte) []byte               { panic("not implemented") }
...
func (bloom *stateBloom) Put(key []byte, value []byte, from string) error {
	log.Debugf("stateBloom Put %s. key:%x, vSize:%d", from, key, len(value))
...
func (bloom *stateBloom) Delete([]byte, string) error { panic("not supported") }
