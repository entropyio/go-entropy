...
func NewChainIndexer(chainDb, indexDb database.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
// queue.
func (c *ChainIndexer) eventLoop(currentHeader *model.Header, events chan ChainEvent, sub event.Subscription) {
