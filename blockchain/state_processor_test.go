// Copyright 2020 The go-entropy Authors
// This file is part of the go-entropy library.
//
// The go-entropy library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// The go-entropy library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.
//
// You should have received a copy of the GNU Lesser General Public License
// along with the go-entropy library. If not, see <http://www.gnu.org/licenses/>.
...
func TestStateProcessorErrors(t *testing.T) {
	var (
		configObj = &config.ChainConfig{
			...
			Ethash:         new(config.EthashConfig),
		}
		signer  = model.LatestSigner(configObj)
		key1, _ = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		key2, _ = crypto.HexToECDSA("0202020202020202020202020202020202020202020202020202002020202020")
	)
	var makeTx = func(key *ecdsa.PrivateKey, nonce uint64, to common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte) *model.Transaction {
		tx, _ := model.SignTx(model.NewTransaction(nonce, to, amount, gasLimit, gasPrice, data), signer, key)
		return tx
	}
	var mkDynamicTx = func(nonce uint64, to common.Address, gasLimit uint64, gasTipCap, gasFeeCap *big.Int) *model.Transaction {
		tx, _ := model.SignTx(model.NewTx(&model.DynamicFeeTx{
			Nonce:     nonce,
			GasTipCap: gasTipCap,
			GasFeeCap: gasFeeCap,
			Gas:       gasLimit,
	...
	{ // Tests against a 'recent' chain definition
		...
		for i, tt := range []struct {
			...
			{ // ErrNonceTooLow
				txs: []*model.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), config.TxGas, big.NewInt(875000000), nil),
					makeTx(key1, 0, common.Address{}, big.NewInt(0), config.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrNonceTooHigh
				txs: []*model.Transaction{
					makeTx(key1, 100, common.Address{}, big.NewInt(0), config.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrNonceMax
				txs: []*model.Transaction{
					makeTx(key2, math.MaxUint64, common.Address{}, big.NewInt(0), config.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrInsufficientFundsForTransfer
				txs: []*model.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(1000000000000000000), config.TxGas, big.NewInt(875000000), nil),
				},
			...
			{ // ErrInsufficientFunds
				txs: []*model.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), config.TxGas, big.NewInt(900000000000000000), nil),
				},
			...
			{ // ErrIntrinsicGas
				txs: []*model.Transaction{
					makeTx(key1, 0, common.Address{}, big.NewInt(0), config.TxGas-1000, big.NewInt(875000000), nil),
				},
			...
			{ // ErrFeeCapTooLow
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, big.NewInt(0), big.NewInt(0)),
				},
			...
			{ // ErrTipVeryHigh
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, tooBigNumber, big.NewInt(1)),
				},
			...
			{ // ErrFeeCapVeryHigh
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, big.NewInt(1), tooBigNumber),
				},
			...
			{ // ErrTipAboveFeeCap
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, big.NewInt(2), big.NewInt(1)),
				},
			...
			{ // ErrInsufficientFunds
				...
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, big.NewInt(1), big.NewInt(50000000000000)),
				},
			...
			{ // Another ErrInsufficientFunds, this one to ensure that feecap/tip of max u256 is allowed
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas, bigNumber, bigNumber),
				},
			...
			_, err := blockchain.InsertChain(model.Blocks{block})
			if err == nil {
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		...
		for i, tt := range []struct {
			...
			{ // ErrTxTypeNotSupported
				txs: []*model.Transaction{
					mkDynamicTx(0, common.Address{}, config.TxGas-1000, big.NewInt(0), big.NewInt(0)),
				},
...
func GenerateBadBlock(parent *model.Block, engine consensus.Engine, txs model.Transactions, config *config.ChainConfig) *model.Block {
	header := &model.Header{
		ParentHash: parent.Hash(),
		Coinbase:   parent.Coinbase(),
		Difficulty: engine.CalcDifficulty(&fakeChainReader{config}, parent.Time()+10, &model.Header{
	...
	for _, tx := range txs {
		txh := tx.Hash()
		hasher.Write(txh[:])
		receipt := model.NewReceipt(nil, false, cumulativeGas+tx.Gas())
	...
	return model.NewBlock(header, txs, nil, receipts, trie.NewStackTrie(nil))
}
