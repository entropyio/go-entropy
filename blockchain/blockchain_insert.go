...
type insertIterator struct {
	chain model.Blocks // Chain of blocks being iterated over
...
func newInsertIterator(chain model.Blocks, results <-chan error, validator Validator) *insertIterator {
...
func (it *insertIterator) next() (*model.Block, error) {
...
func (it *insertIterator) peek() (*model.Block, error) {
	// If we reached the end of the chain, abort
	if it.index+1 >= len(it.chain) {
		return nil, nil
	}
...
func (it *insertIterator) previous() *model.Header {
	if it.index < 1 {
		return nil
	}
...
func (it *insertIterator) first() *model.Block {
	return it.chain[0]
}
