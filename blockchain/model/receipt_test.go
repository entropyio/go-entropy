...
func TestLegacyReceiptDecoding(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := rlputil.DecodeBytes(enc, &dec); err != nil {
...
func encodeAsStoredReceiptRLP(want *Receipt) ([]byte, error) {
	...
	return rlputil.EncodeToBytes(stored)
}
...
func TestDeriveFields(t *testing.T) {
	...
	if err := receipts.DeriveFields(config.TestChainConfig, hash, number.Uint64(), txs); err != nil {
		t.Fatalf("DeriveFields(...) = %v, want <nil>", err)
	}
	// Iterate over all the computed fields and check that they're correct
	signer := MakeSigner(config.TestChainConfig, number)
