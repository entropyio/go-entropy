...
// [deprecated by entropy/63]
// StorageBlock defines the RLP encoding of a Block stored in the
// state database. The StorageBlock encoding contains fields that
// would otherwise need to be recomputed.
type StorageBlock Block

// "external" block encoding. used for entropy protocol, etc.
...
func (b *Block) DecodeRLP(s *rlputil.Stream) error {
	var eb extblock
	_, size, _ := s.Kind()
	...
	b.size.Store(common.StorageSize(rlputil.ListSize(size)))
	return nil
}

// EncodeRLP serializes b into the Entropy RLP block format.
func (b *Block) EncodeRLP(w io.Writer) error {
	return rlputil.Encode(w, extblock{
		Header: b.header,
		Txs:    b.transactions,
		Uncles: b.uncles,
	})
...
// [deprecated by entropy/63]
func (b *StorageBlock) DecodeRLP(s *rlputil.Stream) error {
	var sb storageblock
...
func (sorter blockSorter) Len() int { return len(sorter.blocks) }
func (sorter blockSorter) Swap(i, j int) {
	sorter.blocks[i], sorter.blocks[j] = sorter.blocks[j], sorter.blocks[i]
}
func (sorter blockSorter) Less(i, j int) bool { return sorter.by(sorter.blocks[i], sorter.blocks[j]) }
