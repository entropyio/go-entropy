...
func TestEIP1559BlockEncoding(t *testing.T) {
	...
	check("Time", block.Time(), uint64(1426516743))
	check("Size", block.Size(), common.StorageSize(len(blockEnc)))
	check("BaseFee", block.BaseFee(), new(big.Int).SetUint64(config.InitialBaseFee))
...
func makeBenchBlock() *Block {
	var (
		...
		signer   = LatestSigner(config.TestChainConfig)
		uncles   = make([]*Header, 3)
	)
	header := &Header{
		Difficulty: mathutil.BigPow(11, 11),
		Number:     mathutil.BigPow(2, 9),
		GasLimit:   12345678,
		GasUsed:    1476322,
		Time:       9876543,
	...
	for i := range txs {
		amount := mathutil.BigPow(2, int64(i))
