...
func TestDeriveSha(t *testing.T) {
	...
	for len(txs) < 1000 {
		exp := model.DeriveSha(txs, trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := model.DeriveSha(txs, trie.NewStackTrie(nil))
...
func TestEIP2718DeriveSha(t *testing.T) {
	for _, tc := range []struct {
		...
		txs := model.Transactions{&t1, &t2}
		model.DeriveSha(txs, d)
...
func TestFuzzDeriveSha(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		seed := rndSeed + i
		exp := model.DeriveSha(newDummy(i), trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := model.DeriveSha(newDummy(i), trie.NewStackTrie(nil))
		if !bytes.Equal(got[:], exp[:]) {
...
func TestDerivableList(t *testing.T) {
	...
	for i, tc := range tcs[1:] {
		exp := model.DeriveSha(flatList(tc), trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase())))
		got := model.DeriveSha(flatList(tc), trie.NewStackTrie(nil))
		if !bytes.Equal(got[:], exp[:]) {
...
func genTxs(num uint64) (model.Transactions, error) {
	...
	newTx := func(i uint64) (*model.Transaction, error) {
		signer := model.NewEIP155Signer(big.NewInt(18))
		utx := model.NewTransaction(i, addr, new(big.Int), 0, new(big.Int).SetUint64(10000000), nil)
		tx, err := model.SignTx(utx, signer, key)
		return tx, err
	}
...
func printList(l model.DerivableList) {
