...
var log = logger.NewLogger("[scwallet]")
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Entropy account
				if nextAddrs[i] == (common.Address{}) {
					if nextAcc, err = w.session.derive(nextPaths[i]); err != nil {
						w.log.Warning("Smartcard wallet account derivation failed", "err", err)
						break
					}
				...
				if err != nil {
					w.log.Warning("Smartcard wallet balance retrieval failed", "err", err)
					break
				}
				nonce, err = w.deriveChain.NonceAt(context, nextAddrs[i], nil)
				if err != nil {
					w.log.Warning("Smartcard wallet nonce retrieval failed", "err", err)
					break
				}
