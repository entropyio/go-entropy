...
var log = logger.NewLogger("[usbwallet]")
...
type driver interface {
	...
	SignTx(path accounts.DerivationPath, tx *model.Transaction, chainID *big.Int) (common.Address, *model.Transaction, error)
...
type wallet struct {
	...
	deriveChain     entropyio.ChainStateReader // Blockchain state reader to discover used account with
			...
			for empty := false; !empty; {
				...
				if nextAddrs[i] == (common.Address{}) {
					if nextAddrs[i], err = w.driver.Derive(nextPaths[i]); err != nil {
						w.log.Warning("USB wallet account derivation failed", "err", err)
						break
					}
				}
				...
				if err != nil {
					w.log.Warning("USB wallet balance retrieval failed", "err", err)
					break
				}
				nonce, err = w.deriveChain.NonceAt(context, nextAddrs[i], nil)
				if err != nil {
					w.log.Warning("USB wallet nonce retrieval failed", "err", err)
					break
				}
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain entropyio.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) SignTx(account accounts.Account, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	w.stateLock.RLock() // Comms have own mutex, this is for the state fields
	defer w.stateLock.RUnlock()
...
// Since USB wallets don't rely on passphrases, these are silently ignored.
func (w *wallet) SignTxWithPassphrase(account accounts.Account, passphrase string, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
