...
import (
	...
	"github.com/entropyio/go-entropy/accounts/usbwallet/trezor"
...
// or whether the Entropy app was not started on it.
...
// and returning the Entropy address located on that derivation path.
...
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *model.Transaction, chainID *big.Int) (common.Address, *model.Transaction, error) {
	if w.device == nil {
...
// Entropy address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EntropyAddress)
	if _, err := w.trezorExchange(&trezor.EntropyGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *model.Transaction, chainID *big.Int) (common.Address, *model.Transaction, error) {
	// Create the transaction initiation message
	data := tx.Data()
	length := uint32(len(data))

	request := &trezor.EntropySignTx{
	...
	response := new(trezor.EntropyTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EntropyTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
	// Extract the Entropy signature and do a sanity validation
	...
	if chainID == nil {
		...
		signer = model.NewEIP155Signer(chainID)
	...
	sender, err := model.Sender(signer, signed)
	if err != nil {
		return common.Address{}, nil, err
	}
...
func (w *trezorDriver) trezorExchange(req proto.Message, results ...proto.Message) (int, error) {
	...
	for len(payload) > 0 {
		...
		w.log.Debug("Data chunk sent to the Trezor", "chunk", hexutil.Bytes(chunk))
	...
	for {
		...
		w.log.Debug("Data chunk received from the Trezor", "chunk", hexutil.Bytes(chunk))
	...
	return 0, fmt.Errorf("trezor: expected reply model %s, got %s", expected, trezor.Name(kind))
}
