...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *model.Transaction, chainID *big.Int) (common.Address, *model.Transaction, error) {
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *model.Transaction, chainID *big.Int) (common.Address, *model.Transaction, error) {
	// Flatten the derivation path into the Ledger request
	path := make([]byte, 1+4*len(derivationPath))
	path[0] = byte(len(derivationPath))
	for i, component := range derivationPath {
...
func (w *ledgerDriver) ledgerExchange(opcode ledgerOpcode, p1 ledgerParam1, p2 ledgerParam2, data []byte) ([]byte, error) {
	...
	for i := 0; len(apdu) > 0; i++ {
		...
		w.log.Debug("Data chunk sent to the Ledger", "chunk", hexutil.Bytes(chunk))
	...
	for {
		...
		w.log.Debug("Data chunk received from the Ledger", "chunk", hexutil.Bytes(chunk))
