...
func (ks *KeyStore) refreshWallets() {
	...
	for _, accountObj := range accs {
		...
		if len(ks.wallets) == 0 || ks.wallets[0].URL().Cmp(accountObj.URL) > 0 {
			wallet := &keystoreWallet{account: accountObj, keystore: ks}
		...
		if ks.wallets[0].Accounts()[0] == accountObj {
...
func (ks *KeyStore) SignTx(a accounts.Account, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	// Look up the key to sign with and abort if it cannot be found
	ks.mu.RLock()
	defer ks.mu.RUnlock()
...
func (ks *KeyStore) SignTxWithPassphrase(a accounts.Account, passphrase string, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	_, key, err := ks.getDecryptedKey(a, passphrase)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) NewAccount(passphrase string) (accounts.Account, error) {
	_, accountObj, err := storeNewKey(ks.storage, crand.Reader, passphrase)
	if err != nil {
		return accounts.Account{}, err
	}
	// Add the account to the cache immediately rather
	// than waiting for file system notifications to pick it up.
	ks.cache.add(accountObj)
	ks.refreshWallets()
...
// ImportPreSaleKey decrypts the given Entropy presale wallet and stores
