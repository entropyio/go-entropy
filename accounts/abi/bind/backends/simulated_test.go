...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != entropy.NotFound {
		t.Fatalf("err should be `entropy.NotFound` but received %v", err)
	}
	...
	tx := model.NewContractCreation(0, big.NewInt(0), gas, gasPrice, common.FromHex(code))
	tx, _ = model.SignTx(tx, model.HomesteadSigner{}, key)
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != config.TestChainConfig {
		t.Errorf("expected sim config to equal config.AllEthashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != config.TestChainConfig {
		t.Errorf("expected sim blockchain config to equal config.AllEthashProtocolChanges, got %v", sim.config)
	}
...
func TestAdjustTime(t *testing.T) {
	sim := NewSimulatedBackend(
		blockchain.GenesisAlloc{}, 10000000,
	)
	defer sim.Close()
...
func TestNewAdjustTimeFail(t *testing.T) {
	...
	tx := model.NewTransaction(0, testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	signedTx, err := model.SignTx(tx, model.HomesteadSigner{}, testKey)
	if err != nil {
	...
	tx2 := model.NewTransaction(1, testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	signedTx2, err := model.SignTx(tx2, model.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestNonceAt(t *testing.T) {
	...
	tx := model.NewTransaction(nonce, testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	signedTx, err := model.SignTx(tx, model.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSendTransaction(t *testing.T) {
	...
	tx := model.NewTransaction(uint64(0), testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	signedTx, err := model.SignTx(tx, model.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(blockchain.GenesisAlloc{addr: {Balance: big.NewInt(config.Ether)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		name        string
		message     entropy.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", entropy.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
			...
			Value:    big.NewInt(1),
			Data:     nil,
		}, config.TxGas, nil, nil},

		{"plain transfer(invalid)", entropy.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(blockchain.GenesisAlloc{addr: {Balance: big.NewInt(config.Ether*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", entropy.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", entropy.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", entropy.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", entropy.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateEIP1559WithHighFees", entropy.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
		...
		{"EstimateEIP1559WithSuperHighFees", entropy.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1ether
			...
			Data:      nil,
		}, config.TxGas, errors.New("gas required exceeds allowance (20999)")}, // 20999=(2.2ether-0.1ether-1wei)/(1e14)
	}
	for i, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestPendingNonceAt(t *testing.T) {
	...
	tx = model.NewTransaction(uint64(1), testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	signedTx, err = model.SignTx(tx, model.HomesteadSigner{}, testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, entropy.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, entropy.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, entropy.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, entropy.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
...
func TestForkResendTx(t *testing.T) {
	...
	_tx := model.NewTransaction(0, testAddr, big.NewInt(1000), config.TxGas, gasPrice, nil)
	tx, _ := model.SignTx(_tx, model.HomesteadSigner{}, testKey)
