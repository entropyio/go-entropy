...
var log = logger.NewLogger("[external]")
...
func (api *ExternalSigner) SelfDerive(bases []accounts.DerivationPath, chain entropyio.ChainStateReader) {
...
func (api *ExternalSigner) SignText(account accounts.Account, text []byte) ([]byte, error) {
	...
	if signature[64] == 27 || signature[64] == 28 {
		...
		// the signature to entropy-type signature.
...
func (api *ExternalSigner) SignTx(account accounts.Account, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	...
	switch tx.Type() {
	case model.LegacyTxType, model.AccessListTxType:
...
func (api *ExternalSigner) SignTxWithPassphrase(account accounts.Account, passphrase string, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
