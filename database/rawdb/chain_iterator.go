...
func InitDatabaseFromFreezer(db database.Database) {
	...
	for i := uint64(0); i < frozen; {
		...
		if err != nil {
			log.Critical("Failed to init database from freezer", "err", err)
		}
		for j, h := range data {
			number := i + uint64(j)
			...
			if batch.ValueSize() > database.IdealBatchSize {
				if err := batch.Write(); err != nil {
					log.Critical("Failed to write data to db", "err", err)
				}
				batch.Reset()
			}
		}
...
func iterateTransactions(db database.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
	...
	process := func() {
		...
		for data := range rlpCh {
			...
			if err := rlp.DecodeBytes(data.rlp, &body); err != nil {
				log.Warning("Failed to decode block body", "block", data.number, "error", err)
				return
			}
...
// signal received.
func indexTransactions(db database.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	...
	for chanDelivery := range hashesCh {
		...
		for !queue.Empty() {
			...
			if batch.ValueSize() > database.IdealBatchSize {
				...
				if err := batch.Write(); err != nil {
					log.Critical("Failed writing batch to db", "error", err)
					return
				}
				batch.Reset()
			}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db database.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db database.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db database.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if from >= to {
		return
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db database.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db database.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
