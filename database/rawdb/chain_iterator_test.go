...
func TestChainIterator(t *testing.T) {
	...
	block = model.NewBlock(&model.Header{Number: big.NewInt(int64(0))}, nil, nil, nil, newHasher()) // Empty genesis block
	WriteBlock(chainDb, block)
	WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())
	for i := uint64(1); i <= 10; i++ {
		var tx *model.Transaction
		if i%2 == 0 {
			tx = model.NewTx(&model.LegacyTx{
				Nonce:    i,
			...
			tx = model.NewTx(&model.AccessListTx{
		...
		txs = append(txs, tx)
		block = model.NewBlock(&model.Header{Number: big.NewInt(int64(i))}, []*model.Transaction{tx}, nil, nil, newHasher())
		WriteBlock(chainDb, block)
		WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())
	}
...
func TestIndexTransactions(t *testing.T) {
	...
	block = model.NewBlock(&model.Header{Number: big.NewInt(int64(0))}, nil, nil, nil, newHasher())
	WriteBlock(chainDb, block)
	WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())
