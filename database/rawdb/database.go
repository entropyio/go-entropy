...
import (
	...
	"github.com/entropyio/go-entropy/database/leveldb"
...
var log = logger.NewLogger("[rawDB]")
...
func (db *nofreezedb) ModifyAncients(func(database.AncientWriteOp) error) (int64, error) {
	return 0, errNotSupported
}
...
func (db *nofreezedb) ReadAncients(fn func(reader database.AncientReaderOp) error) (err error) {
...
// storage.
func NewDatabaseWithFreezer(db database.KeyValueStore, freezer string, namespace string, readonly bool) (database.Database, error) {
	...
	if kvgenesis, _ := db.Get(headerHashKey(0), "headerHash"); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			...
			if kvhash, _ := db.Get(headerHashKey(frozen), "headerHash"); len(kvhash) == 0 {
			...
			if ReadHeadHeaderHash(db) != common.BytesToHash(kvgenesis) {
				...
				if kvblob, _ := db.Get(headerHashKey(1), "headerHash"); len(kvblob) == 0 {
...
func NewLevelDBDatabase(file string, cache int, handles int, namespace string, readonly bool) (database.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string, readonly bool) (database.Database, error) {
...
func InspectDatabase(db database.Database, keyPrefix, keyStart []byte) error {
