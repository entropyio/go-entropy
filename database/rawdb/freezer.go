...
func NewFreezer(datadir string, namespace string, readonly bool, maxTableSize uint32, tables map[string]bool) (*Freezer, error) {
	...
	if info, err := os.Lstat(datadir); !os.IsNotExist(err) {
		if info.Mode()&os.ModeSymlink != 0 {
			log.Warning("Symbolic link ancient database is not supported", "path", datadir)
...
func (f *Freezer) ReadAncients(fn func(database.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(database.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
