...
func (t *table) Has(key []byte, from string) (bool, error) {
	return t.db.Has(append([]byte(t.prefix), key...), from)
}

// Get retrieves the given prefixed key if it's present in the database.
func (t *table) Get(key []byte, from string) ([]byte, error) {
	return t.db.Get(append([]byte(t.prefix), key...), from)
}
...
func (t *table) ModifyAncients(fn func(database.AncientWriteOp) error) (int64, error) {
	return t.db.ModifyAncients(fn)
}

func (t *table) ReadAncients(fn func(reader database.AncientReaderOp) error) (err error) {
...
// provided key.
func (t *table) Put(key []byte, value []byte, from string) error {
	return t.db.Put(append([]byte(t.prefix), key...), value, from)
}

// Delete removes the given prefixed key from the database.
func (t *table) Delete(key []byte, from string) error {
	return t.db.Delete(append([]byte(t.prefix), key...), from)
}
...
func (t *table) NewBatchWithSize(size int) database.Batch {
...
func (t *table) NewSnapshot() (database.Snapshot, error) {
...
func (b *tableBatch) Put(key, value []byte, from string) error {
	return b.batch.Put(append([]byte(b.prefix), key...), value, from)
}

// Delete inserts the a key removal into the batch for later committing.
func (b *tableBatch) Delete(key []byte, from string) error {
	return b.batch.Delete(append([]byte(b.prefix), key...), from)
}
...
func (r *tableReplayer) Put(key []byte, value []byte, from string) error {
	trimmed := key[len(r.prefix):]
	return r.w.Put(trimmed, value, from)
}

// Delete implements the interface KeyValueWriter.
func (r *tableReplayer) Delete(key []byte, from string) error {
	trimmed := key[len(r.prefix):]
	return r.w.Delete(trimmed, from)
}
