...
func ReadTxLookupEntry(db database.Reader, hash common.Hash) *uint64 {
	data, _ := db.Get(txLookupKey(hash), "legacyTx")
	if len(data) == 0 {
		return nil
	}
...
func writeTxLookupEntry(db database.KeyValueWriter, hash common.Hash, numberBytes []byte) {
	if err := db.Put(txLookupKey(hash), numberBytes, "legacyTx"); err != nil {
...
func WriteTxLookupEntriesByBlock(db database.KeyValueWriter, block *model.Block) {
...
func DeleteTxLookupEntry(db database.KeyValueWriter, hash common.Hash) {
	if err := db.Delete(txLookupKey(hash), "legacyTx"); err != nil {
		log.Critical("Failed to delete transaction lookup entry", "err", err)
	}
}
...
func ReadBloomBits(db database.KeyValueReader, bit uint, section uint64, head common.Hash) ([]byte, error) {
	return db.Get(bloomBitsKey(bit, section, head), "bloomBits")
}

// WriteBloomBits stores the compressed bloom bits vector belonging to the given
// section and bit index.
func WriteBloomBits(db database.KeyValueWriter, bit uint, section uint64, head common.Hash, bits []byte) {
	if err := db.Put(bloomBitsKey(bit, section, head), bits, "bloomBits"); err != nil {
...
func DeleteBloombits(db database.Database, bit uint, from uint64, to uint64) {
	...
	for it.Next() {
		...
		_ = db.Delete(it.Key(), "bloomBits")
	}
	if it.Error() != nil {
		log.Critical("Failed to delete bloom bits", "err", it.Error())
	}
}
