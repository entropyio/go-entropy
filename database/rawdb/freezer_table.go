...
func newTable(path string, name string, readMeter metrics.Meter, writeMeter metrics.Meter, sizeGauge metrics.Gauge, maxFilesize uint32, noCompression, readonly bool) (*freezerTable, error) {
	...
	tab := &freezerTable{
		...
		name:          name,
		path:          path,
		logger:        logger.NewLogger("table:" + name),
...
func (t *freezerTable) repair() error {
	...
	for contentExp != contentSize {
		...
		if contentExp < contentSize {
			t.logger.Warning("Truncating dangling head", "indexed", common.StorageSize(contentExp), "stored", common.StorageSize(contentSize))
		...
		if contentExp > contentSize {
			t.logger.Warning("Truncating dangling indexes", "indexed", common.StorageSize(contentExp), "stored", common.StorageSize(contentSize))
...
func (t *freezerTable) truncateHead(items uint64) error {
	...
	log := t.logger.Debug
	if existing > items+1 {
		log = t.logger.Warning // Only loud warn if we delete multiple items
	}
