...
// content map.
func checkTrieContents(t *testing.T, db *TrieDatabase, root common.Hash, content map[string][]byte) {
	// Check root availability and trie contents
	trie, err := New(root, db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, root); err != nil {
...
func checkTrieConsistency(db *TrieDatabase, root common.Hash) error {
...
func testIterativeSync(t *testing.T, batch int) {
	...
	checkTrieContents(t, triedb, srcTrie.Hash(), srcData)
}
