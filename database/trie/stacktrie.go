...
func stackTrieFromPool(db database.KeyValueWriter, owner common.Hash) *StackTrie {
...
func NewStackTrieWithOwner(db database.KeyValueWriter, owner common.Hash) *StackTrie {
	return &StackTrie{
		owner:    owner,
		nodeType: emptyNode,
		db:       db,
...
func newLeaf(owner common.Hash, key, val []byte, db database.KeyValueWriter) *StackTrie {
...
func newExt(owner common.Hash, key []byte, child *StackTrie, db database.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db, owner)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
...
func (st *StackTrie) hashRec(hasher *hasher) {
	...
	if st.db != nil {
		...
		_ = st.db.Put(st.val, encodedNode, "stackTrie")
	}
}
...
func (st *StackTrie) Hash() (h common.Hash) {
	...
	hasher.sha.Reset()
	_, _ = hasher.sha.Write(st.val)
	_, _ = hasher.sha.Read(h[:])
	return h
}
...
func (st *StackTrie) Commit() (h common.Hash, err error) {
	...
	_, _ = hasher.sha.Write(st.val)
	_, _ = hasher.sha.Read(h[:])
	_ = st.db.Put(h[:], st.val, "stackTrie")
	return h, nil
}
