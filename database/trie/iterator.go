...
type NodeIterator interface {
	...
	AddResolver(database.KeyValueReader)
}
...
type nodeIterator struct {
	...
	resolver database.KeyValueReader // Optional intermediate resolver above the disk layer
}
...
func (it *nodeIterator) AddResolver(resolver database.KeyValueReader) {
...
func (it *nodeIterator) resolveHash(hash hashNode, path []byte) (node, error) {
	if it.resolver != nil {
		if blob, err := it.resolver.Get(hash, "trieNode"); err == nil && len(blob) > 0 {
...
func (it *nodeIterator) resolveBlob(hash hashNode, path []byte) ([]byte, error) {
	if it.resolver != nil {
		if blob, err := it.resolver.Get(hash, "nodeRLP"); err == nil && len(blob) > 0 {
			return blob, nil
		}
	}
...
func (it *differenceIterator) AddResolver(resolver database.KeyValueReader) {
...
func (it *unionIterator) AddResolver(resolver database.KeyValueReader) {
	panic("not implemented")
}
