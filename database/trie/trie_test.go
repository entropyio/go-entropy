...
func makeAccounts(size int) (addresses [][20]byte, accounts [][]byte) {
	...
	for i := 0; i < len(accounts); i++ {
		...
		data, _ := rlp.EncodeToBytes(&model.StateAccount{Nonce: nonce, Balance: balance, Root: root, CodeHash: code})
		accounts[i] = data
	}
...
func (s *spongeDb) NewBatchWithSize(size int) database.Batch { return &spongeBatch{s} }
func (s *spongeDb) NewSnapshot() (database.Snapshot, error)  { panic("implement me") }
...
func tempDB(tb testing.TB) *TrieDatabase {
	dir := tb.TempDir()
