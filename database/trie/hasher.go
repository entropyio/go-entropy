...
func (h *hasher) hash(n node, db *TrieDatabase, force bool) (node, node, error) {
	...
	if hashObj, dirty := n.cache(); hashObj != nil {
		if db == nil {
...
func (h *hasher) store(n node, db *TrieDatabase, force bool) (node, error) {
	...
	h.tmp.Reset()
	if err := rlputil.Encode(&h.tmp, n); err != nil {
	...
	if db != nil {
		...
		hashObj := common.BytesToHash(hashNode)

		db.lock.Lock()
		db.insert(hashObj, h.tmp, n)
		db.lock.Unlock()
