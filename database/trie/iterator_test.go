...
type loggingDb struct {
	getCount uint64
	backend  database.KeyValueStore
}
...
func (l *loggingDb) NewBatch() database.Batch {
	return l.backend.NewBatch()
}

func (l *loggingDb) NewBatchWithSize(size int) database.Batch {
	return l.backend.NewBatchWithSize(size)
}

func (l *loggingDb) NewIterator(prefix []byte, start []byte) database.Iterator {
	return l.backend.NewIterator(prefix, start)
}

func (l *loggingDb) NewSnapshot() (database.Snapshot, error) {
...
func makeLargeTestTrie() (*TrieDatabase, *SecureTrie, *loggingDb) {
