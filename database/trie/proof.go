...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb database.KeyValueWriter) error {
	...
	for i, n := range nodes {
		...
		if hash, ok := hn.(hashNode); ok || i == 0 {
			...
			_ = proofDb.Put(hash, enc, "proofNode")
		}
	}
	return nil
}
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb database.KeyValueReader) (value []byte, err error) {
	...
	wantHash := rootHash
	for i := 0; ; i++ {
		buf, _ := proofDb.Get(wantHash[:], "proofNode")
		if buf == nil {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb database.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
	...
	resolveNode := func(hash common.Hash) (node, error) {
		buf, _ := proofDb.Get(hash[:], "proofNode")
		if buf == nil {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof database.KeyValueReader) (bool, error) {
	...
	if proof == nil {
		...
		for index, key := range keys {
			_ = tr.TryUpdate(key, values[index])
		}
