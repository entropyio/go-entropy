...
type Sync struct {
	database database.DBReader        // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database database.DBReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw database.DBWriter) (int, error) {
