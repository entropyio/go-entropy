...
type cachedNode struct {
	node node   // Cached collapsed trie node, or raw rlputil data
...
// rlputil returns the raw rlputil encoded blob of the cached node, either directly from
// the cache, or by regenerating it from the collapsed node.
func (n *cachedNode) rlp() []byte {
	if node, ok := n.node.(rawNode); ok {
		return node
	}
	blob, err := rlputil.EncodeToBytes(n.node)
	if err != nil {
		panic(err)
	}
	return blob
...
// or by regenerating it from the rlputil encoded blob.
...
func NewDatabase(diskdb database.Database) *TrieDatabase {
	return &TrieDatabase{
		diskdb:    diskdb,
...
func (db *TrieDatabase) DiskDB() database.DBReader {
	return db.diskdb
}
...
func (db *TrieDatabase) Commit(node common.Hash, report bool) error {
	...
	log.Debug("Persisted trie from memory database", "nodes", nodes-len(db.nodes)+int(db.flushnodes), "size", storage-db.nodesSize+db.flushsize, "time", time.Since(start)+db.flushtime,
		"gcnodes", db.gcnodes, "gcsize", db.gcsize, "gctime", db.gctime, "livenodes", len(db.nodes), "livesize", db.nodesSize)
...
func (db *TrieDatabase) commit(hash common.Hash, batch database.Batch) error {
