...
func NewLDBDatabase(file string, cache int, handles int) (*LDBDatabase, error) {
	...
	log.Debugf("Allocated cache and file handles, cache=%d, handles=%d, file=%s", cache, handles, file)
...
func (db *LDBDatabase) Put(key []byte, value []byte) error {
	//log.Debugf("levelDB:Put key=%X, value=%X", key, value)
	return db.db.Put(key, value, nil)
}

func (db *LDBDatabase) Has(key []byte) (bool, error) {
	//log.Debugf("levelDB:Has key=%X", key)
	return db.db.Has(key, nil)
}

// Get returns the given key if it's present.
func (db *LDBDatabase) Get(key []byte) ([]byte, error) {
	//log.Debugf("levelDB:Get key=%X", key)
	dat, err := db.db.Get(key, nil)
	if err != nil {
		return nil, err
	}
	//log.Debugf("levelDB:Get value=%X", dat)
	return dat, nil
}

// Delete deletes the key from the queue and database
func (db *LDBDatabase) Delete(key []byte) error {
	//log.Debugf("levelDB:Delete key=%X", key)
