...
func (b HookedBatch) Put(key []byte, value []byte, from string) error {
	if b.OnPut != nil {
		b.OnPut(key, value)
	}
	return b.Batch.Put(key, value, from)
}

// Delete removes the key from the key-value data store.
func (b HookedBatch) Delete(key []byte, from string) error {
	if b.OnDelete != nil {
		b.OnDelete(key)
	}
	return b.Batch.Delete(key, from)
}
