...
func getJEntropy(vm *goja.Runtime) *goja.Object {
	jEntropy := vm.Get("jEntropy")
	if jEntropy == nil {
		panic(vm.ToValue("jEntropy object does not exist"))
	}
	return jEntropy.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC method that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC method (saved in jEntropy.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJEntropy(call.VM).Get("newAccount"))
	if !callable {
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJEntropy(call.VM).Get("openWallet"))
	if !callable {
...
func (b *bridge) readPinAndReopenWallet(call jsre.Call) (goja.Value, error) {
	...
	_, _ = fmt.Fprintf(b.printer, "Look at the device for number positions\n\n")
	_, _ = fmt.Fprintf(b.printer, "7 | 8 | 9\n")
	_, _ = fmt.Fprintf(b.printer, "--+---+--\n")
	_, _ = fmt.Fprintf(b.printer, "4 | 5 | 6\n")
	_, _ = fmt.Fprintf(b.printer, "--+---+--\n")
	_, _ = fmt.Fprintf(b.printer, "1 | 2 | 3\n\n")
...
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	var passwd goja.Value
	if goja.IsUndefined(call.Argument(1)) || goja.IsNull(call.Argument(1)) {
		_, _ = fmt.Fprintf(b.printer, "Unlock account %s\n", account)
	...
	unlockAccount, callable := goja.AssertFunction(getJEntropy(call.VM).Get("unlockAccount"))
	if !callable {
...
// jEntropy.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	if goja.IsUndefined(passwd) || goja.IsNull(passwd) {
		_, _ = fmt.Fprintf(b.printer, "Give password for account %s\n", account)
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJEntropy(call.VM).Get("sign"))
	if !callable {
...
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		_ = resp.Set("jsonrpc", "2.0")
		_ = resp.Set("id", req.ID)
	...
	if fn, isFunc := goja.AssertFunction(call.Argument(1)); isFunc {
		_, _ = fn(goja.Null(), goja.Null(), result)
