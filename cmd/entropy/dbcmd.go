...
func showLeveldbStats(db database.KeyValueStater) {
	if stats, err := db.Stat("leveldb.stats"); err != nil {
		log.Warning("Failed to read database stats", "error", err)
	} else {
		fmt.Println(stats)
	}
	if ioStats, err := db.Stat("leveldb.iostats"); err != nil {
		log.Warning("Failed to read database iostats", "error", err)
	} else {
		fmt.Println(ioStats)
	}
}
...
func dbDelete(ctx *cli.Context) error {
	...
	if err = db.Delete(key, "dbCmd"); err != nil {
...
type snapshotIterator struct {
	init    bool
	account database.Iterator
	storage database.Iterator
}
...
var chainExporters = map[string]func(db database.Database) utils.ChainDataIterator{
	"preimage": func(db database.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db database.Database) utils.ChainDataIterator {
...
func freezerMigrate(ctx *cli.Context) error {
	...
	start := time.Now()
	if err := db.MigrateTable("receipts", model.ConvertLegacyStoredReceipts); err != nil {
		return err
	}
...
func dbHasLegacyReceipts(db database.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
	...
	legacy, err = model.IsLegacyStoredReceipts(first)
