...
type environment struct {
	...
	uncles   map[common.Hash]*model.Header
}
...
func (env *environment) unclelist() []*model.Header {
...
type getWorkReq struct {
	...
	result chan *model.Block // non-blocking channel
	err    chan error
}
...
type worker struct {
	...
	localUncles  map[common.Hash]*model.Block // A set of side blocks generated locally as the possible uncle blocks.
	remoteUncles map[common.Hash]*model.Block // A set of side blocks as the possible uncle blocks.
	...
	snapshotReceipts model.Receipts
	...
	// External functions
	isLocalBlock func(header *model.Header) bool // Function used to determine whether the specified block is mined by local miner.
...
func newWorker(config *Config, chainConfig *config.ChainConfig, engine consensus.Engine, entropy Backend, mux *event.TypeMux, isLocalBlock func(header *model.Header) bool, init bool) *worker {
	worker := &worker{
		config:             config,
		...
		localUncles:        make(map[common.Hash]*model.Block),
		remoteUncles:       make(map[common.Hash]*model.Block),
		unconfirmed:        newUnconfirmedBlocks(entropy.BlockChain(), sealingLogAtDepth),
		...
		resultCh:           make(chan *model.Block, resultQueueSize),
	...
	if recommit < minRecommitInterval {
		log.Warning("Sanitizing miner recommit interval", "provided", recommit, "updated", minRecommitInterval)
...
func (w *worker) pendingBlockAndReceipts() (*model.Block, model.Receipts) {
	// return a snapshot to avoid contention on currentMu mutex
	w.snapshotMu.RLock()
	defer w.snapshotMu.RUnlock()
	...
	for {
		...
		case interval := <-w.resubmitIntervalCh:
			...
			if interval < minRecommitInterval {
				log.Warning("Sanitizing miner recommit interval", "provided", interval, "updated", minRecommitInterval)
	...
	for {
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				if gp := w.current.gasPool; gp != nil && gp.Gas() < config.TxGas {
					continue
				}
				...
				txset := model.NewTransactionsByPriceAndNonce(w.current.signer, txs, w.current.header.BaseFee)
...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			var (
				receipts = make([]*model.Receipt, len(task.receipts))
				logs     []*model.Log
			)
			for i, taskReceipt := range task.receipts {
				...
				*receipt = *taskReceipt
				...
				receipt.Logs = make([]*model.Log, len(taskReceipt.Logs))
			...
			log.Infof("Successfully sealed new block. number=%d, sealhash=%X, hash=%X, elapsed=%d", block.Number(), sealhash, hash,
...
func (w *worker) makeEnv(parent *model.Block, header *model.Header, coinbase common.Address) (*environment, error) {
	...
	env := &environment{
		signer:    model.MakeSigner(w.chainConfig, header.Number),
		state:     state,
		coinbase:  coinbase,
		ancestors: mapset.NewSet(),
		family:    mapset.NewSet(),
		header:    header,
		uncles:    make(map[common.Hash]*model.Header),
	}
...
func (w *worker) commitUncle(env *environment, uncle *model.Header) error {
...
func (w *worker) commitTransaction(env *environment, tx *model.Transaction) ([]*model.Log, error) {
	snap := env.state.Snapshot()

	receipt, err := blockchain.ApplyTransaction(w.chainConfig, w.chain, &env.coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, *w.chain.GetVMConfig())
	if err != nil {
...
func (w *worker) commitTransactions(env *environment, txs *model.TransactionsByPriceAndNonce, interrupt *int32) error {
	gasLimit := env.header.GasLimit
	if env.gasPool == nil {
		env.gasPool = new(blockchain.GasPool).AddGas(gasLimit)
	}
	...
	for {
		...
		case errors.Is(err, blockchain.ErrTxTypeNotSupported):
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if !genParams.noUncle {
		commitUncles := func(blocks map[common.Hash]*model.Block) {
			for hash, uncle := range blocks {
				if len(env.uncles) == 2 {
					break
				}
				if err := w.commitUncle(env, uncle.Header()); err != nil {
					log.Debug("Possible uncle rejected", "hash", hash, "reason", err)
				} else {
...
func (w *worker) generateWork(params *generateParams) (*model.Block, error) {
...
func (w *worker) getSealingBlock(parent common.Hash, timestamp uint64, coinbase common.Address, random common.Hash, noTxs bool) (chan *model.Block, chan error, error) {
	var (
...
func copyReceipts(receipts []*model.Receipt) []*model.Receipt {
	result := make([]*model.Receipt, len(receipts))
