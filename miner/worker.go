...
type worker struct {
	...
	// External functions
	isLocalBlock func(block *model.Block) bool // Function used to determine whether the specified block is mined by local miner.
...
func (w *worker) makeCurrent(parent *model.Block, header *model.Header) error {
	...
	env := &environment{
		signer:    model.NewEIP155Signer(w.chainConfig.ChainID),
		state:     state,
...
func (w *worker) commitTransactions(txs *model.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	if w.current.gasPool == nil {
		w.current.gasPool = new(blockchain.GasPool).AddGas(w.current.header.GasLimit)
	}
	...
	for {
		...
		if w.current.gasPool.Gas() < config.TxGas {
			log.Debug("Not enough gas for further transactions", "have", w.current.gasPool, "want", config.TxGas)
			break
		}
		...
		from, _ := model.Sender(w.current.signer, tx)
		// Check whether the tx is replay protected. If we're not in the EIP155 hf
		// phase, start ignoring the sender until we do.
		if tx.Protected() && !w.chainConfig.IsEIP155(w.current.header.Number) {
			log.Warning("Ignoring reply protected transaction", "hash", tx.Hash(), "eip155", w.chainConfig.EIP155Block)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	header := &model.Header{
		...
		GasLimit:      blockchain.CalcGasLimit(parent, w.config.GasFloor, w.config.GasCeil),
		Extra:         w.extra,
	...
	if len(localTxs) > 0 {
		txs := model.NewTransactionsByPriceAndNonce(w.current.signer, localTxs)
		if w.commitTransactions(txs, w.coinbase, interrupt) {
			return
		}
	}
	if len(remoteTxs) > 0 {
		txs := model.NewTransactionsByPriceAndNonce(w.current.signer, remoteTxs)
		if w.commitTransactions(txs, w.coinbase, interrupt) {
			return
		}
	}
...
func (w *worker) commit(uncles []*model.Header, interval func(), update bool, start time.Time) error {
	...
	// TODO: set claudeContext here. -- wang
	block, err := w.engine.FinalizeAndAssemble(w.chain, w.current.header, s, w.current.txs, uncles, w.current.receipts, nil)
	if err != nil {
		return err
	}
	if w.isRunning() {
		...
		case w.taskCh <- &task{receipts: receipts, state: s, block: block, createdAt: time.Now()}:
			...
			feesEth := new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(config.Ether)))

			log.Info("Commit new mining work. number=%d, sealhash=%X, uncles=%d, txs=%d, gas=%d, fees=%d, elapsed=%d",
				block.Number(), w.engine.SealHash(block.Header()),
				len(uncles), w.current.tcount, block.GasUsed(), feesEth, common.PrettyDuration(time.Since(start)))
