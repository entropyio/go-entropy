...
type chainRetriever interface {
	...
	GetBlockByNumber(number uint64) *model.Block
}
...
func (set *unconfirmedBlocks) Insert(index uint64, hash common.Hash) {
	...
	log.Infof("🔨 mined potential block. number=%d, hash=%X", index, hash)
}
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		switch {
		case header == nil:
			log.Warningf("Failed to retrieve header of mined block. number=%d, hash=%X", next.index, next.hash)
		case header.Hash() == next.hash:
			log.Infof("🔗 block reached canonical chain. number=%d, hash=%X", next.index, next.hash)
		default:
			...
			if included {
				log.Infof("⑂ block became an uncle. number=%d, hash=%X", next.index, next.hash)
			} else {
				log.Infof("😱 block lost. number=%d, hash=%X", next.index, next.hash)
			}
		}
