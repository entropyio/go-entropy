...
func (db *Dashboard) handleLogRequest(r *LogsRequest, c *client) {
	...
	if err != nil {
		log.Warning("Failed to open logdir", "path", db.logdir, "err", err)
		return
	}
	...
	if len(fileNames) < 1 {
		log.Warning("No log files in logdir", "path", db.logdir)
		return
	}
	...
	if buf, err = ioutil.ReadFile(path); err != nil {
		log.Warning("Failed to read file", "path", path, "err", err)
		return
	}
	chunk, end := prepLogs(buf)
	if end < 0 {
		log.Warning("The file doesn't contain valid logs", "path", path)
		return
	}
	c.msg <- &Message{
		Logs: &LogsMessage{
...
func (db *Dashboard) streamLogs() {
	...
	if opened, err = os.OpenFile(filepath.Join(db.logdir, files[i].Name()), os.O_RDONLY, 0600); err != nil {
		log.Warning("Failed to open file", "name", files[i].Name(), "err", err)
		return
	}
	defer opened.Close() // Close the lastly opened file.
	fi, err := opened.Stat()
	if err != nil {
		log.Warning("Problem with file", "name", opened.Name(), "err", err)
		return
	}
	db.lock.Lock()
	...
	if err := notify.Watch(db.logdir, watcher, notify.Create); err != nil {
		log.Warning("Failed to create file system watcher", "err", err)
		return
	}
...
loop:
	for err == nil || errc == nil {
		...
		case event := <-watcher:
			...
			if opened == nil {
				log.Warning("The last log file is not opened")
				break loop
			}
			...
			if err != nil {
				log.Warning("Failed to read file", "name", opened.Name(), "err", err)
				break loop
			}
			...
			if opened, err = os.OpenFile(event.Path(), os.O_RDONLY, 0644); err != nil {
				log.Warning("Failed to open file", "name", event.Path(), "err", err)
				break loop
			}
			buf = buf[:0]
