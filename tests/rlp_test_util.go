...
func translateJSON(v interface{}) interface{} {
	...
	case string:
		if len(v) > 0 && v[0] == '#' { // # starts a faux big int.
			bigObj, ok := new(big.Int).SetString(v[1:], 10)
			if !ok {
		...
		return []byte(v)
	case []interface{}:
		newObj := make([]interface{}, len(v))
		for i := range v {
			newObj[i] = translateJSON(v[i])
		}
		return newObj
	default:
...
func checkDecodeFromJSON(s *rlp.Stream, exp interface{}) error {
	...
	case *big.Int:
		bigObj := new(big.Int)
		if err := s.Decode(&bigObj); err != nil {
			return addStack("Big", exp, err)
		}
		if bigObj.Cmp(exp) != 0 {
			return addStack("Big", exp, fmt.Errorf("result mismatch: got %d", bigObj))
		}
	case []byte:
		b, err := s.Bytes()
		if err != nil {
