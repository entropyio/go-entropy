...
func withTrace(t *testing.T, gasLimit uint64, test func(evm.Config) error) {
	...
	tracer := tracelogger.NewJSONLogger(&tracelogger.Config{}, w)
...
func BenchmarkEVM(b *testing.B) {
	...
	if os.IsNotExist(err) || !dirinfo.IsDir() {
		_, _ = fmt.Fprintf(os.Stderr, "can't find test files in %s, did you clone the evm-benchmarks submodule?\n", dir)
...
func runBenchmark(b *testing.B, t *StateTest) {
	for _, subtest := range t.Subtests() {
		...
		b.Run(key, func(b *testing.B) {
			...
			sender := evm.NewContract(evm.AccountRef(msg.From()), evm.AccountRef(msg.From()),
				nil, 0)
			...
			for n := 0; n < b.N; n++ {
				...
				_, _, err = evmObj.Call(sender, *msg.To(), msg.Data(), msg.Gas(), msg.Value())
				if err != nil {
					b.Error(err)
					return
				}
