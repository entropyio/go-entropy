...
type btHeaderMarshaling struct {
	...
	BaseFeePerGas *mathutil.HexOrDecimal256
}

func (t *BlockTest) Run(snapshotter bool) error {
	configObj, ok := Forks[t.json.Network]
	if !ok {
	...
	gblock, err := t.genesis(configObj).Commit(db)
	if err != nil {
		return err
	}
	...
	cache := &blockchain.CacheConfig{TrieCleanLimit: 0}
	if snapshotter {
		cache.SnapshotLimit = 1
		cache.SnapshotWait = true
	}
	chain, err := blockchain.NewBlockChain(db, cache, configObj, engine, evm.Config{}, nil, nil)
	if err != nil {
		return err
	}
	defer chain.Stop()
...
func (t *BlockTest) genesis(config *config.ChainConfig) *blockchain.Genesis {
...
/* See https://github.com/entropy/tests/wiki/Blockchain-Tests-II
   ...
   post state.
*/
func (t *BlockTest) insertBlocks(bc *blockchain.BlockChain) ([]btBlock, error) {
	...
	for bi, b := range t.json.Blocks {
		...
		if b.BlockHeader == nil {
			if data, err := json.MarshalIndent(cb.Header(), "", "  "); err == nil {
				_, _ = fmt.Fprintf(os.Stderr, "block (index %d) insertion should have failed due to: %v:\n%v\n",
...
func validateHeader(h *btHeader, h2 *model.Header) error {
	if h.Bloom != h2.Bloom {
...
func (bb *btBlock) decode() (*model.Block, error) {
