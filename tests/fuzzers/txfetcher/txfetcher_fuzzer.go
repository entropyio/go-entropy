...
func init() {
	...
	txs = make([]*model.Transaction, 65536) // We need to bump enough to hit all the limits
	for i := 0; i < len(txs); i++ {
		txs[i] = model.NewTransaction(rand.Uint64(), common.Address{byte(rand.Intn(256))}, new(big.Int), 0, new(big.Int), nil)
	}
}
	...
	for {
		...
		case 1:
			...
			var (
				...
				deliveries  = make([]*model.Transaction, deliver)
			)
