...
func (tm *testMatcher) config(pattern string, cfg config.ChainConfig) {
...
func (tm *testMatcher) findConfig(t *testing.T) *config.ChainConfig {
...
func (tm *testMatcher) walk(t *testing.T, dir string, runTest interface{}) {
	...
	if os.IsNotExist(err) || !dirinfo.IsDir() {
		_, _ = fmt.Fprintf(os.Stderr, "can't find test files in %s, did you clone the tests submodule?\n", dir)
