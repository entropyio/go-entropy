...
import (
	...
	"github.com/entropyio/go-entropy/accounts/usbwallet"
...
var log = logger.NewLogger("[signer]")
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*entropyapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx entropyapi.SignTransactionResult)
...
func StartClefAccountManager(ksLocation string, nousb, lightKDF bool, scpath string) *accounts.Manager {
	...
	if !nousb {
		...
		if ledgerhub, err := usbwallet.NewLedgerHub(); err != nil {
			log.Warning(fmt.Sprintf("Failed to start Ledger hub, disabling: %v", err))
		} else {
		...
		if trezorhub, err := usbwallet.NewTrezorHubWithHID(); err != nil {
			log.Warning(fmt.Sprintf("Failed to start HID Trezor hub, disabling: %v", err))
		} else {
		...
		if trezorhub, err := usbwallet.NewTrezorHubWithWebUSB(); err != nil {
			log.Warning(fmt.Sprintf("Failed to start WebUSB Trezor hub, disabling: %v", err))
		} else {
			backends = append(backends, trezorhub)
		...
		if err != nil {
			...
			if fi.Mode()&os.ModeType != os.ModeSocket {
				...
				if schub, err := scwallet.NewHub(scpath, scwallet.Scheme, ksLocation); err != nil {
					log.Warning(fmt.Sprintf("Failed to start smart card hub, disabling: %v", err))
				} else {
...
func (api *SignerAPI) openTrezor(url accounts.URL) {
	...
	if err != nil {
		log.Warning("failed getting trezor pin", "err", err)
		return
	}
	...
	if err != nil {
		log.Warning("wallet unavailable", "url", url)
		return
	}
	err = w.Open(resp.Text)
	if err != nil {
		log.Warning("failed to open wallet", "wallet", url, "err", err)
		return
	}
			...
			var derive = func(limit int, next func() accounts.DerivationPath) {
				...
				for i := 0; i < limit; i++ {
					...
					if acc, err := event.Wallet.Derive(path, true); err != nil {
						log.Warning("Account derivation failed", "error", err)
					} else {
...
func (api *SignerAPI) newAccount() (common.Address, error) {
	...
	for i := 0; i < 3; i++ {
		resp, err := api.UI.OnInputRequired(UserInputRequest{
			...
			true})
		if err != nil {
			log.Warning("error obtaining password", "attempt", i, "error", err)
			continue
		}
...
func (api *SignerAPI) lookupOrQueryPassword(address common.Address, title, prompt string) (string, error) {
	...
	if err != nil {
		log.Warning("error obtaining password", "error", err)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*entropyapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := entropyapi.SignTransactionResult{Raw: data, Tx: signedTx}
