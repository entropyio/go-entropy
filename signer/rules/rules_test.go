...
func (alwaysDenyUI) OnApprovedTx(tx entropyapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx entropyapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(entropyapi.SignTransactionResult{})
...
func dummySigned(value *big.Int) *model.Transaction {
	to := common.HexToAddress("000000000000000000000000000000000000dead")
	gas := uint64(21000)
	gasPrice := big.NewInt(2000000)
	data := make([]byte, 0)
	return model.NewTransaction(3, to, value, gas, gasPrice, data)
}
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := entropyapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx entropyapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
