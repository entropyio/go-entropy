...
func NewState(ctx context.Context, head *model.Header, odr OdrBackend) *state.StateDB {
	state, _ := state.New(head.Root, NewStateDatabase(ctx, head, odr), nil)
	return state
}

func NewStateDatabase(ctx context.Context, head *model.Header, odr OdrBackend) state.StateDatabase {
...
func (db *odrDatabase) TrieDB() *trie.TrieDatabase {
	return nil
}
...
func (t *odrTrie) TryUpdateAccount(key []byte, acc *model.StateAccount) error {
	key = crypto.Keccak256(key)
