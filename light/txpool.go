...
type TxPool struct {
	...
	pending      map[common.Hash]*model.Transaction   // pending transactions by tx hash
	mined        map[common.Hash][]*model.Transaction // mined transactions by block hash
...
type TxRelayBackend interface {
	Send(txs model.Transactions)
	NewHead(head common.Hash, mined []common.Hash, rollback []common.Hash)
	Discard(hashes []common.Hash)
}

// NewTxPool creates a new light transaction pool
func NewTxPool(config *config.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		config:      config,
		signer:      model.LatestSigner(config),
		nonce:       make(map[common.Address]uint64),
		pending:     make(map[common.Hash]*model.Transaction),
		mined:       make(map[common.Hash][]*model.Transaction),
		quit:        make(chan bool),
...
func (pool *TxPool) GetNonce(ctx context.Context, addr common.Address) (uint64, error) {
	stateDB := pool.currentState(ctx)
	nonce := stateDB.GetNonce(addr)
	if stateDB.Error() != nil {
		return 0, stateDB.Error()
	}
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *model.Header) (txStateChanges, error) {
...
func (pool *TxPool) setNewHead(head *model.Header) {
	pool.mu.Lock()
...
// SubscribeNewTxsEvent registers a subscription of blockchain.NewTxsEvent and
// starts sending event to the given channel.
func (pool *TxPool) SubscribeNewTxsEvent(ch chan<- blockchain.NewTxsEvent) event.Subscription {
...
func (pool *TxPool) validateTx(ctx context.Context, tx *model.Transaction) error {
	// Validate sender
	var (
		from common.Address
		err  error
	...
	if from, err = model.Sender(pool.signer, tx); err != nil {
		return blockchain.ErrInvalidSender
	}
	...
	if tx.Value().Sign() < 0 {
		return blockchain.ErrNegativeValue
	}

	// Transactor should have enough funds to cover the costs
	// cost == V + GP * GL
	if b := currentState.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
		return blockchain.ErrInsufficientFunds
	}

	// Should supply enough intrinsic gas
	gas, err := blockchain.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil, true)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
		return blockchain.ErrIntrinsicGas
	}
...
func (pool *TxPool) add(ctx context.Context, tx *model.Transaction) error {
	hash := tx.Hash()
	...
	if _, ok := pool.pending[hash]; !ok {
		...
		go pool.txFeed.Send(blockchain.NewTxsEvent{Txs: model.Transactions{tx}})
	}

	// Print a log message if low enough level is set
	log.Debug("Pooled new transaction", "hash", hash, "from", "", "to", tx.To())
	return nil
}
...
func (pool *TxPool) Add(ctx context.Context, tx *model.Transaction) error {
	...
	pool.relay.Send(model.Transactions{tx})

	_ = pool.chainDb.Put(tx.Hash().Bytes(), data, "transaction")
	return nil
}

// AddTransactions adds all valid transactions to the pool and passes them to
// the tx relay backend
func (pool *TxPool) AddBatch(ctx context.Context, txs []*model.Transaction) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	var sendTx model.Transactions
...
// and nil otherwise.
func (pool *TxPool) GetTransaction(hash common.Hash) *model.Transaction {
	// check the txs first
...
func (pool *TxPool) GetTransactions() (txs model.Transactions, err error) {
	pool.mu.RLock()
	defer pool.mu.RUnlock()

	txs = make(model.Transactions, len(pool.pending))
	i := 0
...
func (pool *TxPool) Content() (map[common.Address]model.Transactions, map[common.Address]model.Transactions) {
	...
	for _, tx := range pool.pending {
		account, _ := model.Sender(pool.signer, tx)
...
func (pool *TxPool) ContentFrom(addr common.Address) (model.Transactions, model.Transactions) {
	...
	// There are no queued transactions in a light pool, just return an empty map
	return pending, model.Transactions{}
}

// RemoveTransactions removes all given transactions from the pool.
func (pool *TxPool) RemoveTransactions(txs model.Transactions) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	...
	for _, tx := range txs {
		...
		_ = batch.Delete(hash.Bytes(), "transaction")
...
func (pool *TxPool) RemoveTx(hash common.Hash) {
	...
	delete(pool.pending, hash)
	_ = pool.chainDb.Delete(hash[:], "transaction")
