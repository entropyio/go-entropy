...
var (
	...
	DefaultServerIndexerConfig = &IndexerConfig{
		ChtSize:           config.CHTFrequency,
		ChtConfirms:       config.HelperTrieProcessConfirmations,
		BloomSize:         config.BloomBitsBlocks,
		BloomConfirms:     config.BloomConfirms,
		BloomTrieSize:     config.BloomTrieFrequency,
		BloomTrieConfirms: config.HelperTrieProcessConfirmations,
	}
	// DefaultClientIndexerConfig wraps a set of configs as a default indexer config for client side.
	DefaultClientIndexerConfig = &IndexerConfig{
		ChtSize:           config.CHTFrequency,
		ChtConfirms:       config.HelperTrieConfirmations,
		BloomSize:         config.BloomBitsBlocksClient,
		BloomConfirms:     config.HelperTrieConfirmations,
		BloomTrieSize:     config.BloomTrieFrequency,
		BloomTrieConfirms: config.HelperTrieConfirmations,
	}
...
func GetChtRoot(db database.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	...
	data, _ := db.Get(append(append(chtPrefix, encNumber[:]...), sectionHead.Bytes()...), "chtRoot")
...
func StoreChtRoot(db database.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
	_ = db.Put(append(append(chtPrefix, encNumber[:]...), sectionHead.Bytes()...), root.Bytes(), "chtRoot")
}

// ChtIndexerBackend implements blockchain.ChainIndexerBackend.
...
func NewChtIndexer(db database.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *blockchain.ChainIndexer {
	...
	return blockchain.NewChainIndexer(db, rawdb.NewTable(db, "chtIndexV2-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
// Reset implements blockchain.ChainIndexerBackend
...
// Process implements blockchain.ChainIndexerBackend
func (c *ChtIndexerBackend) Process(ctx context.Context, header *model.Header) error {
...
// Commit implements blockchain.ChainIndexerBackend
	...
	if !c.disablePruning {
		...
		for it.Next() {
			...
			if !c.trieset.Contains(common.BytesToHash(trimmed)) {
				_ = c.trieTable.Delete(trimmed, "chtIndex")
				deleted += 1
			} else {
				remaining += 1
			}
...
// Prune implements blockchain.ChainIndexerBackend which deletes all chain data
...
func GetBloomTrieRoot(db database.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
	data, _ := db.Get(append(append(bloomTriePrefix, encNumber[:]...), sectionHead.Bytes()...), "bloomTrieRoot")
	return common.BytesToHash(data)
}
...
func StoreBloomTrieRoot(db database.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
	_ = db.Put(append(append(bloomTriePrefix, encNumber[:]...), sectionHead.Bytes()...), root.Bytes(), "bloomTrieRoot")
}

// BloomTrieIndexerBackend implements blockchain.ChainIndexerBackend
...
func NewBloomTrieIndexer(db database.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *blockchain.ChainIndexer {
	...
	return blockchain.NewChainIndexer(db, rawdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}

// fetchMissingNodes tries to retrieve the last entries of the latest trusted bloom trie from the
// ODR backend in order to be able to add new entries and calculate subsequent root hashes
func (b *BloomTrieIndexerBackend) fetchMissingNodes(ctx context.Context, section uint64, root common.Hash) error {
	indexCh := make(chan uint, model.BloomBitLength)
	type res struct {
		nodes *NodeSet
		err   error
	}
	resCh := make(chan res, model.BloomBitLength)
	for i := 0; i < 20; i++ {
		go func() {
	...
	for i := uint(0); i < model.BloomBitLength; i++ {
		indexCh <- i
	}
	close(indexCh)
...
// Process implements blockchain.ChainIndexerBackend
func (b *BloomTrieIndexerBackend) Process(ctx context.Context, header *model.Header) error {
	...
	if !b.disablePruning {
		...
		for it.Next() {
			...
			if !b.trieset.Contains(common.BytesToHash(trimmed)) {
				b.trieTable.Delete(trimmed, "bloomTrieIndexer")
				deleted += 1
			} else {
				remaining += 1
			}
		...
		_ = b.triedb.Commit(root, false, nil)
	}
...
// Prune implements blockchain.ChainIndexerBackend which deletes all
