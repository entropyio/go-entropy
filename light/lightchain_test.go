...
func makeHeaderChain(parent *model.Header, n int, db database.Database, seed int) []*model.Header {
	blocks, _ := blockchain.GenerateChain(config.TestChainConfig, model.NewBlockWithHeader(parent), ethash.NewFaker(), db, n, func(i int, b *blockchain.BlockGen) {
...
func newCanonical(n int) (database.Database, *LightChain, error) {
	...
	gspec := blockchain.Genesis{Config: config.TestChainConfig}
...
func testHeaderChainImport(chain []*model.Header, lightchain *LightChain) error {
...
func makeHeaderChainWithDiff(genesis *model.Block, d []int, seed byte) []*model.Header {
	...
	for i, difficulty := range d {
		...
		chain = append(chain, model.CopyHeader(header))
	}
	return chain
}
...
func TestBadHeaderHashes(t *testing.T) {
	...
	blockchain.BadHashes[headers[2].Hash()] = true
	if _, err = bc.InsertHeaderChain(headers, 1); !errors.Is(err, blockchain.ErrBannedHash) {
		t.Errorf("error mismatch: have: %v, want %v", err, blockchain.ErrBannedHash)
	}
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	blockchain.BadHashes[headers[3].Hash()] = true
	defer func() { delete(blockchain.BadHashes, headers[3].Hash()) }()

	// Create a new LightChain and check that it rolled back the state.
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, config.TestChainConfig, ethash.NewFaker(), nil)
	if err != nil {
