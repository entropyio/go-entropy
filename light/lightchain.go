...
var log = logger.NewLogger("[lightChain]")
...
// validator.
func NewLightChain(odr OdrBackend, config *config.ChainConfig, engine consensus.Engine, checkpoint *config.TrustedCheckpoint) (*LightChain, error) {
	...
	bc.forker = blockchain.NewForkChoice(bc, nil)
	var err error
	bc.hc, err = blockchain.NewHeaderChain(odr.Database(), config, bc.engine, bc.getProcInterrupt)
	if err != nil {
		return nil, err
	}
	bc.genesisBlock, _ = bc.GetBlockByNumber(NoOdr, 0)
	if bc.genesisBlock == nil {
		return nil, blockchain.ErrNoGenesis
	}
	if checkpoint != nil {
	...
	for hash := range blockchain.BadHashes {
...
func (lc *LightChain) AddTrustedCheckpoint(cp *config.TrustedCheckpoint) {
...
func (lc *LightChain) HeaderChain() *blockchain.HeaderChain {
	return lc.hc
}
...
func (lc *LightChain) ResetWithGenesisBlock(genesis *model.Block) {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to reset genesis block", "err", err)
	}
...
func (lc *LightChain) Genesis() *model.Block {
	return lc.genesisBlock
}

func (lc *LightChain) StateCache() state.StateDatabase {
	panic("not implemented")
}

// GetBody retrieves a block body (transactions and uncles) from the database
// or ODR service by hash, caching it if found.
func (lc *LightChain) GetBody(ctx context.Context, hash common.Hash) (*model.Body, error) {
...
// caching it if found.
func (lc *LightChain) GetBlock(ctx context.Context, hash common.Hash, number uint64) (*model.Block, error) {
	// Short circuit if the block's already in the cache, retrieve otherwise
	if block, ok := lc.blockCache.Get(hash); ok {
		return block.(*model.Block), nil
	}
...
// caching it if found.
func (lc *LightChain) GetBlockByHash(ctx context.Context, hash common.Hash) (*model.Block, error) {
	number := lc.hc.GetBlockNumber(hash)
	if number == nil {
		return nil, errors.New("unknown block")
	}
...
func (lc *LightChain) GetBlockByNumber(ctx context.Context, number uint64) (*model.Block, error) {
...
func (lc *LightChain) Rollback(chain []common.Hash) {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to rollback light chain", "error", err)
	}
}

func (lc *LightChain) InsertHeader(header *model.Header) error {
	// Verify the header first before obtaining the lock
	headers := []*model.Header{header}
...
func (lc *LightChain) SetCanonical(header *model.Header) error {
	lc.chainmu.Lock()
	defer lc.chainmu.Unlock()
	...
	if err := lc.hc.Reorg([]*model.Header{header}); err != nil {
		return err
	}
	// Emit events
	block := model.NewBlockWithHeader(header)
	lc.chainFeed.Send(blockchain.ChainEvent{Block: block, Hash: block.Hash()})
	lc.chainHeadFeed.Send(blockchain.ChainHeadEvent{Block: block})
...
func (lc *LightChain) InsertHeaderChain(chain []*model.Header, checkFreq int) (int, error) {
	if len(chain) == 0 {
		return 0, nil
	}
	...
	var (
		...
		block      = model.NewBlockWithHeader(lastHeader)
	)
	switch status {
	...
	case blockchain.SideStatTy:
		lc.chainSideFeed.Send(blockchain.ChainSideEvent{Block: block})
	}
	return 0, err
}
...
func (lc *LightChain) CurrentHeader() *model.Header {
...
// caching it if found.
func (lc *LightChain) GetHeader(hash common.Hash, number uint64) *model.Header {
	return lc.hc.GetHeader(hash, number)
}

// GetHeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (lc *LightChain) GetHeaderByHash(hash common.Hash) *model.Header {
...
func (lc *LightChain) GetHeaderByNumber(number uint64) *model.Header {
	return lc.hc.GetHeaderByNumber(number)
}

// GetHeaderByNumberOdr retrieves a block header from the database or network
// by number, caching it (associated with its hash) if found.
func (lc *LightChain) GetHeaderByNumberOdr(ctx context.Context, number uint64) (*model.Header, error) {
...
func (lc *LightChain) Config() *config.ChainConfig { return lc.hc.Config() }
...
func (lc *LightChain) SyncCheckpoint(ctx context.Context, checkpoint *config.TrustedCheckpoint) bool {
...
func (lc *LightChain) SubscribeChainEvent(ch chan<- blockchain.ChainEvent) event.Subscription {
	return lc.scope.Track(lc.chainFeed.Subscribe(ch))
}

// SubscribeChainHeadEvent registers a subscription of ChainHeadEvent.
func (lc *LightChain) SubscribeChainHeadEvent(ch chan<- blockchain.ChainHeadEvent) event.Subscription {
	return lc.scope.Track(lc.chainHeadFeed.Subscribe(ch))
}

// SubscribeChainSideEvent registers a subscription of ChainSideEvent.
func (lc *LightChain) SubscribeChainSideEvent(ch chan<- blockchain.ChainSideEvent) event.Subscription {
	return lc.scope.Track(lc.chainSideFeed.Subscribe(ch))
}

// SubscribeLogsEvent implements the interface of filters.Backend
// LightChain does not send logs events, so return an empty subscription.
func (lc *LightChain) SubscribeLogsEvent(ch chan<- []*model.Log) event.Subscription {
	return lc.scope.Track(new(event.Feed).Subscribe(ch))
}

// SubscribeRemovedLogsEvent implements the interface of filters.Backend
// LightChain does not send blockchain.RemovedLogsEvent, so return an empty subscription.
func (lc *LightChain) SubscribeRemovedLogsEvent(ch chan<- blockchain.RemovedLogsEvent) event.Subscription {
	return lc.scope.Track(new(event.Feed).Subscribe(ch))
}
