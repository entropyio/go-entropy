...
func (oracle *Oracle) resolveBlockRange(ctx context.Context, lastBlock rpc.BlockNumber, blocks int) (*model.Block, []*model.Receipt, uint64, int, error) {
	var (
		headBlock       rpc.BlockNumber
		pendingBlock    *model.Block
		pendingReceipts model.Receipts
	)
...
func (oracle *Oracle) FeeHistory(ctx context.Context, blocks int, unresolvedLastBlock rpc.BlockNumber, rewardPercentiles []float64) (*big.Int, [][]*big.Int, []*big.Int, []float64, error) {
	...
	if blocks > maxFeeHistory {
		log.Warning("Sanitizing fee history length", "requested", blocks, "truncated", maxFeeHistory)
