...
func serviceNonContiguousBlockHeaderQuery(chain *blockchain.BlockChain, query *GetBlockHeadersPacket, peer *Peer) []rlp.RawValue {
	...
	for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit &&
		...
		if rlpData, err := rlp.EncodeToBytes(origin); err != nil {
			log.Critical("Unable to decode our own headers", "err", err)
		} else {
		...
		case hashMode && !query.Reverse:
			...
			if next <= current {
				...
				peer.Log().Warning("GetBlockHeaders skip overflow attack", "current", current, "skip", query.Skip, "next", next, "attacker", infos)
				unknown = true
			} else {
...
func ServiceGetReceiptsQuery(chain *blockchain.BlockChain, query GetReceiptsPacket) []rlp.RawValue {
	...
	for lookups, hash := range query {
		...
		if results == nil {
			if header := chain.GetHeaderByHash(hash); header == nil || header.ReceiptHash != model.EmptyRootHash {
				continue
			}
		}
...
func handleNewBlock(backend Backend, msg Decoder, peer *Peer) error {
	...
	if hash := model.CalcUncleHash(ann.Block.Uncles()); hash != ann.Block.UncleHash() {
		log.Warning("Propagated block has invalid uncles", "have", hash, "exp", ann.Block.UncleHash())
		return nil // TODO(karalabe): return error eventually, but wait a few releases
	}
	if hash := model.DeriveSha(ann.Block.Transactions(), trie.NewStackTrie(nil)); hash != ann.Block.TxHash() {
		log.Warning("Propagated block has invalid body", "have", hash, "exp", ann.Block.TxHash())
		return nil // TODO(karalabe): return error eventually, but wait a few releases
	}
