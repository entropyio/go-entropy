...
// testBackend is a mock implementation of the live Entropy message handler. Its
...
func newTestBackendWithGenerator(blocks int, generator func(int, *blockchain.BlockGen)) *testBackend {
	...
	bs, _ := blockchain.GenerateChain(config.TestChainConfig, chain.Genesis(), ethash.NewFaker(), db, blocks, generator)
	if _, err := chain.InsertChain(bs); err != nil {
		panic(err)
	}
	txconfig := blockchain.DefaultTxPoolConfig
	txconfig.Journal = "" // Don't litter the disk with test journals

	return &testBackend{
		db:     db,
		chain:  chain,
		txpool: blockchain.NewTxPool(txconfig, config.TestChainConfig, chain),
	}
}
...
func (b *testBackend) Chain() *blockchain.BlockChain { return b.chain }
...
func testGetNodeData(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *blockchain.BlockGen) {
		...
		case 0:
			...
			tx, _ := model.SignTx(model.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(10_000_000_000_000_000), config.TxGas, block.BaseFee(), nil), signer, testKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more ether to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := model.SignTx(model.NewTransaction(block.TxNonce(testAddr), acc1Addr, big.NewInt(1_000_000_000_000_000), config.TxGas, block.BaseFee(), nil), signer, testKey)
			tx2, _ := model.SignTx(model.NewTransaction(block.TxNonce(acc1Addr), acc2Addr, big.NewInt(1_000_000_000_000_000), config.TxGas, block.BaseFee(), nil), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
