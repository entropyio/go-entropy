...
var log = logger.NewLogger("[eth_protocol]")
...
type TransactionsPacket []*model.Transaction
...
type BlockHeadersPacket []*model.Header
...
func (p *BlockBodiesPacket) Unpack() ([][]*model.Transaction, [][]*model.Header) {
	var (
		txset    = make([][]*model.Transaction, len(*p))
		uncleset = make([][]*model.Header, len(*p))
	)
	for i, body := range *p {
...
type ReceiptsPacket [][]*model.Receipt
...
type PooledTransactionsPacket []*model.Transaction
