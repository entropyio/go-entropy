...
func ServiceGetAccountRangeQuery(chain *blockchain.BlockChain, req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	...
	if err := tr.Prove(req.Origin[:], 0, proof); err != nil {
		log.Warning("Failed to prove account range", "origin", req.Origin, "err", err)
		return nil, nil
	}
	if last != (common.Hash{}) {
		if err := tr.Prove(last[:], 0, proof); err != nil {
			log.Warning("Failed to prove account range", "last", last, "err", err)
			return nil, nil
		}
	}
	var proofs [][]byte
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			if err := stTrie.Prove(origin[:], 0, proof); err != nil {
				log.Warning("Failed to prove storage range", "origin", req.Origin, "err", err)
				return nil, nil
			}
			if last != (common.Hash{}) {
				if err := stTrie.Prove(last[:], 0, proof); err != nil {
					log.Warning("Failed to prove storage range", "last", last, "err", err)
					return nil, nil
				}
			}
			for _, blob := range proof.NodeList() {
