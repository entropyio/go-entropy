...
import (
	...
	"github.com/entropyio/go-entropy/light"
	"github.com/entropyio/go-entropy/server/p2p/msgrate"
...
type accountResponse struct {
	...
	accounts []*model.StateAccount // Expanded accounts in the returned range
...
type accountTask struct {
	...
	genBatch database.Batch  // Batch used by the node generator
...
// Syncer is an Entropy account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db database.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        database.Batch     // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the Entropy state over the
// snap protocol.
func NewSyncer(db database.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = database.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = database.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) revertAccountRequest(req *accountRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Account request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertBytecodeRequest(req *bytecodeRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Bytecode request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertStorageRequest(req *storageRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Storage request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertTrienodeHealRequest(req *trienodeHealRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Trienode heal request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) revertBytecodeHealRequest(req *bytecodeHealRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Bytecode heal request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	for i, account := range res.accounts {
		...
		if account.Root != emptyRoot {
			if ok, err := s.db.Has(account.Root[:], "account"); err != nil || !ok {
...
func (s *Syncer) processBytecodeResponse(res *bytecodeResponse) {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to persist bytecodes", "err", err)
	}
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > database.IdealBatchSize || res.subTask.done {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to persist storage slots", "err", err)
	}
...
func (s *Syncer) processTrienodeHealResponse(res *trienodeHealResponse) {
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to persist healing data", "err", err)
	}
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	// Flush anything written just now and update the stats
	if err := batch.Write(); err != nil {
		log.Critical("Failed to persist accounts", "err", err)
	}
	...
	if task.genBatch.ValueSize() > database.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	if !ok {
		...
		logger.Warning("Unexpected account range packet")
		s.lock.Unlock()
		return nil
	}
	...
	if err != nil {
		logger.Warning("Account range failed proof", "err", err)
		// Signal this request as failed, and ready for rescheduling
		s.scheduleRevertAccountRequest(req)
		return err
	}
	accs := make([]*model.StateAccount, len(accounts))
	for i, account := range accounts {
		acc := new(model.StateAccount)
...
func (s *Syncer) onByteCodes(peer SyncPeer, id uint64, bytecodes [][]byte) error {
	...
	logger := peer.Log()
	logger.Debug("Delivering set of bytecodes", "bytecodes", len(bytecodes), "bytes", size)
	...
	if !ok {
		// Request stale, perhaps the peer timed out but came through in the end
		logger.Warning("Unexpected bytecode packet")
		s.lock.Unlock()
		return nil
	}
	...
	for i, j := 0, 0; i < len(bytecodes); i++ {
		...
		logger.Warning("Unexpected bytecodes", "count", len(bytecodes)-i)
		// Signal this request as failed, and ready for rescheduling
		s.scheduleRevertBytecodeRequest(req)
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash, slots [][][]byte, proof [][]byte) error {
	...
	if !ok {
		// Request stale, perhaps the peer timed out but came through in the end
		logger.Warning("Unexpected storage ranges packet")
		s.lock.Unlock()
		return nil
	}
	...
	if len(hashes) != len(slots) {
		...
		logger.Warning("Hash and slot set size mismatch", "hashset", len(hashes), "slotset", len(slots))
		return errors.New("hash and slot set size mismatch")
	}
	if len(hashes) > len(req.accounts) {
		s.lock.Unlock()
		s.scheduleRevertStorageRequest(req) // reschedule request
		logger.Warning("Hash set larger than requested", "hashset", len(hashes), "requested", len(req.accounts))
	...
	for i := 0; i < len(hashes); i++ {
		...
		if len(nodes) == 0 {
			...
			if err != nil {
				s.scheduleRevertStorageRequest(req) // reschedule request
				logger.Warning("Storage slots failed proof", "err", err)
				return err
			}
		} else {
			...
			if err != nil {
				s.scheduleRevertStorageRequest(req) // reschedule request
				logger.Warning("Storage range failed proof", "err", err)
				return err
			}
		}
	}
...
func (s *Syncer) OnTrieNodes(peer SyncPeer, id uint64, trienodes [][]byte) error {
	...
	logger := peer.Log()
	logger.Debug("Delivering set of healing trienodes", "trienodes", len(trienodes), "bytes", size)
	...
	if !ok {
		// Request stale, perhaps the peer timed out but came through in the end
		logger.Warning("Unexpected trienode heal packet")
		s.lock.Unlock()
		return nil
	}
	...
	for i, j := 0, 0; i < len(trienodes); i++ {
		...
		// We've either ran out of hashes, or got unrequested data
		logger.Warning("Unexpected healing trienodes", "count", len(trienodes)-i)
		// Signal this request as failed, and ready for rescheduling
		s.scheduleRevertTrienodeHealRequest(req)
...
func (s *Syncer) onHealByteCodes(peer SyncPeer, id uint64, bytecodes [][]byte) error {
	...
	logger := peer.Log()
	logger.Debug("Delivering set of healing bytecodes", "bytecodes", len(bytecodes), "bytes", size)
	...
	if !ok {
		// Request stale, perhaps the peer timed out but came through in the end
		logger.Warning("Unexpected bytecode heal packet")
		s.lock.Unlock()
		return nil
	}
	...
	for i, j := 0, 0; i < len(bytecodes); i++ {
		...
		// We've either ran out of hashes, or got unrequested data
		logger.Warning("Unexpected healing bytecodes", "count", len(bytecodes)-i)
		// Signal this request as failed, and ready for rescheduling
		s.scheduleRevertBytecodeHealRequest(req)
...
func estimateRemainingSlots(hashes int, last common.Hash) (uint64, error) {
	...
	space := new(big.Int).Mul(mathutil.MaxBig256, big.NewInt(int64(hashes)))
