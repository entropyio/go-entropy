...
func (t *testPeer) RequestAccountRange(id uint64, root, origin, limit common.Hash, bytes uint64) error {
	t.logger.Debug("Fetching range of accounts", "reqid", id, "root", root, "origin", origin, "limit", limit, "bytes", common.StorageSize(bytes))
	t.nAccountRequests++
...
func (t *testPeer) RequestTrieNodes(id uint64, root common.Hash, paths []TrieNodePathSet, bytes uint64) error {
	t.logger.Debug("Fetching set of trie nodes", "reqid", id, "root", root, "pathsets", len(paths), "bytes", common.StorageSize(bytes))
	t.nTrienodeRequests++
...
func (t *testPeer) RequestStorageRanges(id uint64, root common.Hash, accounts []common.Hash, origin, limit []byte, bytes uint64) error {
	...
	if len(accounts) == 1 && origin != nil {
		t.logger.Debug("Fetching range of large storage slots", "reqid", id, "root", root, "account", accounts[0], "origin", common.BytesToHash(origin), "limit", common.BytesToHash(limit), "bytes", common.StorageSize(bytes))
	} else {
		t.logger.Debug("Fetching ranges of small storage slots", "reqid", id, "root", root, "accounts", len(accounts), "first", accounts[0], "bytes", common.StorageSize(bytes))
	}
...
func (t *testPeer) RequestByteCodes(id uint64, hashes []common.Hash, bytes uint64) error {
	t.nBytecodeRequests++
	t.logger.Debug("Fetching set of byte codes", "reqid", id, "hashes", len(hashes), "bytes", common.StorageSize(bytes))
...
func makeAccountTrieNoStorage(n int) (*trie.Trie, entrySlice) {
	...
	for i := uint64(1); i <= uint64(n); i++ {
		value, _ := rlp.EncodeToBytes(&model.StateAccount{
			Nonce:    i,
...
func makeStorageTrieWithSeed(owner common.Hash, n, seed uint64, db *trie.TrieDatabase) (*trie.Trie, entrySlice) {
...
// boundary hash.
func makeBoundaryStorageTrie(owner common.Hash, n int, db *trie.TrieDatabase) (*trie.Trie, entrySlice) {
	var (
		entries    entrySlice
		boundaries []common.Hash
		trie, _    = trie.New(owner, common.Hash{}, db)
...
func verifyTrie(db database.KeyValueStore, root common.Hash, t *testing.T) {
	t.Helper()
	...
	for accIt.Next() {
		...
		if err := rlp.DecodeBytes(accIt.Value, &acc); err != nil {
			log.Critical("Invalid account encountered during snapshot creation", "err", err)
		}
		accounts++
