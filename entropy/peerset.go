...
type peerSet struct {
	peers     map[string]*entropyPeer // Peers connected on the `eth` protocol
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*entropyPeer),
...
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	...
	if !peer.RunningCap(ent.ProtocolName, ent.ProtocolVersions) {
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *ent.Peer) (*snap.Peer, error) {
...
func (ps *peerSet) registerPeer(peer *ent.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *entropyPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*entropyPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*entropyPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*entropyPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
func (ps *peerSet) peerWithHighestTD() *ent.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
