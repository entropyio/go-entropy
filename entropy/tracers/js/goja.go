...
import (
	...
	jsassets "github.com/entropyio/go-entropy/entropy/tracers/js/internal/tracers"
...
// init retrieves the JavaScript transaction tracers included in go-entropy.
func init() {
	var err error
...
func toBuf(evm *goja.Runtime, bufType goja.Value, val []byte) (goja.Value, error) {
	// bufType is usually Uint8Array. This is equivalent to `new Uint8Array(val)` in JS.
	return evm.New(bufType, evm.ToValue(evm.NewArrayBuffer(val)))
}
