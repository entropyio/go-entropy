...
import (
	...
	_ "github.com/entropyio/go-entropy/entropy/tracers/js"
	_ "github.com/entropyio/go-entropy/entropy/tracers/native"
)
...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			evmObj := evm.NewEVM(context, txContext, statedb, test.Genesis.Config, evm.Config{Debug: true, Tracer: tracer})
...
func TestZeroValueToNotExitCall(t *testing.T) {
	...
	tx, err := model.SignNewTx(privkey, signer, &model.LegacyTx{
	...
	var code = []byte{
		byte(evm.PUSH1), 0x0, byte(evm.DUP1), byte(evm.DUP1), byte(evm.DUP1), // in and outs zero
		byte(evm.DUP1), byte(evm.PUSH1), 0xff, byte(evm.GAS), // value=0,address=0xff, gas=GAS
		byte(evm.CALL),
	}
	var alloc = blockchain.GenesisAlloc{
		to: blockchain.GenesisAccount{
			Nonce: 1,
			Code:  code,
		},
		origin: blockchain.GenesisAccount{
			Nonce:   0,
	...
	evmObj := evm.NewEVM(context, txContext, statedb, config.MainnetChainConfig, evm.Config{Debug: true, Tracer: tracer})
	msg, err := tx.AsMessage(signer, nil)
	if err != nil {
		t.Fatalf("failed to prepare transaction for tracing: %v", err)
	}
