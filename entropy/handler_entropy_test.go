...
// testEntropyHandler is a mock event handler to listen for inbound network requests
// on the `eth` protocol and convert them into a more easily testable form.
type testEntropyHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testEntropyHandler) Chain() *blockchain.BlockChain        { panic("no backing chain") }
func (h *testEntropyHandler) TxPool() ent.TxPool                   { panic("no backing tx pool") }
func (h *testEntropyHandler) AcceptTxs() bool                      { return true }
func (h *testEntropyHandler) RunPeer(*ent.Peer, ent.Handler) error { panic("not used in tests") }
func (h *testEntropyHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEntropyHandler) Handle(peer *ent.Peer, packet ent.Packet) error {
	...
	case *ent.TransactionsPacket:
		h.txBroadcasts.Send(([]*model.Transaction)(*packet))
		return nil
...
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, ent.ETH66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		configNoFork  = &config.ChainConfig{HomesteadBlock: big.NewInt(1)}
		configProFork = &config.ChainConfig{
		...
		gspecNoFork  = &blockchain.Genesis{Config: configNoFork}
		gspecProFork = &blockchain.Genesis{Config: configProFork}

		genesisNoFork  = gspecNoFork.MustCommit(dbNoFork)
		genesisProFork = gspecProFork.MustCommit(dbProFork)

		chainNoFork, _  = blockchain.NewBlockChain(dbNoFork, nil, configNoFork, engine, evm.Config{}, nil, nil)
		chainProFork, _ = blockchain.NewBlockChain(dbProFork, nil, configProFork, engine, evm.Config{}, nil, nil)

		blocksNoFork, _  = blockchain.GenerateChain(configNoFork, genesisNoFork, engine, dbNoFork, 2, nil)
		blocksProFork, _ = blockchain.GenerateChain(configProFork, genesisProFork, engine, dbProFork, 2, nil)
	...
	peerNoFork := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- ethNoFork.runEntropyPeer(peerProFork, func(peer *ent.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ethProFork.runEntropyPeer(peerNoFork, func(peer *ent.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = ent.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = ent.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, ent.ETH66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	txs := make(chan blockchain.NewTxsEvent)
	...
	src := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runEntropyPeer(sink, func(peer *ent.Peer) error {
		return ent.Handle((*entropyHandler)(handler.handler), peer)
	})
	...
	tx := model.NewTransaction(0, common.Address{}, big.NewInt(0), 100000, big.NewInt(0), nil)
	tx, _ = model.SignTx(tx, model.HomesteadSigner{}, testKey)

	if err := src.SendTransactions([]*model.Transaction{tx}); err != nil {
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, ent.ETH66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	insert := make([]*model.Transaction, 100)
	for nonce := range insert {
		tx := model.NewTransaction(uint64(nonce), common.Address{}, big.NewInt(0), 100000, big.NewInt(0), make([]byte, 10240))
		tx, _ = model.SignTx(tx, model.HomesteadSigner{}, testKey)
	...
	go handler.handler.runEntropyPeer(src, func(peer *ent.Peer) error {
		return ent.Handle((*entropyHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, ent.ETH66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runEntropyPeer(sourcePeer, func(peer *ent.Peer) error {
			return ent.Handle((*entropyHandler)(source.handler), peer)
		})
		go sink.handler.runEntropyPeer(sinkPeer, func(peer *ent.Peer) error {
			return ent.Handle((*entropyHandler)(sink.handler), peer)
		})
	}
	// Subscribe to all the transaction pools
	txChs := make([]chan blockchain.NewTxsEvent, len(sinks))
	for i := 0; i < len(sinks); i++ {
		txChs[i] = make(chan blockchain.NewTxsEvent, 1024)

		sub := sinks[i].txpool.SubscribeNewTxsEvent(txChs[i])
		defer sub.Unsubscribe()
	}
	// Fill the source pool with transactions and wait for them at the sinks
	txs := make([]*model.Transaction, 1024)
	for nonce := range txs {
		tx := model.NewTransaction(uint64(nonce), common.Address{}, big.NewInt(0), 100000, big.NewInt(0), nil)
		tx, _ = model.SignTx(tx, model.HomesteadSigner{}, testKey)
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	if checkpoint {
		number := (uint64(rand.Intn(500))+1)*config.CHTFrequency - 1
	...
	local := ent.NewPeer(ent.ETH66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := ent.NewPeer(ent.ETH66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runEntropyPeer(local, func(peer *ent.Peer) error {
			return ent.Handle((*entropyHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(ent.GetBlockHeadersPacket66)
		...
		if !timeout {
			if empty {
				...
				responseRlp, _ := rlp.EncodeToBytes(&model.Header{Number: response.Number})
				if err := remote.ReplyBlockHeadersRLP(request.RequestId, []rlp.RawValue{responseRlp}); err != nil {
					t.Fatalf("failed to answer challenge: %v", err)
				}
			}
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testEntropyHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testEntropyHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := ent.NewPeer(ent.ETH66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := ent.NewPeer(ent.ETH66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
		...
		go ent.Handle(sink, sinkPeer)
	}
	// Subscribe to all the transaction pools
	blockChs := make([]chan *model.Block, len(sinks))
	for i := 0; i < len(sinks); i++ {
		blockChs[i] = make(chan *model.Block, 1)
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, ent.ETH66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := ent.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runEntropyPeer(src, func(peer *ent.Peer) error {
		return ent.Handle((*entropyHandler)(source.handler), peer)
	})
	...
	blocks := make(chan *model.Block, 1)
	...
	for _, header := range []*model.Header{malformedUncles, malformedTransactions, malformedEverything} {
		block := model.NewBlockWithHeader(header).WithBody(head.Transactions(), head.Uncles())
