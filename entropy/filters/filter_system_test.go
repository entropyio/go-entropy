...
func (b *testBackend) HeaderByNumber(ctx context.Context, blockNr rpc.BlockNumber) (*model.Header, error) {
	...
	if blockNr == rpc.LatestBlockNumber {
		hash = mapper.ReadHeadBlockHash(b.db)
		number := mapper.ReadHeaderNumber(b.db, hash)
		if number == nil {
			return nil, nil
		}
		num = *number
		...
		num = uint64(blockNr)
		hash = mapper.ReadCanonicalHash(b.db, num)
	}
	return mapper.ReadHeader(b.db, hash, num), nil
}
...
func (b *testBackend) HeaderByHash(ctx context.Context, hash common.Hash) (*model.Header, error) {
	...
	return mapper.ReadHeader(b.db, hash, *number), nil
}
			...
			case request := <-requests:
				...
				for i, section := range task.Sections {
					if rand.Int()%4 != 0 { // Handle occasional missing deliveries
						head := mapper.ReadCanonicalHash(b.db, (section+1)*config.BloomBitsBlocks-1)
						task.Bitsets[i], _ = mapper.ReadBloomBits(b.db, task.Bit, section, head)
					}
				}
				request <- task
			}
...
func TestLogFilter(t *testing.T) {
	...
	var (
		...
		// posted twice, once as evm.Logs and once as blockchain.PendingLogsEvent
		allLogs = []*model.Log{
			{Address: firstAddr},
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		allLogs = []blockchain.PendingLogsEvent{
			{Logs: []*model.Log{{Address: firstAddr, Topics: []common.Hash{}, BlockNumber: 0}}},
			{Logs: []*model.Log{{Address: firstAddr, Topics: []common.Hash{firstTopic}, BlockNumber: 1}}},
			{Logs: []*model.Log{{Address: secondAddr, Topics: []common.Hash{firstTopic}, BlockNumber: 2}}},
			{Logs: []*model.Log{{Address: thirdAddress, Topics: []common.Hash{secondTopic}, BlockNumber: 3}}},
			{Logs: []*model.Log{{Address: thirdAddress, Topics: []common.Hash{secondTopic}, BlockNumber: 4}}},
			{Logs: []*model.Log{
		...
		convertLogs = func(pl []blockchain.PendingLogsEvent) []*model.Log {
			var logs []*model.Log
			for _, l := range pl {
		...
		testCases = []struct {
			crit     entropy.FilterQuery
			expected []*model.Log
			c        chan []*model.Log
			sub      *Subscription
		}{
			// match all
			{entropy.FilterQuery{}, convertLogs(allLogs), nil, nil},
			// match none due to no matching addresses
			{entropy.FilterQuery{Addresses: []common.Address{{}, notUsedAddress}, Topics: [][]common.Hash{nil}}, []*model.Log{}, nil, nil},
			// match logs based on addresses, ignore topics
			{entropy.FilterQuery{Addresses: []common.Address{firstAddr}}, append(convertLogs(allLogs[:2]), allLogs[5].Logs[3]), nil, nil},
			// match none due to no matching topics (match with address)
			{entropy.FilterQuery{Addresses: []common.Address{secondAddr}, Topics: [][]common.Hash{{notUsedTopic}}}, []*model.Log{}, nil, nil},
			// match logs based on addresses and topics
			{entropy.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}}, append(convertLogs(allLogs[3:5]), allLogs[5].Logs[0]), nil, nil},
			// match logs based on multiple addresses and "or" topics
			{entropy.FilterQuery{Addresses: []common.Address{secondAddr, thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}}, append(convertLogs(allLogs[2:5]), allLogs[5].Logs[0]), nil, nil},
			// block numbers are ignored for filters created with New***Filter, these return all logs that match the given criteria when the state changes
			{entropy.FilterQuery{Addresses: []common.Address{firstAddr}, FromBlock: big.NewInt(2), ToBlock: big.NewInt(3)}, append(convertLogs(allLogs[:2]), allLogs[5].Logs[3]), nil, nil},
			// multiple pending logs, should match only 2 topics from the logs in block 5
			{entropy.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, fourthTopic}}}, []*model.Log{allLogs[5].Logs[0], allLogs[5].Logs[2]}, nil, nil},
		}
	)
