...
type TxFetcher struct {
	...
	addTxs   func([]*model.Transaction) []error // Insert a batch of transactions into local txpool
...
func NewTxFetcher(hasTx func(common.Hash) bool, addTxs func([]*model.Transaction) []error, fetchTxs func(string, []common.Hash) error) *TxFetcher {
...
func NewTxFetcherForTests(
	hasTx func(common.Hash) bool, addTxs func([]*model.Transaction) []error, fetchTxs func(string, []common.Hash) error,
...
func (f *TxFetcher) Enqueue(peer string, txs []*model.Transaction, direct bool) error {
	...
	for i, err := range errs {
		...
		// Track a few interesting failure model
		switch {
	...
	for {
		...
		case delivery := <-f.cleanup:
			...
			if delivery.direct {
				...
				if req == nil {
					log.Warning("Unexpected transaction delivery", "peer", delivery.origin)
					break
				}
