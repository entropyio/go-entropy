...
var (
	...
	genesis      = blockchain.GenesisBlockForTesting(testdb, testAddress, big.NewInt(1000000000000000))
	unknownBlock = model.NewBlock(&model.Header{GasLimit: config.GenesisGasLimit, BaseFee: big.NewInt(config.InitialBaseFee)}, nil, nil, nil, trie.NewStackTrie(nil))
)
...
// reassembly.
func makeChain(n int, seed byte, parent *model.Block) ([]common.Hash, map[common.Hash]*model.Block) {
	blocks, _ := blockchain.GenerateChain(config.TestChainConfig, parent, ethash.NewFaker(), testdb, n, func(i int, block *blockchain.BlockGen) {
		...
		if i > 0 && i%5 == 0 {
			block.AddUncle(&model.Header{ParentHash: block.PrevBlock(i - 2).Hash(), Number: big.NewInt(int64(i - 1))})
		}
	})
...
type fetcherTester struct {
	...
	headers map[common.Hash]*model.Header // Headers belonging to the tester
	blocks  map[common.Hash]*model.Block  // Blocks belonging to the tester
...
func newTester(light bool) *fetcherTester {
	tester := &fetcherTester{
		...
		headers: map[common.Hash]*model.Header{genesis.Hash(): genesis.Header()},
		blocks:  map[common.Hash]*model.Block{genesis.Hash(): genesis},
...
func (f *fetcherTester) getHeader(hash common.Hash) *model.Header {
	f.lock.RLock()
...
func (f *fetcherTester) getBlock(hash common.Hash) *model.Block {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *fetcherTester) verifyHeader(header *model.Header) error {
	return nil
}

// broadcastBlock is a nop placeholder for the block broadcasting.
func (f *fetcherTester) broadcastBlock(block *model.Block, propagate bool) {
}
...
func (f *fetcherTester) insertHeaders(headers []*model.Header) (int, error) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *fetcherTester) insertChain(blocks model.Blocks) (int, error) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*model.Block, drift time.Duration) headerRequesterFn {
	closure := make(map[common.Hash]*model.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that return a header from the closure
	return func(hash common.Hash, sink chan *ent.Response) (*ent.Request, error) {
		// Gather the blocks to return
		headers := make([]*model.Header, 0, 1)
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*model.Block, drift time.Duration) bodyRequesterFn {
	closure := make(map[common.Hash]*model.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that returns blocks from the closure
	return func(hashes []common.Hash, sink chan *ent.Response) (*ent.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*ent.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &ent.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
...
func testSequentialAnnouncements(t *testing.T, light bool) {
	...
	tester.fetcher.importedHook = func(header *model.Header, block *model.Block) {
		if light {
			if header == nil {
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash common.Hash, sink chan *ent.Response) (*ent.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *ent.Response) (*ent.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash common.Hash, sink chan *ent.Response) (*ent.Request, error) {
		atomic.AddUint32(&counter, 1)

		// Simulate a long running fetch
		resink := make(chan *ent.Response)
...
func TestQueueGapFill(t *testing.T) {
	...
	// Iteratively announce blocks, skipping one entry
	imported := make(chan interface{}, len(hashes)-1)
	tester.fetcher.importedHook = func(header *model.Header, block *model.Block) { imported <- block }
...
func TestImportDeduplication(t *testing.T) {
	...
	counter := uint32(0)
	tester.fetcher.insertChain = func(blocks model.Blocks) (int, error) {
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	tester.blocks = map[common.Hash]*model.Block{head: blocks[head]}
	tester.lock.Unlock()
...
func testDistantAnnouncementDiscarding(t *testing.T, light bool) {
	...
	tester.lock.Lock()
	tester.hashes = []common.Hash{head}
	tester.headers = map[common.Hash]*model.Header{head: blocks[head].Header()}
	tester.blocks = map[common.Hash]*model.Block{head: blocks[head]}
	tester.lock.Unlock()
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	targetBlocks := hashLimit + 2*maxQueueDist
	hashes, blocks := makeChain(targetBlocks, 0, genesis)
	attack := make(map[common.Hash]*model.Block)
