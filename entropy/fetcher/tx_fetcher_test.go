...
var (
	...
	testTxs = []*model.Transaction{
		model.NewTransaction(5577006791947779410, common.Address{0x0f}, new(big.Int), 0, new(big.Int), nil),
		model.NewTransaction(15352856648520921629, common.Address{0xbb}, new(big.Int), 0, new(big.Int), nil),
		model.NewTransaction(3916589616287113937, common.Address{0x86}, new(big.Int), 0, new(big.Int), nil),
		model.NewTransaction(9828766684487745566, common.Address{0xac}, new(big.Int), 0, new(big.Int), nil),
	}
...
func TestTransactionFetcherCleanup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		init: func() *TxFetcher {
			return NewTxFetcher(
				func(common.Hash) bool { return false },
				func(txs []*model.Transaction) []error {
		...
		steps: []interface{}{
			...
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0]}, direct: true},
			isScheduled{nil, nil, nil},
		},
	})
}
...
func TestTransactionFetcherCleanupEmpty(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxEnqueue{peer: "A", txs: []*model.Transaction{}, direct: true},
			isScheduled{nil, nil, nil},
		},
	})
}
...
func TestTransactionFetcherMissingRescheduling(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0]}, direct: true}, // This depends on the deterministic random
			isScheduled{
				tracking: map[string][]common.Hash{
					"A": {testTxsHashes[2]},
				},
...
func TestTransactionFetcherMissingCleanup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			// Deliver the middle transaction requested, the one before which
			// should be dropped and the one after re-requested.
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[1]}, direct: true}, // This depends on the deterministic random
			isScheduled{nil, nil, nil},
		},
	})
}
...
func TestTransactionFetcherBroadcasts(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			// concurrent requests.
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0], testTxs[1], testTxs[2]}, direct: false},
			isWaiting(nil),
			isScheduled{
				tracking: nil,
				fetching: nil,
			...
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0], testTxs[1], testTxs[2]}, direct: true},
			isScheduled{nil, nil, nil},
		},
	})
}
...
func TestTransactionFetcherUnderpricedDedup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
			...
			return NewTxFetcher(
				...
				func(txs []*model.Transaction) []error {
					...
					for i := 0; i < len(errs); i++ {
						if i%2 == 0 {
							errs[i] = blockchain.ErrUnderpriced
						} else {
							errs[i] = blockchain.ErrReplaceUnderpriced
						}
					}
					return errs
				},
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A", hashes: []common.Hash{testTxsHashes[0], testTxsHashes[1]}},
			doWait{time: txArriveTimeout, step: true},
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0], testTxs[1]}, direct: true},
			isScheduled{nil, nil, nil},
...
func TestTransactionFetcherUnderpricedDoSProtection(t *testing.T) {
	...
	for i := 0; i < maxTxUnderpricedSetSize+1; i++ {
		txs = append(txs, model.NewTransaction(rand.Uint64(), common.Address{byte(rand.Intn(256))}, new(big.Int), 0, new(big.Int), nil))
	}
	...
	testTransactionFetcher(t, txFetcherTest{
		...
		steps: append(steps, []interface{}{
			...
			doWait{time: txArriveTimeout, step: true},
			doTxEnqueue{peer: "A", txs: []*model.Transaction{txs[maxTxUnderpricedSetSize]}, direct: true},
...
func TestTransactionFetcherOutOfBoundDeliveries(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			isWaiting(nil),
			isScheduled{nil, nil, nil},
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0]}, direct: false},
			isWaiting(nil),
			isScheduled{nil, nil, nil},
			...
			doTxEnqueue{peer: "B", txs: []*model.Transaction{testTxs[0], testTxs[1], testTxs[2], testTxs[3]}, direct: true},
			isWaiting(nil),
			isScheduled{
				tracking: nil,
				fetching: nil,
...
func TestTransactionFetcherFuzzCrash01(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A", hashes: []common.Hash{testTxsHashes[0]}},
			doWait{time: txArriveTimeout, step: true},
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0]}},
...
func TestTransactionFetcherFuzzCrash03(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A", hashes: []common.Hash{testTxsHashes[0], testTxsHashes[1]}},
			doWait{time: txFetchTimeout, step: true},
			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[0], testTxs[1]}},

			// Notify the dangling transaction once more, partially deliver, clash&crash with a timeout
			doTxNotify{peer: "B", hashes: []common.Hash{testTxsHashes[0]}},
			doWait{time: txArriveTimeout, step: true},

			doTxEnqueue{peer: "A", txs: []*model.Transaction{testTxs[1]}, direct: true},
			doWait{time: txFetchTimeout, step: true},
		},
	})
}
