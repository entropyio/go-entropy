...
type beaconBackfiller struct {
	...
	filled     *model.Header // Last header filled by the last terminated sync loop
...
func (b *beaconBackfiller) suspend() *model.Header {
...
func (d *Downloader) BeaconSync(mode SyncMode, head *model.Header) error {
...
func (d *Downloader) BeaconExtend(mode SyncMode, head *model.Header) error {
...
// Internally backfilling and state sync is done the same way, but the header
// retrieval and scheduling is replaced.
func (d *Downloader) beaconSync(mode SyncMode, head *model.Header, force bool) error {
