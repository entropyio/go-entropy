...
// reassembly.
func makeChain(n int, seed byte, parent *model.Block, empty bool) ([]*model.Block, []model.Receipts) {
...
func (chain *chainData) headers() []*model.Header {
	hdrs := make([]*model.Header, len(chain.blocks))
...
func XTestDelivery(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			if f != nil {
				var (
					emptyList []*model.Header
					txset     [][]*model.Transaction
					uncleset  [][]*model.Header
				)
				...
				for i, txs := range txset {
					txsHashes[i] = model.DeriveSha(model.Transactions(txs), hasher)
				}
				for i, uncles := range uncleset {
					uncleHashes[i] = model.CalcUncleHash(uncles)
				}
...
func (n *network) getTransactions(blocknum uint64) model.Transactions {
	index := blocknum - uint64(n.offset)
	return n.chain[index].Transactions()
}
func (n *network) getReceipts(blocknum uint64) model.Receipts {
	index := blocknum - uint64(n.offset)
...
func (n *network) headers(from int) []*model.Header {
	numHeaders := 128
