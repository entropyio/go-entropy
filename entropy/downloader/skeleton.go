...
type headUpdate struct {
	header *model.Header // Header to update the sync target to
...
type headerResponse struct {
	...
	headers []*model.Header // Chain of headers
}
...
//
// Opposed to the original Entropy block synchronization which is trustless (and
...
// not from within (vs. legacy Entropy sync).
//
...
type skeleton struct {
	db     database.Database // Database backing the skeleton
	...
	scratchSpace  []*model.Header // Scratch space to accumulate headers in (first = recent)
...
func newSkeleton(db database.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) Sync(head *model.Header, force bool) error {
...
func (s *skeleton) sync(head *model.Header) (*model.Header, error) {
	...
	s.scratchSpace = make([]*model.Header, scratchHeaders)
...
func (s *skeleton) initSync(head *model.Header) {
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			if err := batch.Write(); err != nil {
				log.Critical("Failed to write skeleton sync status", "err", err)
			}
			return
		}
	}
	...
	if err := batch.Write(); err != nil {
		log.Critical("Failed to write initial skeleton sync status", "err", err)
	}
	log.Debug("Created initial skeleton subchain", "head", number, "tail", number)
}

// saveSyncStatus marshals the remaining sync tasks into leveldb.
func (s *skeleton) saveSyncStatus(db database.KeyValueWriter) {
...
func (s *skeleton) processNewHead(head *model.Header, force bool) bool {
	...
	if lastchain.Tail >= number {
		...
		if force {
			log.Warning("Beacon chain reorged", "tail", lastchain.Tail, "head", lastchain.Head, "newHead", number)
		}
		return true
	}
	if lastchain.Head+1 < number {
		if force {
			log.Warning("Beacon chain gapped", "head", lastchain.Head, "newHead", number)
		}
		return true
	}
	if parent := rawdb.ReadSkeletonHeader(s.db, number-1); parent.Hash() != head.ParentHash {
		if force {
			log.Warning("Beacon chain forked", "ancestor", parent.Number, "hash", parent.Hash(), "want", head.ParentHash)
		}
		return true
	}
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, ent.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	peer.log.Debug("Fetching skeleton headers", "from", req.head, "count", requestCount)
	...
	case <-timeoutTimer.C:
		...
		peer.log.Warning("Header request timed out, dropping peer", "elapsed", ttl)
		headerTimeoutMeter.Mark(1)
		s.peers.rates.Update(peer.id, ent.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*ent.BlockHeadersPacket)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, ent.BlockHeadersMsg, res.Time, len(headers))
...
func (s *skeleton) revertRequest(req *headerRequest) {
	...
	select {
	case <-req.stale:
		log.Debug("Header request already reverted", "peer", req.peer, "reqid", req.id)
		return
	default:
	}
	close(req.stale)
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	res.peer.log.Debug("Processing header response", "head", res.headers[0].Number, "hash", res.headers[0].Hash(), "count", len(res.headers))
	...
	for s.scratchSpace[0] != nil {
		...
		if s.progress.Subchains[0].Next != s.scratchSpace[0].Hash() {
			...
			log.Warning("Invalid skeleton headers", "peer", s.scratchOwners[0], "number", tail-1, "want", want, "have", have)
		...
		log.Debug("Primary subchain extended", "head", head, "tail", tail, "next", next)
	...
	s.saveSyncStatus(batch)
	if err := batch.Write(); err != nil {
		log.Critical("Failed to write skeleton headers and progress", "err", err)
	}
...
func (s *skeleton) cleanStales(filled *model.Header) error {
	...
	for n := start; n < end; n++ {
		...
		if batch.ValueSize() >= database.IdealBatchSize {
			...
			if err := batch.Write(); err != nil {
				log.Critical("Failed to write beacon trim data", "err", err)
			}
			batch.Reset()
...
func (s *skeleton) Bounds() (head *model.Header, tail *model.Header, err error) {
...
func (s *skeleton) Header(number uint64) *model.Header {
