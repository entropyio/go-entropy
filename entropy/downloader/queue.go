...
type fetchRequest struct {
	...
	Headers []*model.Header // Requested headers, sorted by request order
...
func newFetchResult(header *model.Header, fastSync bool) *fetchResult {
	item := &fetchResult{
		Header: header,
	}
	if !header.EmptyBody() {
...
type queue struct {
	...
	headerTaskPool  map[uint64]*model.Header       // Pending header retrieval tasks, mapping starting indexes to skeleton headers
	...
	headerResults   []*model.Header                // Result cache accumulating the completed headers
	...
	blockTaskPool  map[common.Hash]*model.Header // Pending block (body) retrieval tasks, mapping hashes to headers
	...
	receiptTaskPool  map[common.Hash]*model.Header // Pending receipt retrieval tasks, mapping hashes to headers
...
// skeleton.
func (q *queue) RetrieveHeaders() ([]*model.Header, []common.Hash, int) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) Schedule(headers []*model.Header, hashes []common.Hash, from uint64) []*model.Header {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*model.Header, taskQueue *prque.Prque,
	...
	for proc := 0; len(send) < count && !taskQueue.Empty(); proc++ {
		...
		if err != nil {
			...
			log.Warning("Failed to reserve headers", "err", err)
			// There are no resultslots available. Leave it in the task queue
			break
		}
		if item.Done(kind) {
...
func (q *queue) DeliverHeaders(id string, headers []*model.Header, hashes []common.Hash, headerProcCh chan *headerTask) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	if accepted {
		if headers[0].Number.Uint64() != request.From {
			log.Debug("First header broke chain ordering", "number", headers[0].Number, "hash", hashes[0], "expected", request.From)
			accepted = false
		} else if hashes[len(headers)-1] != target {
			log.Debug("Last header broke skeleton structure ", "number", headers[len(headers)-1].Number, "hash", hashes[len(headers)-1], "expected", target)
			accepted = false
		}
	}
	if accepted {
		...
		for i, header := range headers[1:] {
			...
			if want := request.From + 1 + uint64(i); header.Number.Uint64() != want {
				log.Warning("Header broke chain ordering", "number", header.Number, "hash", hash, "expected", want)
				accepted = false
				break
			}
	...
	if !accepted {
		log.Debug("Skeleton filling not accepted", "from", request.From)
		headerDropMeter.Mark(int64(len(headers)))
	...
	if ready > 0 {
		...
		processHeaders := make([]*model.Header, ready)
		...
		case headerProcCh <- &headerTask{
			...
			log.Debug("Pre-scheduled new headers", "count", len(processHeaders), "from", processHeaders[0].Number)
...
func (q *queue) DeliverBodies(id string, txLists [][]*model.Transaction, txListHashes []common.Hash, uncleLists [][]*model.Header, uncleListHashes []common.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()

	validate := func(index int, header *model.Header) error {
...
func (q *queue) DeliverReceipts(id string, receiptList [][]*model.Receipt, receiptListHashes []common.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(id string, taskPool map[common.Hash]*model.Header,
	...
	results int, validate func(index int, header *model.Header) error,
