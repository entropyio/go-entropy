...
type BlockChain interface {
	...
	InsertReceiptChain(model.Blocks, []model.Receipts) (int, error)
}
...
// model, this method is used by each for data gathering and is instrumented with
		...
		case headers := <-d.headerProcCh:
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					unknown := make([]*model.Header, 0, len(headers))
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	...
	if _, err := d.blockchain.InsertReceiptChain([]*model.Block{block}, []model.Receipts{result.Receipts}); err != nil {
		return err
	}
