...
var log = logger.NewLogger("[downloader]")
...
var (
	...
	fullMaxForkAncestry  uint64 = config.FullImmutabilityThreshold  // Maximum chain reorganisation (locally redeclared so tests can reduce it)
	lightMaxForkAncestry uint64 = config.LightImmutabilityThreshold // Maximum chain reorganisation (locally redeclared so tests can reduce it)
...
type Downloader struct {
	...
	// State sync
	pivotHeader *model.Header // Pivot block header to dynamically push the syncing state root
	...
	bodyFetchHook    func([]*model.Header) // Method to call upon starting a block body fetch
	receiptFetchHook func([]*model.Header) // Method to call upon starting a receipt fetch
...
type LightChain interface {
	...
	GetHeaderByHash(common.Hash) *model.Header
	...
	InsertHeaderChain([]*model.Header, int) (int, error)
...
type BlockChain interface {
	...
	GetBlockByHash(common.Hash) *model.Block

	// CurrentBlock retrieves the head block from the local chain.
	CurrentBlock() *model.Block

	// CurrentFastBlock retrieves the head snap block from the local chain.
	CurrentFastBlock() *model.Block

	// SnapSyncCommitHead directly commits the head block to a certain entity.
	SnapSyncCommitHead(common.Hash) error

	// InsertChain inserts a batch of blocks into the local chain.
	InsertChain(model.Blocks) (int, error)

	// InsertReceiptChain inserts a batch of receipts into the local chain.
	InsertReceiptChain(model.Blocks, []model.Receipts, uint64) (int, error)
	// Snapshots returns the blockchain snapshot tree to paused it during sync.
	Snapshots() *snapshot.Tree
}

// New creates a new downloader to fetch hashes and blocks from remote peers.
func New(checkpoint uint64, stateDb database.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() entropyio.SyncProgress {
...
func (d *Downloader) RegisterPeer(id string, version uint, peer Peer) error {
	...
	if err := d.peers.Register(newPeerConnection(id, version, peer, log)); err != nil {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "entropy", p.version, "head", hash, "td", td, "mode", d.mode)
	defer func(start time.Time) {
		log.Debug("Synchronisation terminated", "elapsed", common.PrettyDuration(time.Since(start)))
	}(time.Now())

	// Look up the sync boundaries: the common ancestor and the target block
	var latest, pivot *model.Header
	if !beaconMode {
			...
			if pivot = d.skeleton.Header(number); pivot == nil {
				...
				if number < oldest.Number.Uint64() {
					...
					if len(headers) == count {
						...
						log.Warning("Retrieved pivot header from local", "number", pivot.Number, "hash", pivot.Hash(), "latest", latest.Number, "oldest", oldest.Number)
					}
				}
			}
...
// a remote peer.
func (d *Downloader) fetchHead(p *peerConnection) (head *model.Header, pivot *model.Header, err error) {
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *model.Header) (uint64, error) {
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (uint64, error) {
	...
	if len(headers) == 0 {
		p.log.Warning("Empty head header set")
	...
	for i, header := range headers {
		...
		if number := header.Number.Int64(); number != expectNumber {
			p.log.Warning("Head headers broke chain ordering", "index", i, "requested", expectNumber, "received", number)
	...
	if hash != (common.Hash{}) {
		if int64(number) <= floor {
			p.log.Warning("Ancestor below allowance", "number", number, "hash", hash, "allowance", floor)
...
func (d *Downloader) findAncestorBinarySearch(p *peerConnection, mode SyncMode, remoteHeight uint64, floor int64) (uint64, error) {
	...
	for start+1 < end {
		...
		// Make sure the peer actually gave something valid
		if len(headers) != 1 {
			p.log.Warning("Multiple headers for single request", "headers", len(headers))
		...
		if header.Number.Uint64() != check {
			p.log.Warning("Received non requested header", "number", header.Number, "hash", header.Hash(), "request", check)
	...
	if int64(start) <= floor {
		p.log.Warning("Ancestor below allowance", "number", start, "hash", hash, "allowance", floor)
		return 0, errInvalidAncestor
	}
	...
	for {
		...
		if pivoting {
			if len(headers) == 2 {
				if have, want := headers[0].Number.Uint64(), pivot+uint64(fsMinFullBlocks); have != want {
					log.Warning("Peer sent invalid next pivot", "have", have, "want", want)
				...
				if have, want := headers[1].Number.Uint64(), pivot+2*uint64(fsMinFullBlocks)-8; have != want {
					log.Warning("Peer sent invalid pivot confirmer", "have", have, "want", want)
					return fmt.Errorf("%w: next pivot confirmer number %d != requested %d", errInvalidChain, have, want)
				}
				log.Warning("Pivot seemingly stale, moving", "old", pivot, "new", headers[0].Number)
		...
		if skeleton && len(headers) == 0 {
			...
			if from+uint64(MaxHeaderFetch)-1 <= head {
				p.log.Warning("Peer withheld skeleton headers", "advertised", head, "withheld", from+uint64(MaxHeaderFetch)-1)
		...
		if skeleton {
			...
			if n := len(headers); n < MaxHeaderFetch && headers[n-1].Number.Uint64() < head {
				p.log.Warning("Peer withheld headers", "advertised", head, "delivered", headers[n-1].Number.Uint64())
		...
		if len(headers) == 0 && !progressed {
			p.log.Debug("All headers delayed, waiting")
			select {
			case <-time.After(fsHeaderContCheck):
				continue
			case <-d.cancelCh:
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*model.Header) ([]*model.Header, []common.Hash, int, error) {
...
func (d *Downloader) processHeaders(origin uint64, td, ttd *big.Int, beaconMode bool) error {
	...
	defer func() {
		if rollback > 0 {
			...
			log.Warning("Rolled back chain segment",
				...
				if mode == SnapSync || mode == LightSync {
					...
					if len(chunkHeaders) > 0 {
						if n, err := d.lightchain.InsertHeaderChain(chunkHeaders, frequency); err != nil {
							...
							log.Warning("Invalid header encountered", "number", chunkHeaders[n].Number, "hash", chunkHashes[n], "parent", chunkHeaders[n].ParentHash, "err", err)
							return fmt.Errorf("%w: %v", errInvalidChain, err)
						}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	blocks := make([]*model.Block, len(results))
	for i, result := range results {
		blocks[i] = model.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
	}
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height >= pivot.Number.Uint64()+2*uint64(fsMinFullBlocks)-uint64(reorgProtHeaderDelay) {
				log.Warning("Pivot became stale, moving", "old", pivot.Number.Uint64(), "new", height-uint64(fsMinFullBlocks)+uint64(reorgProtHeaderDelay))
...
func (d *Downloader) commitSnapSyncData(results []*fetchResult, stateSync *stateSync) error {
	...
	blocks := make([]*model.Block, len(results))
	receipts := make([]model.Receipts, len(results))
	for i, result := range results {
		blocks[i] = model.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	block := model.NewBlockWithHeader(result.Header).WithBody(result.Transactions, result.Uncles)
	log.Debug("Committing snap sync pivot as new head", "number", block.Number(), "hash", block.Hash())

	// Commit the pivot block as the new head, will require full sync from here on
	if _, err := d.blockchain.InsertReceiptChain([]*model.Block{block}, []model.Receipts{result.Receipts}, d.ancientLimit); err != nil {
		return err
	}
...
func (d *Downloader) readHeaderRange(last *model.Header, count int) []*model.Header {
	var (
		current = last
		headers []*model.Header
	)
