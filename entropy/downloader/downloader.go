...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [entropy/62] Maximum number of headers to queue for import (DOS protection)
	maxHeadersProcess        = 2048                         // Number of header download results to import at once into the chain
	maxResultsProcess        = 2048                         // Number of content download results to import at once into the chain
	maxForkAncestry   uint64 = config.ImmutabilityThreshold // Maximum chain reorganisation (locally redeclared so tests can reduce it)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [entropy/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [entropy/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [entropy/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [entropy/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [entropy/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*model.Header // [entropy/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [entropy/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb database.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	...
	dl := &Downloader{
		...
		headerProcCh:   make(chan []*model.Header, 1),
		quitCh:         make(chan struct{}),
		stateCh:        make(chan dataPack),
		stateSyncStart: make(chan *stateSync),
		syncStatsState: stateSyncStats{
			processed: mapper.ReadFastTrieProgress(stateDb),
		},
...
// these are zero.
func (d *Downloader) Progress() entropy.SyncProgress {
	...
	if d.mode == FastSync {
		...
		if origin+1 < frozen {
			...
			for i := origin + 1; i < d.lightchain.CurrentHeader().Number.Uint64(); i++ {
				hashes = append(hashes, mapper.ReadCanonicalHash(d.stateDB, i))
			}
...
func (d *Downloader) fetchHeight(p *peerConnection) (*model.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if (d.mode == FastSync || d.mode == LightSync) && head.Number.Uint64() < d.checkpoint {
				log.Warning("Remote head below checkpoint", "number", head.Number, "hash", head.Hash())
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *model.Header) (uint64, error) {
	...
	log.Debug("Looking for common ancestor", "local", localHeight, "remote", remoteHeight)
	...
	log.Debug("Span searching for common ancestor", "count", count, "from", from, "skip", skip)
	...
	log.Debug("Binary searching for common ancestor", "start", start, "end", end)
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			if len(headers) > 0 {
				...
				log.Debug("All headers delayed, waiting")
				select {
				case <-time.After(fsHeaderContCheck):
					getHeaders(from)
					continue
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*model.Header) ([]*model.Header, int, error) {
...
func (d *Downloader) fetchParts(deliveryCh chan dataPack, deliver func(dataPack) (int, error), wakeCh chan bool,
	...
	fetchHook func([]*model.Header), fetch func(*peerConnection, *fetchRequest) error, cancel func(*fetchRequest), capacity func(*peerConnection) int,
				...
				if peer := d.peers.Peer(pid); peer != nil {
					...
					if fails > 2 {
						...
						if d.dropPeer == nil {
							// The dropPeer method is nil when `--copydb` is used for a local copy.
							// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
							log.Warning("Downloader wants to drop peer, but peerdrop-function is not set", "peer", pid)
						} else {
							d.dropPeer(pid)
...
func (d *Downloader) processHeaders(origin uint64, pivot uint64, td *big.Int) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			log.Warning("Rolled back headers", "count", len(hashes),
		...
		case headers := <-d.headerProcCh:
			...
			for len(headers) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					...
					unknown := make([]*model.Header, 0, len(chunk))
...
func (d *Downloader) processFastSyncContent(latest *model.Header) error {
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height > pivot+2*uint64(fsMinFullBlocks) {
				log.Warning("Pivot became stale, moving", "old", pivot, "new", height-uint64(fsMinFullBlocks))
				pivot = height - uint64(fsMinFullBlocks)
			}
		}
...
func (d *Downloader) DeliverHeaders(id string, headers []*model.Header) (err error) {
	return d.deliver(id, d.headerCh, &headerPack{id, headers}, headerInMeter, headerDropMeter)
}

// DeliverBodies injects a new batch of block bodies received from a remote node.
func (d *Downloader) DeliverBodies(id string, transactions [][]*model.Transaction, uncles [][]*model.Header) (err error) {
	return d.deliver(id, d.bodyCh, &bodyPack{id, transactions, uncles}, bodyInMeter, bodyDropMeter)
}

// DeliverReceipts injects a new batch of receipts received from a remote node.
func (d *Downloader) DeliverReceipts(id string, receipts [][]*model.Receipt) (err error) {
