...
func NewFakePeer(id string, db database.Database, hc *blockchain.HeaderChain, dl *Downloader) *FakePeer {
...
func (p *FakePeer) RequestBodies(hashes []common.Hash) error {
	...
	for _, hash := range hashes {
		block := mapper.ReadBlock(p.db, hash, *p.hc.GetBlockNumber(hash))
...
func (p *FakePeer) RequestReceipts(hashes []common.Hash) error {
	...
	for _, hash := range hashes {
		receipts = append(receipts, mapper.ReadRawReceipts(p.db, hash, *p.hc.GetBlockNumber(hash)))
	}
