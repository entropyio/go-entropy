...
type downloadTester struct {
	...
	ancientHeaders  map[common.Hash]*model.Header  // Ancient headers belonging to the tester
	ancientBlocks   map[common.Hash]*model.Block   // Ancient blocks belonging to the tester
	ancientReceipts map[common.Hash]model.Receipts // Ancient receipts belonging to the tester
...
func newTester() *downloadTester {
	tester := &downloadTester{
		...
		ownHeaders:  map[common.Hash]*model.Header{testGenesis.Hash(): testGenesis.Header()},
		ownBlocks:   map[common.Hash]*model.Block{testGenesis.Hash(): testGenesis},
		ownReceipts: map[common.Hash]model.Receipts{testGenesis.Hash(): nil},
		...
		ancientHeaders:  map[common.Hash]*model.Header{testGenesis.Hash(): testGenesis.Header()},
		ancientBlocks:   map[common.Hash]*model.Block{testGenesis.Hash(): testGenesis},
		ancientReceipts: map[common.Hash]model.Receipts{testGenesis.Hash(): nil},
		ancientChainTd:  map[common.Hash]*big.Int{testGenesis.Hash(): testGenesis.Difficulty()},
	}
	tester.stateDb = mapper.NewMemoryDatabase()
...
func (dl *downloadTester) InsertChain(blocks model.Blocks) (i int, err error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
	...
	for i, block := range blocks {
		...
		dl.ownReceipts[block.Hash()] = make(model.Receipts, 0)
...
func (dl *downloadTester) InsertReceiptChain(blocks model.Blocks, receipts []model.Receipts, ancientLimit uint64) (i int, err error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", entropy.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", entropy.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", entropy.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", entropy.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want entropy.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", entropy.SyncProgress{
