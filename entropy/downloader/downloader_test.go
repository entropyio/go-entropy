...
type downloadTester struct {
	...
	genesis *model.Block      // Genesis blocks used by the tester and peers
	...
	ownHeaders  map[common.Hash]*model.Header  // Headers belonging to the tester
	ownBlocks   map[common.Hash]*model.Block   // Blocks belonging to the tester
	ownReceipts map[common.Hash]model.Receipts // Receipts belonging to the tester
	ownChainTd  map[common.Hash]*big.Int       // Total difficulties of the blocks in the local chain

	peerHashes   map[string][]common.Hash                  // Hash chain belonging to different test peers
	peerHeaders  map[string]map[common.Hash]*model.Header  // Headers belonging to different test peers
	peerBlocks   map[string]map[common.Hash]*model.Block   // Blocks belonging to different test peers
	peerReceipts map[string]map[common.Hash]model.Receipts // Receipts belonging to different test peers
...
func newTester() *downloadTester {
	...
	genesisObj := genesis.GenesisBlockForTesting(testdb, testAddress, big.NewInt(1000000000))

	tester := &downloadTester{
		genesis:           genesisObj,
		peerDb:            testdb,
		ownHashes:         []common.Hash{genesisObj.Hash()},
		ownHeaders:        map[common.Hash]*model.Header{genesisObj.Hash(): genesisObj.Header()},
		ownBlocks:         map[common.Hash]*model.Block{genesisObj.Hash(): genesisObj},
		ownReceipts:       map[common.Hash]model.Receipts{genesisObj.Hash(): nil},
		ownChainTd:        map[common.Hash]*big.Int{genesisObj.Hash(): genesisObj.Difficulty()},
		peerHashes:        make(map[string][]common.Hash),
		peerHeaders:       make(map[string]map[common.Hash]*model.Header),
		peerBlocks:        make(map[string]map[common.Hash]*model.Block),
		peerReceipts:      make(map[string]map[common.Hash]model.Receipts),
		peerChainTds:      make(map[string]map[common.Hash]*big.Int),
		peerMissingStates: make(map[string]map[common.Hash]bool),
	}
	tester.stateDb = database.NewMemDatabase()
	tester.stateDb.Put(genesisObj.Root().Bytes(), []byte{0x00})
...
// reassembly.
func (dl *downloadTester) makeChain(n int, seed byte, parent *model.Block, parentReceipts model.Receipts, heavy bool) ([]common.Hash, map[common.Hash]*model.Header, map[common.Hash]*model.Block, map[common.Hash]model.Receipts) {
	// Generate the block chain
	blocks, receipts := blockchain.GenerateChain(config.TestChainConfig, parent, ethash.NewFaker(), dl.peerDb, n, func(i int, block *blockchain.BlockGen) {
		...
		if parent == dl.genesis && i%3 == 0 {
			...
			tx, err := model.SignTx(model.NewTransaction(block.TxNonce(testAddress), common.Address{seed}, big.NewInt(1000), config.TxGas, nil, nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
	...
	headerm := make(map[common.Hash]*model.Header, n+1)
	headerm[parent.Hash()] = parent.Header()

	blockm := make(map[common.Hash]*model.Block, n+1)
	blockm[parent.Hash()] = parent

	receiptm := make(map[common.Hash]model.Receipts, n+1)
...
func (dl *downloadTester) makeChainFork(n, f int, parent *model.Block, parentReceipts model.Receipts, balanced bool) ([]common.Hash, []common.Hash, map[common.Hash]*model.Header, map[common.Hash]*model.Header, map[common.Hash]*model.Block, map[common.Hash]*model.Block, map[common.Hash]model.Receipts, map[common.Hash]model.Receipts) {
...
func (dl *downloadTester) GetHeaderByHash(hash common.Hash) *model.Header {
	dl.lock.RLock()
...
func (dl *downloadTester) GetBlockByHash(hash common.Hash) *model.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentHeader() *model.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentBlock() *model.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentFastBlock() *model.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) InsertHeaderChain(headers []*model.Header, checkFreq int) (int, error) {
	dl.lock.Lock()
...
func (dl *downloadTester) InsertChain(blocks model.Blocks) (int, error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) InsertReceiptChain(blocks model.Blocks, receipts []model.Receipts) (int, error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) newPeer(id string, version int, hashes []common.Hash, headers map[common.Hash]*model.Header, blocks map[common.Hash]*model.Block, receipts map[common.Hash]model.Receipts) error {
...
func (dl *downloadTester) newSlowPeer(id string, version int, hashes []common.Hash, headers map[common.Hash]*model.Header, blocks map[common.Hash]*model.Block, receipts map[common.Hash]model.Receipts, delay time.Duration) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
	...
	if err == nil {
		...
		dl.peerHeaders[id] = make(map[common.Hash]*model.Header)
		dl.peerBlocks[id] = make(map[common.Hash]*model.Block)
		dl.peerReceipts[id] = make(map[common.Hash]model.Receipts)
...
func (dlp *downloadTesterPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool) error {
	...
	result := make([]*model.Header, 0, amount)
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash) error {
	...
	transactions := make([][]*model.Transaction, 0, len(hashes))
	uncles := make([][]*model.Header, 0, len(hashes))
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []common.Hash) error {
	...
	results := make([][]*model.Receipt, 0, len(hashes))
	for _, hash := range hashes {
		if receipt, ok := receipts[hash]; ok {
...
func TestInactiveDownloader62(t *testing.T) {
	...
	if err := tester.downloader.DeliverHeaders("bad peer", []*model.Header{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
	if err := tester.downloader.DeliverBodies("bad peer", [][]*model.Transaction{}, [][]*model.Header{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
}
...
func TestInactiveDownloader63(t *testing.T) {
	...
	if err := tester.downloader.DeliverReceipts("bad peer", [][]*model.Receipt{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
}
...
func (ftp *floodingTestPeer) RequestHeadersByNumber(from uint64, count, skip int, reverse bool) error {
	...
	for i := 0; i < cap(deliveriesDone); i++ {
		...
		go func() {
			ftp.tester.downloader.DeliverHeaders(peer, []*model.Header{{}, {}, {}, {}})
