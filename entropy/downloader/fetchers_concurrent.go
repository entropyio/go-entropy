...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *ent.Response) (*ent.Request, error)
	...
	deliver(peer *peerConnection, packet *ent.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *ent.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*ent.Request)
	defer func() {
	...
	ordering := make(map[*ent.Request]int)
	timeouts := prque.New(func(data interface{}, index int) {
		ordering[data.(*ent.Request)] = index
	})
	...
	stales := make(map[string]*ent.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
			...
			for _, peer := range d.peers.AllPeers() {
				...
				if pending == nil && stale == nil {
					...
					if waited := time.Since(stale.Sent); waited > timeoutGracePeriod {
						...
						// stall the sync.
						log.Warning("Peer stalling, dropping", "waited", common.PrettyDuration(waited))
						d.dropPeer(peer.id)
					}
				}
			}
		...
		case event := <-peering:
			...
			if event.join {
				...
				if _, ok := pending[peerid]; ok {
					log.Error("Pending request exists for joining peer")
				}
				if _, ok := stales[peerid]; ok {
					log.Error("Stale request exists for joining peer")
				}
