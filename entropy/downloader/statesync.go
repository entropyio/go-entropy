	...
	for {
		...
		case req := <-d.trackStateReq:
			...
			if old := active[req.peer.id]; old != nil {
				log.Warning("Busy peer assigned new state fetch", "peer", old.peer.id)
	...
	for s.sched.Pending() > 0 {
		...
		case req := <-s.deliver:
			...
			if len(req.items) <= 2 && !req.dropped && req.timedOut() {
				...
				log.Warning("Stalling state sync, dropping peer", "peer", req.peer.id)
				if s.d.dropPeer == nil {
					// The dropPeer method is nil when `--copydb` is used for a local copy.
					// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
					log.Warning("Downloader wants to drop peer, but peerdrop-function is not set", "peer", req.peer.id)
				} else {
			...
			if err != nil {
				log.Warning("Node data write error", "err", err)
				return err
			}
...
func (s *stateSync) assignTasks() {
	...
	for _, p := range peers {
		...
		capacity := p.NodeDataCapacity(s.d.requestRTT())
		req := &stateReq{peer: p, timeout: s.d.requestTTL()}
		s.fillTasks(capacity, req)
...
func (s *stateSync) fillTasks(n int, req *stateReq) {
	...
	if len(s.tasks) < n {
		hashList := s.sched.Missing(n - len(s.tasks))
		for _, hashObj := range hashList {
			s.tasks[hashObj] = &stateTask{make(map[string]struct{})}
		}
	}
	...
	for hashObj, t := range s.tasks {
		...
		req.items = append(req.items, hashObj)
		req.tasks[hashObj] = t
		delete(s.tasks, hashObj)
	}
}
...
func (s *stateSync) process(req *stateReq) (int, error) {
	...
	for _, blob := range req.response {
		_, hashObj, err := s.processNodeData(blob)
		switch err {
		case nil:
			s.numUncommitted++
		...
		case trie.ErrAlreadyProcessed:
			duplicate++
		default:
			return successful, fmt.Errorf("invalid state node %s: %v", hashObj.TerminalString(), err)
		}
		delete(req.tasks, hashObj)
	}
	// Put unfulfilled tasks back into the retry queue
	npeers := s.d.peers.Len()
	for hashObj, task := range req.tasks {
		...
		if len(task.attempts) >= npeers {
			return successful, fmt.Errorf("state node %s failed with all peers (%d tries, %d peers)", hashObj.TerminalString(), len(task.attempts), npeers)
		}
		// Missing item, place into the retry queue.
		s.tasks[hashObj] = task
	}
...
func (s *stateSync) updateStats(written, duplicate, unexpected int, duration time.Duration) {
	...
	if written > 0 {
		mapper.WriteFastTrieProgress(s.d.stateDB, s.d.syncStatsState.processed)
	}
}
