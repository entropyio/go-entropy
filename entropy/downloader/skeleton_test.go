...
// on initial sartup.
func (hf *hookedBackfiller) suspend() *model.Header {
...
type skeletonTestPeer struct {
	...
	headers []*model.Header // Headers to serve when requested

	serve func(origin uint64) []*model.Header // Hook to allow custom responses
...
func newSkeletonTestPeer(id string, headers []*model.Header) *skeletonTestPeer {
...
// otherwise require dedicated peer model.
func newSkeletonTestPeerWithHook(id string, headers []*model.Header, serve func(origin uint64) []*model.Header) *skeletonTestPeer {
	return &skeletonTestPeer{
		id:      id,
		headers: headers,
		serve:   serve,
...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *ent.Response) (*ent.Request, error) {
	...
	if headers == nil {
		headers = make([]*model.Header, 0, amount)
	...
	go func() {
		...
		if err := <-res.Done; err != nil {
			log.Warning("Skeleton test peer response rejected", "err", err)
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *ent.Response) (*ent.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *ent.Response) (*ent.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, chan *ent.Response) (*ent.Request, error) {
...
func TestSkeletonSyncInit(t *testing.T) {
	...
	var (
		genesis  = &model.Header{Number: big.NewInt(0)}
		block49  = &model.Header{Number: big.NewInt(49)}
		block49B = &model.Header{Number: big.NewInt(49), Extra: []byte("B")}
		block50  = &model.Header{Number: big.NewInt(50), ParentHash: block49.Hash()}
	)
	tests := []struct {
		headers  []*model.Header // Database content (beside the genesis)
		oldstate []*subchain     // Old sync state with various interrupted subchains
		head     *model.Header   // New head header to announce to reorg to
...
func TestSkeletonSyncExtend(t *testing.T) {
	...
	var (
		...
		block49B = &model.Header{Number: big.NewInt(49), Extra: []byte("B")}
		block50  = &model.Header{Number: big.NewInt(50), ParentHash: block49.Hash()}
		block51  = &model.Header{Number: big.NewInt(51), ParentHash: block50.Hash()}
	)
	tests := []struct {
		head     *model.Header // New head header to announce to reorg to
		extend   *model.Header // New head header to announce to extend with
		newstate []*subchain   // Expected sync state after the reorg
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	chain := []*model.Header{{Number: big.NewInt(0)}}
	for i := 1; i < 10000; i++ {
		chain = append(chain, &model.Header{
	...
	tests := []struct {
		...
		newHead  *model.Header     // New header to annount on top of the old one
		...
		// Joining with a new peer should however unblock the sync.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-skipper", append(append(append([]*model.Header{}, chain[:99]...), nil), chain[100:]...)),
			},
		...
		// Joining with a new peer should however unblock the sync.
		{
			head: chain[requestHeaders+100],
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-skipper", append(append(append([]*model.Header{}, chain[:50]...), nil), chain[51:]...)),
			},
			midstate: []*subchain{{Head: requestHeaders + 100, Tail: 100}},
			midserve: requestHeaders + 101 - 3, // len - head - genesis - missing
			middrop:  1,                        // penalize shortened header deliveries
		...
		// Joining with a new peer should however unblock the sync.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-duper", append(append(append([]*model.Header{}, chain[:99]...), chain[98]), chain[100:]...)),
			},
			midstate: []*subchain{{Head: requestHeaders + 100, Tail: 100}},
		...
		// Joining with a new peer should however unblock the sync.
		{
			head: chain[requestHeaders+100], // We want to force the 100th header to be a request boundary
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-duper", append(append(append([]*model.Header{}, chain[:50]...), chain[49]), chain[51:]...)),
			},
			midstate: []*subchain{{Head: requestHeaders + 100, Tail: 100}},
			midserve: requestHeaders + 101 - 2, // len - head - genesis
			middrop:  1,                        // penalize invalid header sequences
		...
		// and the scratch space must not be consumed further.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeerWithHook("peer-1", chain, func(origin uint64) []*model.Header {
					if origin == chain[2*requestHeaders+1].Number.Uint64() {
						time.Sleep(100 * time.Millisecond)
					}
					return nil // Fallback to default behavior, just delayed
				}),
				newSkeletonTestPeerWithHook("peer-2", chain, func(origin uint64) []*model.Header {
					if origin == chain[2*requestHeaders+1].Number.Uint64() {
						time.Sleep(100 * time.Millisecond)
					}
					return nil // Fallback to default behavior, just delayed
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, ent.ETH66, peer, log))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, ent.ETH66, tt.newPeer, log)); err != nil {
