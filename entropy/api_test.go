...
func TestAccountRange(t *testing.T) {
	var (
		statedb  = state.NewDatabase(mapper.NewMemoryDatabase())
...
func TestStorageRangeAt(t *testing.T) {
	...
	var (
		stateObj, _ = state.New(common.Hash{}, state.NewDatabase(mapper.NewMemoryDatabase()))
	...
	for _, entry := range storage {
		stateObj.SetState(addr, *entry.Key, entry.Value)
	}
	...
	for _, test := range tests {
		result, err := storageRangeAt(stateObj.StorageTrie(addr), test.start, test.limit)
		if err != nil {
			t.Error(err)
		}
