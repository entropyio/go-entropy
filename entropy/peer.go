...
// PeerInfo represents a short summary of the Entropy sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Entropy protocol version negotiated
...
type peer struct {
	...
	*p2p.Peer
	...
	queuedTxs   chan []*model.Transaction // Queue of transactions to broadcast to the peer
	queuedProps chan *propEvent           // Queue of blocks to broadcast to the peer
	queuedAnns  chan *model.Block         // Queue of blocks to announce to the peer
...
func newPeer(version int, p *p2p.Peer, rw p2p.MsgReadWriter) *peer {
	return &peer{
		...
		queuedTxs:   make(chan []*model.Transaction, maxQueuedTxs),
		queuedProps: make(chan *propEvent, maxQueuedProps),
		queuedAnns:  make(chan *model.Block, maxQueuedAnns),
...
func (p *peer) broadcast() {
	for {
		...
		case prop := <-p.queuedProps:
			...
			log.Debug("Propagated block", "number", prop.block.Number(), "hash", prop.block.Hash(), "td", prop.td)

		case block := <-p.queuedAnns:
			if err := p.SendNewBlockHashes([]common.Hash{block.Hash()}, []uint64{block.NumberU64()}); err != nil {
				return
			}
			log.Debug("Announced block", "number", block.Number(), "hash", block.Hash())
...
func (p *peer) SendTransactions(txs model.Transactions) error {
...
func (p *peer) AsyncSendTransactions(txs []*model.Transaction) {
	select {
...
// dropped.
func (p *peer) AsyncSendNewBlockHash(block *model.Block) {
	select {
	...
	default:
		log.Debug("Dropping block announcement", "number", block.NumberU64(), "hash", block.Hash())
	}
}

// SendNewBlock propagates an entire block to a remote peer.
func (p *peer) SendNewBlock(block *model.Block, td *big.Int) error {
	// Mark all the block hash as known, but ensure we don't overflow our limits
	p.knownBlocks.Add(block.Hash())
	for p.knownBlocks.Cardinality() >= maxKnownBlocks {
		p.knownBlocks.Pop()
...
func (p *peer) AsyncSendNewBlock(block *model.Block, td *big.Int) {
	select {
...
func (p *peer) SendBlockHeaders(headers []*model.Header) error {
...
func (p *peer) SendBlockBodiesRLP(bodies []rlputil.RawValue) error {
...
func (p *peer) SendReceiptsRLP(receipts []rlputil.RawValue) error {
...
// Handshake executes the entropy protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("entropy/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Entropy sub-protocol.
type peerSet struct {
