...
func (h *handler) syncTransactions(p *ent.Peer) {
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *ent.Peer) bool {
	select {
	...
	for {
		...
		case err := <-cs.doneCh:
			...
			if errors.Is(err, downloader.ErrMergeTransition) && time.Since(cs.warned) > 10*time.Second {
				log.Warning("Local chain is post-merge, waiting for beacon client sync switch-over...")
				cs.warned = time.Now()
			}
		case <-cs.force.C:
			cs.forced = true
...
func peerToSyncOp(mode downloader.SyncMode, p *ent.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		if stored := rawdb.ReadFastTxLookupLimit(h.database); stored == nil {
			...
			log.Warning("Update txLookup limit", "provided", limit, "updated", *stored)
		}
	}
