...
type ChainReader interface {
	...
	BlockByNumber(ctx context.Context, number *big.Int) (*model.Block, error)
	HeaderByHash(ctx context.Context, hash common.Hash) (*model.Header, error)
	HeaderByNumber(ctx context.Context, number *big.Int) (*model.Header, error)
	TransactionCount(ctx context.Context, blockHash common.Hash) (uint, error)
	TransactionInBlock(ctx context.Context, blockHash common.Hash, index uint) (*model.Transaction, error)

	// This method subscribes to notifications about changes of the head block of
	// the canonical chain.
	SubscribeNewHead(ctx context.Context, ch chan<- *model.Header) (Subscription, error)
}
...
type TransactionReader interface {
	...
	// it's not pending.
	TransactionByHash(ctx context.Context, txHash common.Hash) (tx *model.Transaction, isPending bool, err error)
	// TransactionReceipt returns the receipt of a mined transaction. Note that the
	// transaction may not be included in the current canonical chain even if a receipt
	// exists.
	TransactionReceipt(ctx context.Context, txHash common.Hash) (*model.Receipt, error)
}
...
type SyncProgress struct {
	...
	// "fast sync" fields. These used to be sent by gen, but are no longer used
	// since version v1.10.
...
type CallMsg struct {
	...
	AccessList model.AccessList // EIP-2930 access list.
}
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by entropy_getLogs, return logs only from block with this hash
...
type LogFilterer interface {
	FilterLogs(ctx context.Context, q FilterQuery) ([]model.Log, error)
	SubscribeFilterLogs(ctx context.Context, q FilterQuery, ch chan<- model.Log) (Subscription, error)
}
...
type TransactionSender interface {
	SendTransaction(ctx context.Context, tx *model.Transaction) error
}
...
type PendingStateEventer interface {
	SubscribePendingTransactions(ctx context.Context, ch chan<- *model.Transaction) (Subscription, error)
}
