...
// Encoder is implemented by model that require custom
...
// public fields. Recursive struct model are supported.
//
...
func writeBigInt(i *big.Int, w *encbuf) error {
	if cmp := i.Cmp(big0); cmp == -1 {
		return fmt.Errorf("rlputil: cannot encode negative *big.Int")
	} else if cmp == 0 {
		w.str = append(w.str, 0x80)
	} else {
...
func writeEncoderNoPtr(val reflect.Value, w *encbuf) error {
	if !val.CanAddr() {
		...
		return fmt.Errorf("rlputil: game over: unadressable value of type %v, EncodeRLP is pointer method", val.Type())
	}
...
func makeSliceWriter(typ reflect.Type, ts tags) (writer, error) {
	etypeinfo, err := genCachedTypeInfo(typ.Elem(), tags{})
	if err != nil {
		return nil, err
	}
