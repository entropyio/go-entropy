...
var (
	...
	EOL = errors.New("rlputil: end of list")

	// Actual Errors
	ErrExpectedString   = errors.New("rlputil: expected String or Byte")
	ErrExpectedList     = errors.New("rlputil: expected List")
	ErrCanonInt         = errors.New("rlputil: non-canonical integer format")
	ErrCanonSize        = errors.New("rlputil: non-canonical size information")
	ErrElemTooLarge     = errors.New("rlputil: element is larger than containing list")
	ErrValueTooLarge    = errors.New("rlputil: value size exceeds available input length")
	ErrMoreThanOneValue = errors.New("rlputil: input contains more than one value")

	// internal errors
	errNotInList     = errors.New("rlputil: call of ListEnd outside of any list")
	errNotAtEOL      = errors.New("rlputil: call of ListEnd not positioned at EOL")
	errUintOverflow  = errors.New("rlputil: uint overflow")
	errNoPointer     = errors.New("rlputil: interface given to Decode must be a pointer")
	errDecodeIntoNil = errors.New("rlputil: pointer given to Decode must not be nil")
)

// Decoder is implemented by model that require custom RLP
...
// pointer. This tag can be useful when decoding recursive model.
//
//     type StructWithEmptyOK struct {
//         Foo *[20]byte `rlputil:"nil"`
//     }
//
// To decode into a slice, the input must be a list and the resulting
// slice will contain the input elements in order. For byte slices,
// the input must be an RLP string. Array model decode similarly, with
...
//
// Non-empty interface model are not supported, nor are booleans,
...
func (err *decodeError) Error() string {
	...
	return fmt.Sprintf("rlputil: %s for %v%s", err.msg, err.typ, ctx)
}
...
func makeDecoder(typ reflect.Type, tags tags) (dec decoder, err error) {
	...
	default:
		return nil, fmt.Errorf("rlputil: type %v is not RLP-serializable", typ)
	}
}
...
func makeListDecoder(typ reflect.Type, tag tags) (decoder, error) {
	...
	etypeinfo, err := genCachedTypeInfo(etype, tags{})
	if err != nil {
		return nil, err
	}
	var dec decoder
...
func decodeInterface(s *Stream, val reflect.Value) error {
	if val.Type().NumMethod() != 0 {
		return fmt.Errorf("rlputil: type %v is not RLP-serializable", val.Type())
	}
	kind, _, err := s.Kind()
	if err != nil {
		return err
...
// This decoder is used for non-pointer values of model
...
func (s *Stream) Bool() (bool, error) {
	...
	case 1:
		return true, nil
	default:
		return false, fmt.Errorf("rlputil: invalid boolean value: %d", num)
	}
}
