...
var log = logger.NewLogger("[influexdb]")
...
func InfluxDBWithTags(r metrics.Registry, d time.Duration, url, database, username, password, namespace string, tags map[string]string) {
	...
	if err != nil {
		log.Warning("Unable to parse InfluxDB", "url", url, "err", err)
		return
	}
	...
	if err := rep.makeClient(); err != nil {
		log.Warning("Unable to make InfluxDB client", "err", err)
		return
	}
	...
	for {
		...
		case <-intervalTicker:
			if err := r.send(); err != nil {
				log.Warning("Unable to send to InfluxDB", "err", err)
			}
		case <-pingTicker:
			_, _, err := r.client.Ping()
			if err != nil {
				log.Warning("Got error while sending a ping to InfluxDB, trying to recreate client", "err", err)
