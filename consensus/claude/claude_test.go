...
func mockNewClaudeContext(db trie.TrieDatabase) *model.ClaudeContext {
	claudeContext, err := model.NewClaudeContextFromHash(db, &model.ClaudeContextHash{})
	if err != nil {
		return nil
	}
	delegator := []byte{}
	...
	claudeContext.SetValidators(addresses)
	for j := 0; j < len(MockEpoch); j++ {
		delegator = common.HexToAddress(MockEpoch[j]).Bytes()
		candidate = common.HexToAddress(MockEpoch[j]).Bytes()
		claudeContext.DelegateTrie().TryUpdate(append(candidate, delegator...), candidate)
		claudeContext.CandidateTrie().TryUpdate(candidate, candidate)
		claudeContext.VoteTrie().TryUpdate(candidate, candidate)
	}
