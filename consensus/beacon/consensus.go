...
var (
	...
	beaconNonce      = model.EncodeNonce(0) // The default block nonce in the beacon consensus
)
...
// https://github.com/entropy/EIPs/blob/master/EIPS/eip-3675.md
//
...
func (beacon *Beacon) Author(header *model.Header) (common.Address, error) {
...
// stock Entropy consensus engine.
func (beacon *Beacon) VerifyHeader(chain consensus.ChainHeaderReader, header *model.Header, seal bool) error {
...
func (beacon *Beacon) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*model.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
// rules of the Entropy consensus engine.
func (beacon *Beacon) VerifyUncles(chain consensus.ChainReader, block *model.Block) error {
...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *model.Header) error {
...
func (beacon *Beacon) verifyHeaders(chain consensus.ChainHeaderReader, headers []*model.Header, ancestor *model.Header) (chan<- struct{}, <-chan error) {
	var (
		abort   = make(chan struct{})
		results = make(chan error, len(headers))
	)
...
func (beacon *Beacon) Prepare(chain consensus.ChainHeaderReader, header *model.Header) error {
...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *model.Header, state *state.StateDB, txs []*model.Transaction, uncles []*model.Header) {
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *model.Header, state *state.StateDB, txs []*model.Transaction, uncles []*model.Header, receipts []*model.Receipt) (*model.Block, error) {
...
func (beacon *Beacon) Seal(chain consensus.ChainHeaderReader, block *model.Block, results chan<- *model.Block, stop <-chan struct{}) error {
	if !beacon.IsPoSHeader(block.Header()) {
...
func (beacon *Beacon) SealHash(header *model.Header) common.Hash {
...
func (beacon *Beacon) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *model.Header) *big.Int {
...
func (beacon *Beacon) IsPoSHeader(header *model.Header) bool {
