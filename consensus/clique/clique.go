...
// Entropy testnet following the Ropsten attacks.
type Clique struct {
	...
	signer common.Address // Entropy address of the signing key
...
func New(config *config.CliqueConfig, db database.Database) *Clique {
...
func (clique *Clique) VerifyHeaders(chain consensus.ChainReader, headers []*model.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	go func() {
		for i, header := range headers {
			err := clique.verifyHeader(chain, header, headers[:i])
...
func (clique *Clique) verifyHeader(chain consensus.ChainReader, header *model.Header, parents []*model.Header) error {
	...
	checkpoint := (number % clique.config.Epoch) == 0
...
func (clique *Clique) verifyCascadingFields(chain consensus.ChainReader, header *model.Header, parents []*model.Header) error {
	...
	if parent.Time.Uint64()+clique.config.Period > header.Time.Uint64() {
...
func (clique *Clique) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*model.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number%checkpointInterval == 0 {
			if s, err := loadSnapshot(clique.config, clique.signatures, clique.db, hash); err == nil {
		...
		if number == 0 {
			...
			if err := clique.VerifyHeader(chain, genesis, false); err != nil {
				return nil, err
			}
			...
			snap = newSnapshot(clique.config, clique.signatures, 0, genesis.Hash(), signers)
			if err := snap.store(clique.db); err != nil {
				return nil, err
			}
	...
	clique.recents.Add(snap.Hash, snap)
...
func (clique *Clique) VerifyUncles(chain consensus.ChainReader, block *model.Block) error {
...
func (clique *Clique) Prepare(chain consensus.ChainReader, header *model.Header) error {
	...
	snap, err := clique.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return err
	}
	if number%clique.config.Epoch != 0 {
		...
		if len(addresses) > 0 {
			...
			if clique.proposals[header.Coinbase] {
	...
	header.Time = new(big.Int).Add(parent.Time, new(big.Int).SetUint64(clique.config.Period))
...
func (clique *Clique) Finalize(chain consensus.ChainReader, header *model.Header, state *state.StateDB, txs []*model.Transaction, uncles []*model.Header, receipts []*model.Receipt, claudeContext *model.ClaudeContext) (*model.Block, error) {
...
// with.
func (clique *Clique) Authorize(signer common.Address, signFn SignerFn) {
	clique.lock.Lock()
	defer clique.lock.Unlock()

	clique.signer = signer
	clique.signFn = signFn
}

// Seal implements consensus.Engine, attempting to create a sealed block using
// the local signing credentials.
func (clique *Clique) Seal(chain consensus.ChainReader, block *model.Block, stop <-chan struct{}) (*model.Block, error) {
	header := block.Header()
	...
	clique.lock.RLock()
	signer, signFn := clique.signer, clique.signFn
	clique.lock.RUnlock()
