...
func New(config Config, notify []string, noverify bool) *Ethash {
	if config.CachesInMem <= 0 {
		log.Warningf("One ethash cache must always be in memory. requested=%d", config.CachesInMem)
		config.CachesInMem = 1
	}
	if config.CacheDir != "" && config.CachesOnDisk > 0 {
		log.Debugf("Disk storage enabled for ethash caches. dir=%s, count=%d", config.CacheDir, config.CachesOnDisk)
	}
	if config.DatasetDir != "" && config.DatasetsOnDisk > 0 {
		log.Debugf("Disk storage enabled for ethash DAGs. dir=%s, count=%d", config.DatasetDir, config.DatasetsOnDisk)
	}
	ethash := &Ethash{
		config:   config,
...
// all blocks' seal as valid, though they still have to conform to the Entropy
// consensus rules.
...
// still have to conform to the Entropy consensus rules.
...
// they still have to conform to the Entropy consensus rules.
...
func (ethash *Ethash) APIs(consensus.ChainHeaderReader) []rpc.API {
