...
type Ethash struct {
	...
	workCh       chan *model.Block // Notification channel to push new work to remote sealer
	resultCh     chan *model.Block // Channel used by mining threads to return result
...
func New(config Config, notify []string) *Ethash {
	...
	ethash := &Ethash{
		...
		workCh:       make(chan *model.Block),
		resultCh:     make(chan *model.Block),
