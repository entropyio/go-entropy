...
func TestRemoteMultiNotify(t *testing.T) {
	...
	results := make(chan *model.Block, cap(sink))

	// Stream a lot of work task and ensure all the notifications bubble out.
	for i := 0; i < cap(sink); i++ {
		header := &model.Header{Number: big.NewInt(int64(i)), Difficulty: big.NewInt(100)}
		block := model.NewBlockWithHeader(header)
...
func TestStaleSubmission(t *testing.T) {
	...
	fakeNonce, fakeDigest := model.BlockNonce{0x01, 0x02, 0x03}, common.HexToHash("deadbeef")
	...
	results := make(chan *model.Block, 16)

	for id, c := range testcases {
		for _, h := range c.headers {
			ethash.Seal(nil, model.NewBlockWithHeader(h), results, nil)
		}
