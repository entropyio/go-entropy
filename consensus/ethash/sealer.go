...
func (ethash *Ethash) remote(notify []string, noverify bool) {
		...
		for i, url := range notify {
			...
			go func(req *http.Request, url string) {
				...
				if err != nil {
					...
					log.Debugf("Notified remote miner", "miner", url, "hash", common.HexToHash(work[0]), "target", work[2])
					res.Body.Close()
				}
			}(notifyReqs[i], url)
		}
	...
	makeWork := func(block *model.Block) {
	...
	submitWork := func(nonce model.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		if currentBlock == nil {
		...
		if block == nil {
			log.Warning("Work submitted but none pending", "sealhash", sealhash, "curnumber", currentBlock.NumberU64())
			return false
		}
