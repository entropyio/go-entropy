...
func (ethash *Ethash) Seal(chain consensus.ChainReader, block *model.Block, results chan<- *model.Block, stop <-chan struct{}) error {
	...
	if ethash.config.PowMode == ModeFake || ethash.config.PowMode == ModeFullFake {
		header := block.Header()
		header.Nonce, header.MixDigest = model.BlockNonce{}, common.Hash{}
		select {
		case results <- block.WithSeal(header):
		default:
			log.Warning("Sealing result is not read by miner", "mode", "fake", "sealhash", ethash.SealHash(block.Header()))
		}
		return nil
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warning("Sealing result is not read by miner", "mode", "local", "sealhash", ethash.SealHash(block.Header()))
			}
			close(abort)
		case <-ethash.update:
...
type remoteSealer struct {
	works        map[common.Hash]*model.Block
...
func startRemoteSealer(ethash *Ethash, urls []string, noverify bool) *remoteSealer {
	...
	s := &remoteSealer{
		...
		notifyCtx:    ctx,
		cancelNotify: cancel,
		works:        make(map[common.Hash]*model.Block),
...
func (s *remoteSealer) loop() {
	defer func() {
		log.Debug("Ethash remote sealer is exiting")
		s.cancelNotify()
		s.reqWG.Wait()
		close(s.exitCh)
	}()
...
func (s *remoteSealer) makeWork(block *model.Block) {
...
func (s *remoteSealer) sendNotification(ctx context.Context, url string, json []byte, work [4]string) {
	...
	if err != nil {
		log.Warning("Can't create remote miner notification", "err", err)
		return
	}
...
func (s *remoteSealer) submitWork(nonce model.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
	if s.currentBlock == nil {
	...
	if block == nil {
		log.Warning("Work submitted but none pending", "sealhash", sealhash, "curnumber", s.currentBlock.NumberU64())
		return false
	}
	...
	if !s.noverify {
		if err := s.ethash.verifySeal(nil, header, true); err != nil {
			log.Warning("Invalid proof-of-work submitted", "sealhash", sealhash, "elapsed", time.Since(start), "err", err)
			return false
		}
	}
	// Make sure the result channel is assigned.
	if s.results == nil {
		log.Warning("Ethash result channel is empty, submitted mining result is rejected")
		return false
	}
	log.Debug("Verified correct proof-of-work", "sealhash", sealhash, "elapsed", time.Since(start))
	...
	if solution.NumberU64()+staleThreshold > s.currentBlock.NumberU64() {
		...
		case s.results <- solution:
			...
			return true
		default:
			log.Warning("Sealing result is not read by miner", "mode", "remote", "sealhash", sealhash)
			return false
		}
	}
	// The submitted block is too old to accept, drop it.
	log.Warning("Work submitted is too old", "number", solution.NumberU64(), "sealhash", sealhash, "hash", solution.Hash())
	return false
}
