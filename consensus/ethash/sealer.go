...
func (ethash *Ethash) Seal(chain consensus.ChainReader, block *model.Block, stop <-chan struct{}) (*model.Block, error) {
	...
	if ethash.config.PowMode == ModeFake || ethash.config.PowMode == ModeFullFake {
		...
		log.Debugf("Pow Seal in Fake mode, response block header number=%d", block.Header().Number)
...
func (ethash *Ethash) remote(notify []string) {
	...
	submitWork := func(nonce model.BlockNonce, mixDigest common.Hash, hash common.Hash) bool {
		...
		if err := ethash.VerifySeal(nil, header); err != nil {
			log.Warning("Invalid proof-of-work submitted", "hash", hash, "err", err)
			return false
		}
