...
func (t *udp) findnode(toid NodeID, toaddr *net.UDPAddr, target NodeID) ([]*Node, error) {
	...
	errc := t.pending(toid, neighborsPacket, func(r interface{}) bool {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				log.Debugf("Invalid neighbor node received", "ip", rn.IP, "addr", toaddr, "err", err)
				continue
			}
		...
		case r := <-t.gotreply:
			...
			for el := plist.Front(); el != nil; el = el.Next() {
				...
				if p.from == r.from && p.ptype == r.ptype {
					...
					// required for packet model that expect multiple
					// reply packets.
...
func init() {
	...
	for n := 0; ; n++ {
		...
		size, _, err := rlputil.EncodeToReader(p)
		if err != nil {
...
func (t *udp) write(toaddr *net.UDPAddr, what string, packet []byte) error {
	...
	//log.Debugf(">> "+what, "addr", toaddr, "err", err)
	return err
}
...
func encodePacket(priv *ecdsa.PrivateKey, ptype byte, req interface{}) (packet, hash []byte, err error) {
	...
	b.Write(headSpace)
	b.WriteByte(ptype)
	if err := rlputil.Encode(b, req); err != nil {
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	log.Debugf("<< "+packet.name(), "addr", from, "err", err)
	return err
}
...
func decodePacket(buf []byte) (packet, NodeID, []byte, error) {
	...
	s := rlputil.NewStream(bytes.NewReader(sigdata[1:]), 0)
	err = s.Decode(req)
