...
import (
	...
	"github.com/entropyio/go-entropy/server/p2p/discover/v5wire"
...
func (t *UDPv5) send(toID enode.ID, toAddr *net.UDPAddr, packet v5wire.Packet, c *v5wire.Whoareyou) (v5wire.Nonce, error) {
	...
	if err != nil {
		t.log.Warning(">> "+packet.Name(), "id", toID, "addr", addr, "err", err)
		return nonce, err
	}
	_, err = t.conn.WriteToUDP(enc, toAddr)
	t.log.Debug(">> "+packet.Name(), "id", toID, "addr", addr)
	return nonce, err
}
...
func (t *UDPv5) handlePacket(rawpacket []byte, fromAddr *net.UDPAddr) error {
	...
	if packet.Kind() != v5wire.WhoareyouPacket {
		...
		t.log.Debug("<< "+packet.Name(), "id", fromID, "addr", addr)
	}
...
func (t *UDPv5) handleUnknown(p *v5wire.Unknown, fromID enode.ID, fromAddr *net.UDPAddr) {
	...
	_, _ = crand.Read(challenge.IDNonce[:])
...
func (t *UDPv5) handleWhoareyou(p *v5wire.Whoareyou, fromID enode.ID, fromAddr *net.UDPAddr) {
	...
	t.log.Debug("<< "+p.Name(), "id", c.node.ID(), "addr", fromAddr)
	c.handshakeCount++
	c.challenge = p
	p.Node = c.node
	t.sendCall(c)
