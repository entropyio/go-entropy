...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	if err == nil {
		...
		log.Debugf("Revalidated node", "b", bi, "id", last.ID)
		b.bump(last)
		return
	}
	...
	if r := tab.replace(b, last); r != nil {
		log.Debugf("Replaced dead node", "b", bi, "id", last.ID, "ip", last.IP, "r", r.ID, "rip", r.IP)
	} else {
		//log.Debugf("Removed dead node", "b", bi, "id", last.ID, "ip", last.IP)
	}
}
...
func (tab *Table) closest(target common.Hash, nresults int) *nodesByDistance {
	...
	closeObj := &nodesByDistance{target: target}
	for _, b := range tab.buckets {
		for _, n := range b.entries {
			closeObj.push(n, nresults)
		}
	}
	return closeObj
}
