...
var log = logger.NewLogger("[dnsdisc]")
...
func NewClient(cfg Config) *Client {
	...
	return &Client{cfg: cfg, entries: cache, clock: timeutil.System{}}
}
...
func (c *Client) resolveRoot(ctx context.Context, loc *linkEntry) (rootEntry, error) {
	...
	log.Debug("Updating DNS discovery root", "tree", loc.domain, "err", err)
	if err != nil {
...
func (c *Client) doResolveEntry(ctx context.Context, domain, hash string) (entry, error) {
	...
	log.Debug("DNS discovery lookup", "name", name, "err", err)
	if err != nil {
		return nil, err
	}
	for _, txt := range txts {
...
func (it *randomIterator) nextNode() *enode.Node {
	for {
		...
		if err != nil {
			...
			log.Debug("Error in DNS random node sync", "tree", ct.loc.domain, "err", err)
			continue
		}
		if n != nil {
			return n
