...
func TestProtocolHandshakeErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			code: handshakeMsg,
			msg:  []byte{1, 2, 3},
			err:  newPeerError(errInvalidMsg, "(code 0) (size 4) rlputil: expected input list for p2p.protoHandshake"),
		},
		{
			code: handshakeMsg,
...
func TestRLPXFrameRW(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		payload, _ := ioutil.ReadAll(msg.Payload)
		wantPayload, _ := rlputil.EncodeToBytes(wmsg)
		if !bytes.Equal(payload, wantPayload) {
...
var eip8HandshakeAuthTests = []handshakeAuthTest{
	...
	// (Auth₃) RLPx v4 EIP-8 encoding with version 56, additional list elements
	{
		...
		`,
		wantVersion: 56,
		wantRest:    []rlputil.RawValue{{0x01}, {0x02}, {0xC2, 0x04, 0x05}},
	},
}
...
var eip8HandshakeRespTests = []handshakeAckTest{
	...
	// (Ack₃) EIP-8 encoding with version 57, additional list elements
	{
		...
		`,
		wantVersion: 57,
		wantRest:    []rlputil.RawValue{{0x06}, {0xC2, 0x07, 0x08}, {0x81, 0xFA}},
	},
}
