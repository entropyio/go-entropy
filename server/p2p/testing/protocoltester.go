...
func (t *ProtocolTester) Connect(selfID discover.NodeID, peers ...*adapters.NodeConfig) {
	for _, peer := range peers {
		log.Debug(fmt.Sprintf("start node %v", peer.ID))
		...
		log.Debug(fmt.Sprintf("connect to %v", peer.ID))
...
// mustEncodeMsg uses rlputil to encode a message.
// In case of error it panics.
func mustEncodeMsg(msg interface{}) []byte {
	contentEnc, err := rlputil.EncodeToBytes(msg)
	if err != nil {
