...
// thus one cannot have multiple expects for the SAME peer with DIFFERENT message model
...
func (s *ProtocolSession) trigger(trig Trigger) error {
	...
	go func() {
		log.Debugf(fmt.Sprintf("trigger %v (%v)....", trig.Msg, trig.Code))
		errc <- mockNode.Trigger(&trig)
		log.Debugf(fmt.Sprintf("triggered %v (%v)", trig.Msg, trig.Code))
	}()
