	...
	if !srv.NoDiscovery || srv.DiscoveryV5 {
		...
		if srv.NAT != nil {
			if !realaddr.IP.IsLoopback() {
				go nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "entropy discovery")
			}
...
func (srv *Server) startListening() error {
	...
	if !laddr.IP.IsLoopback() && srv.NAT != nil {
		srv.loopWG.Add(1)
		go func() {
			nat.Map(srv.NAT, srv.quit, "tcp", laddr.Port, laddr.Port, "entropy p2p")
			srv.loopWG.Done()
		}()
	}
	return nil
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *discover.Node) error {
	...
	if dialDest != nil && c.id != dialDest.ID {
		//log.Debug("Dialed identity mismatch", "want", c, dialDest.ID)
	...
	if phs.ID != c.id {
		log.Debug("Wrong devp2p handshake identity", "err", phs.ID)
		return DiscUnexpectedIdentity
	}
