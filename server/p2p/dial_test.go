...
func runDialTest(t *testing.T, test dialtest) {
	...
	for i, round := range test.rounds {
		...
		newTask := test.init.newTasks(running, pm(round.peers), vtime)
		if !sametasks(newTask, round.new) {
			t.Errorf("ERROR round %d: got %v\nwant %v\nstate: %v\nrunning: %v",
				i, spew.Sdump(newTask), spew.Sdump(round.new), spew.Sdump(test.init), spew.Sdump(running))
		}
		t.Logf("round %d new tasks: %s", i, strings.TrimSpace(spew.Sdump(newTask)))
