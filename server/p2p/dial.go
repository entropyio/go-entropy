	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				//d.log.Debug("Discarding dial candidate", "id", node.ID(), "ip", node.IP(), "reason", err)
			} else {
		...
		case node := <-d.addStaticCh:
			...
			d.log.Debug("Adding static node", "id", id, "ip", node.IP(), "added", !exists)
			if exists {
				continue loop
			}
...
func (d *dialScheduler) startDial(task *dialTask) {
	d.log.Debug("Starting p2p dial", "id", task.dest.ID(), "ip", task.dest.IP(), "flag", task.flags)
