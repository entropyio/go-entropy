...
type topicTable struct {
	...
	requestCnt            uint64
	lastGarbageCollection timeutil.AbsTime
}
...
func (t *topicTable) checkDeleteNode(node *Node) {
	if n, ok := t.nodes[node]; ok && len(n.entries) == 0 && n.noRegUntil < timeutil.Now() {
...
func (t *topicTable) addEntry(node *Node, topic Topic) {
	...
	entry := &topicEntry{
		...
		fifoIdx: fifoIdx,
		node:    node,
		expire:  tm + timeutil.AbsTime(fallbackRegistrationExpiry),
	}
	if printTestImgLogs {
...
func (t *topicTable) deleteEntry(e *topicEntry) {
	if printTestImgLogs {
		fmt.Printf("*- %d %v %016x %016x\n", timeutil.Now()/1000000, e.topic, t.self.sha[:8], e.node.sha[:8])
	}
...
func (t *topicTable) useTicket(node *Node, serialNo uint32, topics []Topic, idx int, issueTime uint64, waitPeriods []uint32) (registered bool) {
	...
	if serialNo != n.lastUsedTicket {
		...
		n.noRegUntil = tm + timeutil.AbsTime(noRegTimeout())
		t.storeTicketCounters(node)
	}

	currTime := uint64(tm / timeutil.AbsTime(time.Second))
	...
	if relTime >= -1 && relTime <= regTimeWindow+1 { // give clients a little security margin on both ends
		if e := n.entries[topics[idx]]; e == nil {
			...
			e.expire = tm + timeutil.AbsTime(fallbackRegistrationExpiry)
		}
		return true
	}
...
func (t *topicTable) getTicket(node *Node, topics []Topic) *ticket {
	...
	tic := &ticket{
		...
		regTime:   make([]timeutil.AbsTime, len(topics)),
	}
	for i, topic := range topics {
		...
		tic.regTime[i] = now + timeutil.AbsTime(waitPeriod)
	}
	return tic
}
...
type waitControlLoop struct {
	lastIncoming timeutil.AbsTime
	waitPeriod   time.Duration
}

func (w *waitControlLoop) registered(tm timeutil.AbsTime) {
	w.waitPeriod = w.nextWaitPeriod(tm)
	w.lastIncoming = tm
}

func (w *waitControlLoop) nextWaitPeriod(tm timeutil.AbsTime) time.Duration {
...
func (w *waitControlLoop) hasMinimumWaitPeriod() bool {
	return w.nextWaitPeriod(timeutil.Now()) == minWaitPeriod
}
