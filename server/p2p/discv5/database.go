...
func (db *nodeDB) storeRLP(key []byte, val interface{}) error {
	blob, err := rlputil.EncodeToBytes(val)
	if err != nil {
		return err
	}
	return db.lvl.Put(key, blob, nil)
...
func (db *nodeDB) fetchRLP(key []byte, val interface{}) error {
	...
	err = rlputil.DecodeBytes(blob, val)
	if err != nil {
		log.Warningf("key %x (%T) %v", key, val, err)
	}
	return err
}
...
func nextNode(it iterator.Iterator) *Node {
	for end := false; !end; end = !it.Next() {
		...
		var n Node
		if err := rlputil.DecodeBytes(it.Value(), &n); err != nil {
			log.Warning(fmt.Sprintf("invalid node %x: %v", id, err))
			continue
		}
		return &n
	}
