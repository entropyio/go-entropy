...
func (ref ticketRef) topicRegTime() timeutil.AbsTime {
	return ref.t.regTime[ref.idx]
}

func pongToTicket(localTime timeutil.AbsTime, topics []Topic, node *Node, p *ingressPacket) (*ticket, error) {
	...
	t := &ticket{
		...
		topics:    topics,
		pong:      p.rawData,
		regTime:   make([]timeutil.AbsTime, len(wps)),
	}
	// Convert wait periods to local absolute time.
	for i, wp := range wps {
		t.regTime[i] = localTime + timeutil.AbsTime(time.Second*time.Duration(wp))
	}
	return t, nil
}

func ticketToPong(t *ticket, pong *pong) {
	pong.Expiration = uint64(t.issueTime / timeutil.AbsTime(time.Second))
...
type sentQuery struct {
	sent   timeutil.AbsTime
	lookup lookupInfo
}

type topicTickets struct {
	buckets    map[timeBucket][]ticketRef
	nextLookup timeutil.AbsTime
	nextReg    timeutil.AbsTime
}
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	...
	if s.tickets[topic] == nil {
		log.Warning("Removing non-existent discovery topic", "topic", topic)
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < timeutil.Now() {
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		log.Warning("Adding ticket to non-existent topic", "topic", topic)
		return
	}
	bucket := timeBucket(r.t.regTime[r.idx] / timeutil.AbsTime(ticketTimeBucketLen))
	tickets.buckets[bucket] = append(tickets.buckets[bucket], r)
	r.t.refCnt++

	min := timeutil.Now() - timeutil.AbsTime(collectFrequency)*maxCollectDebt
	if tickets.nextLookup < min {
		tickets.nextLookup = min
	}
	tickets.nextLookup += timeutil.AbsTime(collectFrequency)
...
func (s *ticketStore) nextFilteredTicket() (*ticketRef, time.Duration) {
	...
	for {
		...
		regTime := now + timeutil.AbsTime(wait)
...
func (s *ticketStore) ticketRegistered(ref ticketRef) {
	...
	tickets := s.tickets[topic]
	min := now - timeutil.AbsTime(registerFrequency)*maxRegisterDebt
	if min > tickets.nextReg {
		tickets.nextReg = min
	}
	tickets.nextReg += timeutil.AbsTime(registerFrequency)
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	...
	bucket := timeBucket(ref.t.regTime[ref.idx] / timeutil.AbsTime(ticketTimeBucketLen))
	list := tickets.buckets[bucket]
	idx := -1
	for i, bt := range list {
		if bt.t == ref.t {
...
func (s *ticketStore) adjustWithTicket(now timeutil.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *ticketStore) addTicket(localTime timeutil.AbsTime, pingHash []byte, ticket *ticket) {
	...
	bucket := timeBucket(localTime / timeutil.AbsTime(ticketTimeBucketLen))
...
func (s *ticketStore) canQueryTopic(node *Node, topic Topic) bool {
	...
	if qq != nil {
		now := timeutil.Now()
		for _, sq := range qq {
			if sq.lookup.topic == topic && sq.sent > now-timeutil.AbsTime(topicQueryResend) {
				return false
			}
		}
	}
...
func (s *ticketStore) cleanupTopicQueries(now timeutil.AbsTime) {
	if s.nextTopicQueryCleanup > now {
		return
	}
	exp := now - timeutil.AbsTime(topicQueryResend)
	...
	s.nextTopicQueryCleanup = now + timeutil.AbsTime(topicQueryTimeout)
}
...
func (s *ticketStore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	...
	q, ok := qq[hash]
	if !ok || now > q.sent+timeutil.AbsTime(topicQueryTimeout) {
		return true
	}
	inside := float64(0)
	if len(nodes) > 0 {
...
type topicRadiusBucket struct {
	...
	lastTime   timeutil.AbsTime
	value      float64
	lookupSent map[common.Hash]timeutil.AbsTime
}

func (b *topicRadiusBucket) update(now timeutil.AbsTime) {
	if now == b.lastTime {
		return
	}
	...
	for target, tm := range b.lookupSent {
		if now-tm > timeutil.AbsTime(respTimeout) {
...
func (b *topicRadiusBucket) adjust(now timeutil.AbsTime, inside float64) {
	b.update(now)
	if inside <= 0 {
...
func (r *topicRadius) recalcRadius() (radius uint64, radiusLookup int) {
	...
	if slopeCross != maxBucket && (minRadBucket <= maxBucket || r.needMoreLookups(maxBucket+lookupWidth, len(r.buckets)-1, maxValue)) {
		for len(r.buckets) <= maxBucket+lookupWidth {
			r.buckets = append(r.buckets, topicRadiusBucket{lookupSent: make(map[common.Hash]timeutil.AbsTime)})
		}
...
func (r *topicRadius) nextTarget(forceRegular bool) lookupInfo {
	if !forceRegular {
		...
		if radiusLookup != -1 {
			...
			r.buckets[radiusLookup].lookupSent[target] = timeutil.Now()
...
func (r *topicRadius) adjustWithTicket(now timeutil.AbsTime, targetHash common.Hash, t ticketRef) {
...
func (r *topicRadius) adjust(now timeutil.AbsTime, targetHash, addrHash common.Hash, inside float64) {
