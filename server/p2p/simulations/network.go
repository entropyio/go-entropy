...
func (net *Network) NewNodeWithConfig(conf *adapters.NodeConfig) (*Node, error) {
	...
	log.Debug(fmt.Sprintf("node %v created", conf.ID))
...
func (net *Network) startWithSnapshots(id discover.NodeID, snapshots map[string][]byte) error {
	...
	log.Debug(fmt.Sprintf("starting node %v: %v using %v", id, node.Up, net.nodeAdapter.Name()))
	if err := node.Start(snapshots); err != nil {
		log.Warning(fmt.Sprintf("start up failed: %v", err))
		return err
	}
	node.Up = true
...
func (net *Network) InitConn(oneID, otherID discover.NodeID) (*Conn, error) {
	...
	err = conn.nodesUp()
	if err != nil {
		log.Debug(fmt.Sprintf("nodes not up: %v", err))
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		if err := node.Stop(); err != nil {
			log.Warning(fmt.Sprintf("error stopping node %s", node.ID().TerminalString()), "err", err)
		}
	}
	close(net.quitc)
}
...
func (net *Network) executeControlEvent(event *Event) {
	log.Debug("execute control event", "type", event.Type, "event", event)
	switch event.Type {
	case EventTypeNode:
