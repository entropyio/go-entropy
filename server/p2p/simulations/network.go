...
func (net *Network) NewNodeWithConfig(conf *adapters.NodeConfig) (*Node, error) {
	...
	log.Debug("Node created", "id", conf.ID)
...
func (net *Network) startWithSnapshots(id enode.ID, snapshots map[string][]byte) error {
	...
	log.Debugf("Starting node", "id", id, "adapter", net.nodeAdapter.Name())
	if err := node.Start(snapshots); err != nil {
		log.Debugf("Node startup failed", "id", id, "err", err)
		return err
	}
	node.SetUp(true)
...
func (net *Network) initConn(oneID, otherID enode.ID) (*Conn, error) {
	...
	err = conn.nodesUp()
	if err != nil {
		log.Debug("Nodes not up", "err", err)
...
func (net *Network) Shutdown() {
	for _, node := range net.Nodes {
		...
		if err := node.Stop(); err != nil {
			log.Warning("Can't stop node", "id", node.ID(), "err", err)
		}
		// If the node has the close method, call it.
		if closer, ok := node.Node.(io.Closer); ok {
			if err := closer.Close(); err != nil {
				log.Warning("Can't close node", "id", node.ID(), "err", err)
			}
		}
	}
	close(net.quitc)
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/entropyio/go-entropy/issues/1177
	var node struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}
		...
		for {
			...
			case e := <-events:
				...
				if !e.Conn.Up {
					...
					log.Warning("load snapshot: unexpected disconnection", "one", e.Conn.One, "other", e.Conn.Other)
					continue
				}
...
func (net *Network) executeControlEvent(event *Event) {
	log.Debug("Executing control event", "type", event.Type, "event", event)
	switch event.Type {
	case EventTypeNode:
	...
	case EventTypeMsg:
		log.Warning("Ignoring control msg event")
	}
}
