...
func TestCorsHandler(t *testing.T) {
	...
	defer srv.stop()
	urlObj := "http://" + srv.listenAddr()

	resp := rpcRequest(t, urlObj, "origin", "test.com")
	assert.Equal(t, "test.com", resp.Header.Get("Access-Control-Allow-Origin"))

	resp2 := rpcRequest(t, urlObj, "origin", "bad")
...
func TestVhosts(t *testing.T) {
	...
	resp := rpcRequest(t, urlObj, "host", "test")
	assert.Equal(t, resp.StatusCode, http.StatusOK)

	resp2 := rpcRequest(t, urlObj, "host", "bad")
...
func TestWebsocketOrigins(t *testing.T) {
	...
	for _, tc := range tests {
		...
		urlObj := fmt.Sprintf("ws://%v", srv.listenAddr())
		for _, origin := range tc.expOk {
			if err := wsRequest(t, urlObj, "Origin", origin); err != nil {
