...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of entropy is "entropy". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Entropy", keep that.
	if name == "entropy" || name == "entropy-testnet" {
		name = "Entropy"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "entropy" instances.
var isOldEntropyResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	if warn, isOld := isOldEntropyResource[path]; isOld {
		oldpath := ""
		if c.name() == "geth" {
...
func (c *Config) NodeKey() *ecdsa.PrivateKey {
	...
	if c.DataDir == "" {
		...
		if err != nil {
			log.Critical(fmt.Sprintf("Failed to generate ephemeral node key: %v", err))
		}
		return key
	}
	...
	key, err := crypto.GenerateKey()
	if err != nil {
		log.Critical(fmt.Sprintf("Failed to generate node key: %v", err))
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-entropy-keystore")
		isEphemeral = true
	}
