...
func (s *Server) RegisterName(name string, rcvr interface{}) error {
	...
	/*
		for _, m := range methods {
			log.Debugf("Export RPC service [%s]: %s.%s", svc.typ.String(), name, formatName(m.method.Name))
		}

		for _, s := range subscriptions {
			log.Debugf("Export RPC subscription [%s]: %s.%s", svc.typ.String(), name, formatName(s.method.Name))
		}
	*/
...
func (s *Server) handle(ctx context.Context, codec ServerCodec, req *serverRequest) (interface{}, func()) {
	...
	//log.Debugf("3) Call JSON-RPC method: service=%s, method=%s, config=", req.svcname, req.callb.method.Name, arguments)
...
func (s *Server) readRequest(codec ServerCodec) ([]*serverRequest, bool, Error) {
	...
	for i, r := range reqs {
		...
		if r.isPubSub { // entropy_subscribe, r.method contains the subscription method name
