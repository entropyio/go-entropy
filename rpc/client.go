...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout entropy_subscribe, rpc_modules calls
)
...
// EthSubscribe registers a subscripion under the "entropy" namespace.
func (c *Client) EthSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "entropy", channel, args...)
}
...
func (c *Client) send(ctx context.Context, op *requestOp, msg interface{}) error {
	select {
	case c.requestOp <- op:
		//log.Debugf("1) Send JSON-RPC request: %s", msg)
		err := c.write(ctx, msg)
		c.sendDone <- err
		return err
	case <-ctx.Done():
...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		log.Debugf(fmt.Sprintf("reconnect failed: %v", err))
		return err
	}
	select {
		...
		case batch := <-c.readResp:
			for _, msg := range batch {
				...
				case msg.isNotification():
					log.Debugf("", "msg", fmt.Sprint("<-readResp: notification ", msg))
					c.handleNotification(msg)
				case msg.isResponse():
					//log.Debugf("4) Response JSON-RPC result: %s", msg)
					c.handleResponse(msg)
				default:
					log.Debugf("", "msg", fmt.Sprint("<-readResp: dropping weird message", msg))
					// TODO: maybe close
				}
			}
