...
import (
	...
	"github.com/entropyio/go-entropy/build/build"
)

var (
	// Files that end up in the entropy*.zip archive.
	entropyArchiveFiles = []string{
		"COPYING",
		executablePath("entropy"),
	}

	// Files that end up in the entropy-alltools*.zip archive.
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 9 {
			...
			log.Println("go-entropy requires at least Go version 1.9 and cannot")
...
func doArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archives (usually "entropystore/builds")`)
		ext    string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	if err := build.WriteArchive(entropy, entropyArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{entropy, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "entropy-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "entropy.exe" {
			entropyTool = file
		} else {
	...
	// first section contains the entropy binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Entropy":  entropyTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.entropy.nsi", filepath.Join(*workdir, "entropy.nsi"), 0644, nil)
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "entropy.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload = flag.String("upload", "", `Destination to upload the archive (usually "entropystore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	...
	if *local {
		...
		os.Rename("entropy.aar", filepath.Join(GOBIN, "entropy.aar"))
		return
	}
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "--tags", "ios", "-v", "github.com/entropy/go-entropy/mobile")
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Entropy.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Entropy.podspec", "--allow-warnings", "--verbose")
	}
}
